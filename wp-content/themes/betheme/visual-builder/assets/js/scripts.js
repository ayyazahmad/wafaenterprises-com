...
let sample_img = mfn.themepath+'/muffin-options/svg/placeholders/image.svg';
...
let previewTab;
let pageid = mfn.pageid;
let samplecontentid = mfn.sample_content_id;
let wpnonce = mfn.wpnonce;
var builder_type = mfn.builder_type;
...
var elements = {
    wrap: function(size) {
        ...
        var new_wrap = {};
        new_wrap = JSON.parse( JSON.stringify(mfn.elements.wrap) );

        new_wrap['title'] = 'Wrap';

        if( builder_type == 'header' ) {
            // header presets
            new_wrap.attr['css_advanced_flex_grow'] = {val: {desktop: '1', laptop: '1', tablet: '1', mobile: '1'}, selector: '.mcb-section .mcb-wrap-mfnuidelement', style: 'flex-grow'};
            new_wrap.attr['css_advanced_align_items'] = {val: {desktop: 'center', laptop: 'center', tablet: 'center', mobile: 'center'}, selector: '.mcb-section .mcb-wrap-mfnuidelement .mcb-wrap-inner', style: 'align-items'};
        }
    ...
    section: function() {
        var uid = getUid();
        //var new_section = {...mfn.elements.section};

        var section_class = 'mfn-default-section';

        var new_section = {};
        new_section = JSON.parse( JSON.stringify(mfn.elements.section) );
        new_section.uid = uid;
        ...
        if( builder_type == 'header' ) {
            // header presets
            new_section.attr['css_advanced_align_items'] = {val: {desktop: 'center'}, selector: '.mcb-section-mfnuidelement .section_wrapper', style: 'align-items'};
        }

        if( builder_type == 'megamenu' ){
            // megamenu presets
            new_section.attr['css_advanced_background_color'] = {val: '#ffffff', selector: '.mcb-section-mfnuidelement', style: 'background-color'};
            new_section.attr['css_advanced_align_items'] = {val: 'flex-start', selector: '.mcb-section-mfnuidelement .section_wrapper', style: 'align-items'};
        }

        mfn.pagedata.push(new_section);
    ...
    item: function(name, alias) {
        ...
        if( !_.has(new_item, 'attr') || Array.isArray(new_item['attr'])) new_item['attr'] = {};

        if(_.has(new_item, 'html')) {
            delete new_item.html;
        }else{
            be_layout.drag_new.force_rerender = true;
        }
        
        if(_.has(new_item, 'script')) delete new_item.script;
        ...
        if( builder_type.includes('archive-') && name == 'filters' ){
            new_item.attr['post_type'] = builder_type.replace('archive-', '');
        }

        if( builder_type == 'header' ) {

            if( name != 'header_logo' && name != 'header_search' && name != 'image' ) {
                new_item.attr['width_switcher'] = 'inline';
                html = html.replace('vb-item', 'vb-item mfn-item-inline');
            }
            ...
            if( name == 'header_logo' || name == 'image' ) {
                ...
                new_item.attr['css_advanced_flex'] = {val: '250px', selector: '.mcb-section .mcb-wrap .mcb-item-mfnuidelement', style: 'width'};
                be_layout.drag_new.force_rerender = true;
            }

            if( name == 'header_search' ) {
                new_item.attr['width_switcher'] = 'custom';
                new_item.attr['css_advanced_flex'] = {val: '300px', selector: '.mcb-section .mcb-wrap .mcb-item-mfnuidelement', style: 'width'};
            }

            if( name == 'header_icon' ) {
                new_item.attr['css_icon_count_top'] = {val: '-9px', selector: '.mcb-section .mcb-wrap .mcb-item-mfnuidelement .mfn-icon-box .icon-wrapper .header-cart-count,.mcb-section .mcb-wrap .mcb-item-mfnuidelement .mfn-icon-box .icon-wrapper .header-wishlist-count', style: 'top'};
                new_item.attr['css_icon_count_right'] = {val: '-11px', selector: '.mcb-section .mcb-wrap .mcb-item-mfnuidelement .mfn-icon-box .icon-wrapper .header-cart-count,.mcb-section .mcb-wrap .mcb-item-mfnuidelement .mfn-icon-box .icon-wrapper .header-wishlist-count', style: 'right'};
            }

        }
 
        mfn.pagedata.push(new_item);
...
function backToWidgets(){
    ...
    if( $('.mfn-ui').hasClass('mfn-class-editor') ) $('.mfn-ui').removeClass('mfn-class-editor');
    if( $content.find('body').hasClass('mfn-class-editor-preview') ) $content.find('body').removeClass('mfn-class-editor-preview');
    if( $('.mfn-ui').hasClass('mfn-class-editor-full') ) $('.mfn-ui').addClass('mfn-class-editor-full');
...
function showPrebuilts() {
    ...
    _.map( mfn.prebuilts, (item, i) => html += `
        <li class="category-all category-${item.category}" data-id="${i}">
        <div class="photo"><img loading="lazy" src="${mfn.themepath}/functions/builder/pre-built/images/${i}.png" alt="" /></div>
    ...
    if( $('body').hasClass('mfn-template-builder-blog') || $('body').hasClass('mfn-template-builder-portfolio') ){
      $('.header-prebuilt-sections').find('.mfn-dropdown-item[data-filter="category-tem"]').trigger('click');
    }
}
...
function showGlobals( $section_uid ) {
    ...
    _.map(mfn['global_sections'], function(el, i){
        if( i > 0 ){
    ...
    if( !$('.prebuilt-sections-list.global-sections li').length ){
        $('.prebuilt-sections-list.global-sections').append('<div class="mfn-form-row mfn-form-row "><div class="mfn-alert "><div class="alert-icon mfn-icon-information"></div><div class="alert-content"><p>No global sections have been created yet. If you would like to set global section, please <a target="_blank" href="'+mfn.adminurl+'edit.php?post_type=template&tab=section">create it first.</a></p></div></div></div>');
    }
}
...
function catchShortcuts(e) {
    if((e.ctrlKey || e.metaKey) && e.key == "s") {
        ...
        if( $('.mfn-ui').hasClass('mfn-class-editor') ){
            $('.mfn-class-editor .mfn-class-editor-update').trigger('click');
        }else if(!$('.btn-save-form-primary.btn-save-changes').hasClass('disabled')){
        ...
        if( iframe.window.getSelection().toString() == '' && !$content.find('.mfn-current-editing .mfn-inline-editor').is(":focus") && !$content.find('input').is(':focus') && !$content.find('textarea').is(':focus') && !$('input').is(':focus') && !$('textarea').is(':focus') && !$('textarea').is(':focus') && !$('.CodeMirror-code').is(':focus') ){
            ...
            if( $content.find('.mfn-current-editing').length ){
                contextMenu.item = $content.find('.mfn-current-editing').attr('data-uid');
                copypaste.copy(contextMenu.item);
            }
        }
        return false;
        ...
        if( !$('.components-modal__frame.yoast').length && !$content.find('.mfn-current-editing .mfn-inline-editor').is(":focus") && !$content.find('input').is(':focus') && !$content.find('textarea').is(':focus') && !$('input').is(':focus') && !$('textarea').is(':focus') && !$('.CodeMirror-code').is(':focus') ){
            ...
            if( $content.find('.mfn-current-editing').length ){
                contextMenu.item = $content.find('.mfn-current-editing').attr('data-uid');
                //let $el = $content.find('.vb-item[data-uid="'+contextMenu.item+'"]');
                copypaste.parent = $content.find('.vb-item[data-uid="'+contextMenu.item+'"]');
                copypaste.paste();
            }
        }
        return false;
...
function init() {
    ...
    if( builder_type == 'standard' && ( $content.find('.mfn-shop-archive-tmpl-builder').length || $content.find('.mfn-archive-product-tmpl-builder').length ) ){
        if( $content.find('.mfn-shop-archive-tmpl-builder').length ){
            $content.find('.mfn-shop-archive-tmpl-builder').addClass('block-pointer-inside').append('<a href="'+mfn.adminurl+'post.php?post='+$content.find('.mfn-shop-archive-tmpl-builder').attr("data-id")+'&action='+(mfn.be_slug == 'be' || mfn.be_slug == 'mfn' ? 'mfn' : mfn.be_slug)+'-live-builder" target="_blank" data-tooltip="Edit Shop Template" class="btn-edit-template" data-position="before">Edit shop template</a>');
        }else{
            $content.find('.mfn-archive-product-tmpl-builder').addClass('block-pointer-inside').append('<a href="'+mfn.adminurl+'post.php?post='+$content.find('.mfn-archive-product-tmpl-builder').attr("data-id")+'&action='+(mfn.be_slug == 'be' || mfn.be_slug == 'mfn' ? 'mfn' : mfn.be_slug)+'-live-builder" target="_blank" data-tooltip="Edit Shop Template" class="btn-edit-template" data-position="before">Edit shop template</a>');
        }
    }

    if( builder_type == 'standard' && ( $content.find('.mfn-portfolio-tmpl-builder').length || $content.find('.mfn-archive-portfolio-tmpl-builder').length ) ){
        if( $content.find('.mfn-portfolio-tmpl-builder').length ) {
            $content.find('.mfn-portfolio-tmpl-builder').addClass('block-pointer-inside').append('<a href="'+mfn.adminurl+'post.php?post='+$content.find('.mfn-portfolio-tmpl-builder').attr("data-id")+'&action='+(mfn.be_slug == 'be' || mfn.be_slug == 'mfn' ? 'mfn' : mfn.be_slug)+'-live-builder" target="_blank" data-tooltip="Edit Portfolio Template" class="btn-edit-template" data-position="before">Edit Portfolio Template</a>');
        }else{
            $content.find('.mfn-archive-portfolio-tmpl-builder').addClass('block-pointer-inside').append('<a href="'+mfn.adminurl+'post.php?post='+$content.find('.mfn-archive-portfolio-tmpl-builder').attr("data-id")+'&action='+(mfn.be_slug == 'be' || mfn.be_slug == 'mfn' ? 'mfn' : mfn.be_slug)+'-live-builder" target="_blank" data-tooltip="Edit Portfolio Template" class="btn-edit-template" data-position="before">Edit Portfolio Template</a>');
        }
    }


    if( builder_type == 'standard' && $content.find('.mfn-archive-post-tmpl-builder').length ){
        $content.find('.mfn-archive-post-tmpl-builder').addClass('block-pointer-inside').append('<a href="'+mfn.adminurl+'post.php?post='+$content.find('.mfn-archive-post-tmpl-builder').attr("data-id")+'&action='+(mfn.be_slug == 'be' || mfn.be_slug == 'mfn' ? 'mfn' : mfn.be_slug)+'-live-builder" target="_blank" data-tooltip="Edit Post Template" class="btn-edit-template" data-position="before">Edit Post Template</a>');
    }

    if( $content.find('.mfn-main-slider.mfn-rev-slider').length ){
        $content.find('.mfn-main-slider.mfn-rev-slider').append('<a href="'+mfn.adminurl+'admin.php?page=revslider&view=slide&alias='+mfn.rev_slider_id+'" target="_blank" data-tooltip="Edit with Slider Revolution" class="btn-edit-slider" data-position="before">Edit Slider</a>');
    }
    ...
    if( $content.find('.mfn-global-section').length ){
        ...
        globalSections.each( (index, item) => {
            $(item).append('<a href="'+mfn.adminurl+'post.php?post='+$(item).attr('data-mfn-global')+'&action='+(mfn.be_slug == 'be' || mfn.be_slug == 'mfn' ? 'mfn' : mfn.be_slug)+'-live-builder" target="_blank" data-tooltip="Edit Global Section" class="btn-edit-section" data-position="before">Edit Global Section</a>');
        })
    }
    ...
    if( $content.find('.mfn-global-wrap').length ){
        ...
        globalWraps.each( (index, item) => {
            $(item).append('<a href="'+mfn.adminurl+'post.php?post='+$(item).attr('data-mfn-global')+'&action='+(mfn.be_slug == 'be' || mfn.be_slug == 'mfn' ? 'mfn' : mfn.be_slug)+'-live-builder" target="_blank" data-tooltip="Edit Global Wrap" class="btn-edit-wrap" data-position="before">Edit Global Wrap</a>');
        })
    }

    if( $content.find('#Footer').length ){
        $content.find('#Footer').append('<a href="'+mfn.adminurl+'widgets.php" target="_blank" data-tooltip="Edit Footer" class="btn-edit-footer" data-position="before">Edit Footer</a>');
    }

    if( $content.find('#Header_creative').length ){
        $content.find('#Header_creative').append('<a href="'+mfn.adminurl+'admin.php?page='+mfn.be_slug.replace('mfn', 'be')+'-options#header" target="_blank" data-tooltip="Edit Header" class="btn-edit-header" data-position="before">Edit Header</a>');
    } else if( $content.find('#Top_bar').length ){
        $content.find('#Top_bar').append('<a href="'+mfn.adminurl+'admin.php?page='+mfn.be_slug.replace('mfn', 'be')+'-options#header" target="_blank" data-tooltip="Edit Header" class="btn-edit-header" data-position="before">Edit Header</a>');
    }
    ...
    if( $content.find('.mfn-header-tmpl-builder:not(.mfn-builder-active)').length ){
      if (mfn.view == 'demo'){
        ...
        $content.find('.mfn-header-tmpl-builder:not(.mfn-builder-active)').append('<a href="'+mfn.adminurl+'post.php?post='+$content.find('.mfn-header-tmpl').attr("data-id")+'&action='+(mfn.be_slug == 'be' || mfn.be_slug == 'mfn' ? 'mfn' : mfn.be_slug)+'-live-builder" target="_blank" data-tooltip="Edit Header" class="btn-edit-header" data-position="before">Edit Header</a>');
      }
    }
    ...
    if( $content.find('.mfn-footer-tmpl-builder:not(.mfn-builder-active)').length ){
      if (mfn.view == 'demo'){
        ...
        $content.find('.mfn-footer-tmpl-builder:not(.mfn-builder-active)').append('<a href="'+mfn.adminurl+'post.php?post='+$content.find('.mfn-footer-tmpl').attr("data-id")+'&action='+(mfn.be_slug == 'be' || mfn.be_slug == 'mfn' ? 'mfn' : mfn.be_slug)+'-live-builder" target="_blank" data-tooltip="Edit Footer" class="btn-edit-footer" data-position="before">Edit Footer</a>');
      }
    }
...
function loopAllStyleFields(uid = false) {
    ...
    if( uid && mfn.pagedata.filter( (item) => item.uid == uid ).length ) {
        copied = JSON.parse( JSON.stringify(mfn.pagedata.filter( (item) => item.uid == uid )));
        if($content.find('.mfn-local-style-'+uid+'').length) $content.find('.mfn-local-style-'+uid+'').remove();
    } else {
        copied = JSON.parse( JSON.stringify(mfn.pagedata));
    }

    //console.log('loop all style fields');

    if( !uid ) {
        $.each(mfn.page_options, function(i, v) {
            if( i.includes('css_') ) {

                if( typeof v == 'object' && _.has(v, 'selector') && _.has(v, 'val') ) {
                    if( typeof v['val'] == 'object' ) {
                        if( _.has(v['val'], 'desktop') || _.has(v['val'], 'laptop') || _.has(v['val'], 'tablet') || _.has(v['val'], 'mobile') ){
                            devices.map( (dvc) => _.has(v['val'], dvc) ? prepareCss(v['selector'], v['style'], v['val'][dvc], pageid, dvc) : null );
                        }else{
                            prepareCss(v['selector'], v['style'], v['val'], pageid, 'desktop');
                        }
                    }else{
                        prepareCss(v['selector'], v['style'], v['val'], pageid, 'desktop');
                    }
                    ...
                    var sel_ex = i.split(':');
                    prepareCss(sel_ex[1], sel_ex[2], v, pageid, 'desktop');
                }
            }
        });

        if( mfn.classes.length ){
            copied = copied.concat(JSON.parse( JSON.stringify(mfn.classes)));
        }
    }

    if( uid && !copied.length ) {
        copied = mfn.classes.filter( (item) => item.uid == uid );
    }

    $.each(copied, function(i, v) {

        if( v === null ) { 
            delete(mfn.pagedata[i]); delete(copied[i]);
        }else{
        
            $.each(v.attr, function(a, attr) {

                if( a.includes('css_') && _.has(attr, 'val') ) {

                    if( _.has(attr, 'css_path') || _.has(attr, 'css_style') ) {
                        delete(mfn.pagedata[i]['attr'][a]); 
                        delete(copied[i]['attr'][a]);
                    }else if( (_.has(attr, 'selector') && attr.selector == null) || (_.has(attr, 'style') && attr.style == null) ) {
                        delete(mfn.pagedata[i]['attr'][a]); 
                        delete(copied[i]['attr'][a]);
                    }else{
                        ...
                        if( typeof attr['val'] == 'object' ){
                            ...
                            _.map(attr['val'], (y, x) => {
                                ...
                                if( x == 'desktop' || x == 'laptop' || x == 'tablet' || x == 'mobile' ){

                                    if( attr['style'] == 'transform' ) {
                                        attr['val'][x]['string'] = `matrix(${attr['val'][x]['scaleX']}, ${attr['val'][x]['skewY']}, ${attr['val'][x]['skewX']}, ${attr['val'][x]['scaleY']}, ${attr['val'][x]['translateX']}, ${attr['val'][x]['translateY']}) rotate(${attr['val'][x]['rotate']}deg)`;
                                    }
                                    ...
                                    if( typeof y == 'object' ) {
                                        _.map(y, (c, z) => {
                                            ...
                                            if( (attr['style'] == 'transform') && z != 'string' ) return;
                                    
                                            if( attr['style'] != 'typography' ) sn = attr['style']+'-'+z;
                                            if( attr['style'] == 'transform' ) sn = attr['style'];

                                            prepareCss(attr['selector'], sn, c, v.uid, x);
                                        });
                                    }else{
                                        prepareCss(attr['selector'], attr['style'], y, v.uid, x);

                                        if( _.has( additional_css, a) ){
                                            if( y && _.has( additional_css[a]['rewrites'], y) ) {
                                                prepareCss( additional_css[a]['selector'].replace('mfnuidelement', v.uid), additional_css[a]['style'], additional_css[a]['rewrites'][y], v.uid, x);
                                            }else{
                                                prepareCss(additional_css[a]['selector'].replace('mfnuidelement', v.uid), additional_css[a]['style'], '', v.uid, x);
                                            }
                                        }
                                    ...
                                    if( (attr['style'] == 'gradient' || attr['style'] == 'filter' || attr['style'] == 'backdrop-filter') && x != 'string' ) return;

                                    let sn = x;

                                    if( attr['style'] == 'gradient' || attr['style'] == 'filter' || attr['style'] == 'backdrop-filter' ) {
                                        sn = attr['style'];
                                    }
                                    ...
                                    if( typeof y == 'object' && y.length ){
                                        _.map(y, (c,z) => {
                                            sn = z;
                                            prepareCss(attr['selector'], sn, c, v.uid, 'desktop');
                                        });
                                    }else{
                                        prepareCss(attr['selector'], sn, y, v.uid, 'desktop');
                                    }
                            ...
                            prepareCss(attr['selector'], attr['style'], attr['val'], v.uid, 'desktop');
                        }
                    }
                ...
                if( v.jsclass == 'hotspot' && a == 'hotspots' ) {
                    $.each(attr, function(h, ht) {
                        ...
                        $.each(ht['val'], function(s, st) {
                            ...
                            if( _.has(st, 'val') && typeof st['val'] == 'object' ){
                                _.map(st['val'], (k, r) => {
                                    prepareCss(st['selector'], st['style'], k, v.uid, r);
                                });
                            }else{
                                prepareCss(st['selector'], st['style'], attr['val'], v.uid, 'desktop');
                            }
...
var startY, startX, startWidth, endWidth = ui_mode == 'dev' ? 360 : 420;
var resizer_base = ui_mode == 'dev' ? 360 : 420;
...
function initDrag(e) {
    ...
    startWidth = ui_mode == 'dev' ? 360 : 420;
...
function doDrag(e) {
    ...
    if(endWidth < 1200 && endWidth >= resizer_base) {
        if( $('style.mfn-ui-sidebar-width-style').length ) $('style.mfn-ui-sidebar-width-style').remove();

        $('body').append(`<style class="mfn-ui-sidebar-width-style">.mfn-${ui_mode == 'dev' ? 'dev' : 'default'}-ui .mfn-ui{--mfn-ui-sidebar-width: ${endWidth}px;}</style>`);
...
let historyStorage = {
    ...
    add: function() {
        ...
        let new_hist = {
            'obj': JSON.parse( JSON.stringify(mfn.pagedata) ),
            'pageid': pageid,
...
function iframeReady(){
    ...
    }else if($content.find('.mfn-builder-content[data-id="'+mfn.pageid+'"]').length) {
        $builder = $content.find('.mfn-builder-content[data-id="'+mfn.pageid+'"]');
    }else{
        $builder = $content.find('#Content .entry-content > .mfn-builder-content');
    }

    mfn_multifield_field.init();

    $builder.addClass('mfn-builder-active');

    if( builder_type == 'megamenu' ){
        var page_options = mfn.page_options;
    ...
    $builder.on('click', '.section .mfn-option-dropdown .dropdown-wrapper a.mfn-dropdown-item', function(e) {
        ...
        if($(this).hasClass('mfn-section-hide')){
            ...
            if($it.hasClass('hide')){
                ...
                if( typeof mfn.pagedata.filter( (item) => item.uid == sec_uid )[0]['attr']['hide'] !== 'undefined' ){
                    delete( mfn.pagedata.filter( (item) => item.uid == sec_uid )[0]['attr']['hide'] );
                }
                ...
                mfn.pagedata.filter( (item) => item.uid == sec_uid )[0]['attr']['hide'] = '1';
    ...
    let contextMenu = {
        ...
        init_navigator: function() {
            ...
            $('.mfn-navigator').contextmenu(function(e) {
                ...
                if( $(e.target).closest('li') ) {
                    ...
                    contextMenu.item = $(e.target).closest('li').children('a').attr('data-uid');
                    contextMenu.obj_el = mfn.pagedata.filter( (item) => item.uid == contextMenu.item )[0];

                    $('.mfn-ui').append(contextMenu.dropdown);
                    contextMenu.active_dropdown = $('.mfn-builder-area-contextmenu');

                    let offset_top = e.pageY;

                    if( $('.mfn-topbar').length ) offset_top -= $('.mfn-topbar').outerHeight();

                    if( $(window).height() < (offset_top + contextMenu.active_dropdown.outerHeight()) ) {
                        contextMenu.active_dropdown.show().css({bottom: 0, top: 'initial'});
                    }else{
                        contextMenu.active_dropdown.show().css({top: offset_top});
                    }
                    
                    var diff = $(window).width() - e.pageX;

                    if(  diff < 200 ) {
                        contextMenu.active_dropdown.css({'right': diff, 'left': 'initial'});
                    }else{
                        contextMenu.active_dropdown.css({'left': e.pageX, 'right': 'initial'});
                    }

                    contextMenu.active_dropdown.find('.mfn-contextmenu-navigator').remove();

                    if( $(e.target).closest('li').hasClass('navigator-wrap-global') ) {
                        contextMenu.active_dropdown.find('.mfn-contextmenu-tools .mfn-context-resize-minus').addClass('disabled');
                        contextMenu.active_dropdown.find('.mfn-contextmenu-tools .mfn-context-resize-plus').addClass('disabled');
                        contextMenu.active_dropdown.find('.mfn-contextmenu-tools .mfn-context-clone').addClass('disabled');
                    }

                    if( $(e.target).closest('li').hasClass('navigator-wrap-global') || $(e.target).closest('li').hasClass('navigator-section-global') ) {
                        contextMenu.active_dropdown.find('.mfn-contextmenu-copystyle').remove();
                        contextMenu.active_dropdown.find('.mfn-contextmenu-pastestyle').remove();
                        contextMenu.active_dropdown.find('.mfn-contextmenu-resetstyle').remove();
                        contextMenu.active_dropdown.find('.mfn-contextmenu-save-class').remove();
                        contextMenu.active_dropdown.find('.mfn-contextmenu-convert-global-section').remove();
                        contextMenu.active_dropdown.find('.mfn-contextmenu-convert-global-wrap').remove();
                    }
                    ...
                    const isSectionEdited = contextMenu.active_dropdown.find('.mfn-context-header').attr('data-element') === 'Section';
                    const isWrapEdited = contextMenu.active_dropdown.find('.mfn-context-header').attr('data-element') === 'Wrap';

                    if( isGlobalSection && isSectionEdited ) {
                        contextMenu.active_dropdown.attr('hide-ui', 'section');
                    } else if( isGlobalWrap && isWrapEdited ) {
                        contextMenu.active_dropdown.attr('hide-ui', 'wrap');
                    } else {
                        contextMenu.active_dropdown.removeAttr('hide-ui');
                    }
                }

                $(document).bind('click', contextMenu.hideContext);
        ...
        init_iframe: function() {
            $builder.contextmenu(function(e) {
                ...
                if(e.target.closest('.vb-item')) {
                    ...
                    $content.find('body').append(contextMenu.dropdown);
                    contextMenu.active_dropdown = $content.find('.mfn-builder-area-contextmenu');

                    contextMenu.active_dropdown.removeClass('mfn-context-menu-section mfn-context-menu-wrap mfn-context-menu-item mfn-context-menu-global');

                    contextMenu.active_dropdown.find('.mfn-contextmenu-tools').remove();

                    var diff = iframe.window.jQuery('body').width() - e.pageX;

                    if(  diff < 200 ) {
                        contextMenu.active_dropdown.css({'right': 0, 'left': 'initial'});
                    }else{
                        contextMenu.active_dropdown.css({'left': e.pageX, 'right': 'initial'});
                    }

                    contextMenu.active_dropdown.show().css({top: e.pageY});

                    contextMenu.item = $(e.target).closest('.vb-item').attr('data-uid');
                ...
                $content.find('body').bind('click', contextMenu.hideContext);
            });
        },

        before: function() {
            if( contextMenu.active_dropdown ) contextMenu.active_dropdown.remove();
        },
        ...
        after: function(e) {
            ...
            if( contextMenu.obj_el.jsclass == 'section' ){
                contextMenu.active_dropdown.attr('data-edited', 'Section');
                contextMenu.active_dropdown.find('.mfn-context-header').html('Section');
            }else if( contextMenu.obj_el.jsclass == 'wrap' ){
                contextMenu.active_dropdown.attr('data-edited', 'Wrap');
                contextMenu.active_dropdown.find('.mfn-context-header').html('Wrap');
            }else{  
                contextMenu.active_dropdown.attr('data-edited', contextMenu.obj_el.title);
                contextMenu.active_dropdown.find('.mfn-context-header').html( contextMenu.obj_el.title );
            }

            if( !copypaste.ls_obj ) {
                contextMenu.active_dropdown.find('.mfn-context-paste').addClass('mfn-context-inactive');
                contextMenu.active_dropdown.find('.mfn-context-paste-style').addClass('mfn-context-inactive');
            }else{
                contextMenu.active_dropdown.find('.mfn-context-paste').removeClass('mfn-context-inactive');
                contextMenu.active_dropdown.find('.mfn-context-paste-style').removeClass('mfn-context-inactive');
            }

            if( !copypaste.ls_obj || !contextMenu.obj_el || contextMenu.obj_el.jsclass != copypaste.ls_obj['obj'][0]['jsclass'] ){
                contextMenu.active_dropdown.find('.mfn-context-paste-style').addClass('mfn-context-inactive');
            }

            if( contextMenu.obj_el.jsclass == 'section' ){
                contextMenu.active_dropdown.find('.mfn-contextmenu-convert-global-section').show();
            }else{
                contextMenu.active_dropdown.find('.mfn-contextmenu-convert-global-section').hide();
            }

            if( contextMenu.obj_el.jsclass == 'wrap' ){
                contextMenu.active_dropdown.find('.mfn-contextmenu-convert-global-wrap').show();
            }else{
                contextMenu.active_dropdown.find('.mfn-contextmenu-convert-global-wrap').hide();
            }
        ...
        click: function() {
            ...
            contextMenu.active_dropdown.find('a').on('click', function(e) {
                e.preventDefault();

                let action = $(this).data('action');

                sidebar_form.is_class = false;

                if(action == 'delete'){
                    $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').find('.mfn-header').first().find('.mfn-element-delete').trigger('click');
                }else if(action == 'edit'){
                    $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').find('.mfn-header').first().find('.mfn-element-edit').trigger('click');
                }else if(action == 'decrease'){
                    $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').find('.mfn-header').first().find('.mfn-size-decrease').trigger('click');
                }else if(action == 'clone'){
                    $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').find('.mfn-header').first().find('.mfn-module-clone').trigger('click');
                }else if(action == 'increase'){
                    $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').find('.mfn-header').first().find('.mfn-size-increase').trigger('click');
                }else if(action == 'copy'){
                    copypaste.copy(contextMenu.item);
                }else if(action == 'clone'){
                    $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').find('.mfn-header').first().find('.mfn-module-clone').trigger('click');
                    ...
                    var this_el     = mfn.pagedata.filter( (item) => item.uid == contextMenu.item )[0];
                    ...
                    sidebar_form.open( $builder.find('.vb-item[data-uid="'+contextMenu.item+'"]'), false );
                    loopAllStyleFields(contextMenu.item);
                    historyStorage.add();
                }else if(action == 'save-preset') {
                    presets.modal( contextMenu.item );
                }else if(action == 'save-class') {
                    mfn_classes.uid = contextMenu.item;
                    $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').find('.mfn-header').first().find('.mfn-element-edit').trigger('click');
                    setTimeout(mfn_classes.modal(), 10);
                }else if(action == 'bring_before') {

                    if( !$content.find('.vb-item[data-uid="'+contextMenu.item+'"]').length ) return;
                    if( !$content.find('.vb-item[data-uid="'+contextMenu.item+'"]').prev('.vb-item').length ) return;

                    let cloned = $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').clone();

                    $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').addClass('mfn_to_remove');
                    $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').prev('.vb-item').before(cloned);
                    $content.find('.vb-item.mfn_to_remove[data-uid="'+contextMenu.item+'"]').remove();

                    be_navigator.show(contextMenu.item);

                }else if(action == 'bring_after'){
                    
                    if( !$content.find('.vb-item[data-uid="'+contextMenu.item+'"]').length ) return;
                    if( !$content.find('.vb-item[data-uid="'+contextMenu.item+'"]').next('.vb-item').length ) return;
                    
                    let cloned = $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').clone();
                    
                    $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').addClass('mfn_to_remove');
                    $content.find('.vb-item[data-uid="'+contextMenu.item+'"]').next('.vb-item').after(cloned);
                    $content.find('.vb-item.mfn_to_remove[data-uid="'+contextMenu.item+'"]').remove();

                    be_navigator.show(contextMenu.item);
                    
                }else if(action == 'convert-global-section') {
                    GlobalSections.convertToGlobal( contextMenu.item );
                }else if(action == 'convert-global-wrap') {
                    GlobalWraps.selectedUid = contextMenu.item;
                    contextMenu.item = $builder.find('.vb-item[data-uid="'+contextMenu.item+'"]').closest('.mcb-section').attr('data-uid')
                    GlobalWraps.convertToGlobal( contextMenu.item );
                    ...
                    for(key in listStyles) {
                        if( key.startsWith('css_') || key.startsWith('style:') || key.includes('height_switcher') || key.includes('width_switcher') ) {
                            delete listStyles[key];
                        }
                    }

                    $content.find('style.mfn-local-style').remove();
                    sidebar_form.open( $edited_div, false );
                    loopAllStyleFields();
                ...
                if( !$(this).closest('.mfn-contextmenu-tools').length ) contextMenu.active_dropdown.remove();
            });
        ...
        hideContext: function(e) {
            ...
            if( !contextMenu.active_dropdown.is(e.target) && contextMenu.active_dropdown.has(e.target).length === 0 ) {
                contextMenu.active_dropdown.remove();

                $content.find('body').unbind('click', contextMenu.hideContext);
                $('body').unbind('click', contextMenu.hideContext);
    ...
    $builder.on('mouseup', function(e) {
        ...
        if( $(e.target).closest('ul.mfn-tabber-nav').length ) {
            $edited_div = $builder.find( $(e.target).attr('data-tab') );
        }

        if( !$edited_div.hasClass('mfn-current-editing') || sidebar_form.is_class ) {
            sidebar_form.is_class = false;
            sidebar_form.open($edited_div, false);
        }
    ...
    $builder.on('click', '.mfn-header .mfn-size-change', function(e) {
        ...
        let chenged_item = mfn.pagedata.filter( (item) => item.uid == uid )[0];
        let type = edited_item.type;

        if( chenged_item.jsclass == 'wrap' ) item_type = 'mcb-wrap-';

        let currClass = sizes.filter(size => size.key === chenged_item.size)[0];

        if( !chenged_item.mobile_size ) chenged_item.mobile_size = '1/1';
        if( !chenged_item.tablet_size ) chenged_item.tablet_size = chenged_item.size;

        if(screen == 'tablet'){
            currClass = sizes.filter(size => size.key === chenged_item.tablet_size)[0];
        }else if(screen == 'laptop'){
            currClass = sizes.filter(size => size.key === chenged_item.laptop_size)[0];
        }else if(screen == 'mobile'){
            currClass = sizes.filter(size => size.key === chenged_item.mobile_size)[0];
        }
        ...
        if( !items_size[type] || ( items_size[type].length && items_size[type].includes(newClass.key) ) ) {
            ...
            if(screen == 'desktop') {
                ...
                chenged_item.size = newClass.key;

                if( typeof chenged_item.tablet_resized === 'undefined' || chenged_item.tablet_resized == '0' ) {
                    chenged_item.tablet_size = newClass.key;
                    $content.find('.'+item_type+uid).removeClass(currClass.tablet).addClass(newClass.tablet).attr('data-tablet-size', newClass.key);
                }

                if( typeof chenged_item.laptop_resized === 'undefined' || chenged_item.laptop_resized == '0' ) {
                    chenged_item.laptop_size = newClass.key;
        ...
        if( $content.find('.'+item_type+uid+' .woocommerce-product-gallery').length ) iframe.window.jQuery('body').trigger('resize');
    ...
    $builder.on('click', '.mfn-header .mfn-element-delete', function(e) {
        ...
        $('.btn-modal-confirm').on('click', function(e) {
            ...
            //mfn.pagedata = mfn.pagedata.filter( (item) => item.uid != uid );

            if( $navigator.find('.navigator-tree li.nav-'+uid).length ) {
                $navigator.find('.navigator-tree li.nav-'+uid).remove();
            }

            if( $dom_el.closest('.mfn-looped-items').length ) {
                force_re_render = $dom_el.closest('.mcb-section').attr('data-uid');
            }

            if( $dom_el.hasClass('mcb-wrap') && !$dom_el.hasClass('mfn-nested-wrap') && $dom_el.closest('.mfn-section-tabber').length ) {
                force_re_render = $dom_el.closest('.mcb-section').attr('data-uid');
            }

            if( $dom_el.next('.mfn-read-more-line').length ) $dom_el.next('.mfn-read-more-line').remove();
            
            $builder.find('[data-uid="'+uid+'"]').remove();
            ...
            if( force_re_render ) re_render( force_re_render );
    ...
    setTimeout(mfnBannerBox, 500);
}

$editpanel.on('click', '.btn-save-option', function(e) {
    e.preventDefault();
    if (mfn.view == 'demo') return;
...
function editContentButton() {
    if( $builder.find( '.mfn-builder-content:not(.mfn-builder-active)' ).length && mfn.view != 'demo' ) {
        $builder.find( '.mfn-builder-content:not(.mfn-builder-active)' ).each(function(i, el) {
            let this_item = $(el).closest('.mcb-column.vb-item');
            if( this_item.length && !this_item.find('.btn-edit-content').length ) this_item.append('<a href="'+mfn.adminurl+'post.php?post='+$(el).attr("data-id")+'&action='+(mfn.be_slug == 'be' || mfn.be_slug == 'mfn' ? 'mfn' : mfn.be_slug)+'-live-builder" target="_blank" data-tooltip="Edit Content" class="btn-edit-content" data-position="right">Edit content</a>')
        });
    }
}
...
$editpanel.on('click', '.btn-save-changes', function(e){
    ...
    if( $('.modal-display-conditions').length && !$('.modal-display-conditions').hasClass('show') && ( builder_type == 'popup' || builder_type == 'header' || builder_type == 'footer' || builder_type == 'blog' || builder_type == 'portfolio' || builder_type == 'shop-archive' || builder_type.includes('single-') || builder_type.includes('archive-') ) ) {
    ...
    let pf_obj = JSON.stringify(prepareForm.object);

    if( !mfn.current_user_roles.includes("administrator") ) {
        formData = formData.replaceAll('onclick', 'restricted');
        pf_obj = pf_obj.replaceAll('onclick', 'restricted');
    }
...
var prepareForm = {
    ...
    get: function(uid = false) {
        ...
        prepareForm.object = [];
        //prepareForm.object.push( mfn.pagedata.filter( (item) => item.uid === 'pageoptions' )[0] ); // pageoptions is required
        ...
        wrapper.each(function(s) {
            ...
            var this_vb = JSON.parse( JSON.stringify(mfn.pagedata.filter( (item) => item.uid === vb_uid )[0]) );

            if( prepareForm.is_global_wrap && _.has(this_vb, 'attr') ) {
                $.each(this_vb.attr, function(i, v) {
                    if( i.includes('style:') || i.includes('css_') ) delete(this_vb['attr'][i]);
                });
            }
            ...
            if( this_vb && typeof this_vb.uid !== 'undefined' ) {
                ...
                if( !$(this).hasClass('mfn-global-section') && $(this).find('.wrap.vb-item').length ) {
                    this_vb.wraps = [];
                    prepareForm.wraps( this_vb.uid );
                }

                prepareForm.is_global_wrap = false;
    ...
    wraps: function( wrapper_uid, nested = false ) {

        var wraps = $builder.find( '.mcb-section-'+wrapper_uid+' .section_wrapper .wrap.vb-item:not(.mfn-nested-wrap)' );
        ...
        wraps.each(function(w) {
            ...
            var this_wrap = mfn.pagedata.filter( (item) => item.uid === wrap_uid )[0];

            if( this_wrap ) {

                if( !GlobalWraps.selectedUid.length || GlobalWraps.selectedUid == this_wrap.uid ) { // convert to global - selects wrap to convert in section with multiple wraps
                ...
                if( _.has(this_wrap, 'attr') ) {
                    if( prepareForm.save ) {
                        if( _.has(this_wrap.attr, 'vb_postid') ) delete this_wrap.attr['vb_postid'];
                        if( _.has(this_wrap.attr, 'rwd') ) delete this_wrap.attr['rwd'];
                        if( _.has(this_wrap.attr, 'vb') ) delete this_wrap.attr['vb'];
                    }else{
                ...
                if( !prepareForm.formData[prepareForm.si]['wraps'].filter( (item) => item.uid == wrap_uid ).length ) {
                    ...
                    if( !$(this).hasClass('mfn-global-wrap') ) prepareForm.items( this_wrap.uid );
    ...
    nested_items: function( wrapper_uid ) {

        $builder.find( '.mcb-wrap-'+wrapper_uid+' .vb-item' ).each(function(w) {

            prepareForm.nwi = w;
            var item_uid = $(this).attr('data-uid');
            var this_item = mfn.pagedata.filter( (item) => item.uid === item_uid )[0];
            ...
            if( this_item && !prepareForm.formData[prepareForm.si]['wraps'][prepareForm.wi]['items'][prepareForm.ii]['items'].filter( (item) => item.uid === item_uid ).length ){
                ...
                if( this_item && typeof this_item.attr !== 'undefined' ) {
                    ...
                    if( prepareForm.save ){
                        if( _.has(this_item.attr, 'vb_postid') ) delete this_item.attr['vb_postid'];
                        if( _.has(this_item.attr, 'rwd') ) delete this_item.attr['rwd'];
                        if( _.has(this_item.attr, 'vb') ) delete this_item.attr['vb'];
                    }else{
    ...
    items: function(wrapper_uid) {
        var used_fonts = [];

        let wraps = $builder.find('.mcb-wrap-'+wrapper_uid+' .mcb-wrap-inner-'+wrapper_uid+' > .vb-item');

        if( !wraps.length ) {
            wraps = $builder.find('.mcb-wrap-'+wrapper_uid+' .mcb-wrap-inner-'+wrapper_uid+ ' > div > .vb-item');
        }

        if( !wraps.length ) {
            wraps = $builder.find('.mcb-wrap-'+wrapper_uid+' .mcb-wrap-inner-'+wrapper_uid+ ' > div > div > .vb-item');
        }

        // console.log({ 'html': $builder.find('.mcb-wrap-'+wrapper_uid+' .mcb-wrap-inner-'+wrapper_uid).html(), 'vbitems': wraps });

        wraps.each(function(i) {

            //console.log({'items': $(this).attr('data-uid')});
            ...
            if( !prepareForm.formData[prepareForm.si]['wraps'][prepareForm.wi]['items'].filter( (item) => item.uid === item_uid ).length ){
                ...
                if( this_item ) {
                    ...
                    if( this_item.jsclass != 'wrap' ) {
                        ...
                        if( this_item.jsclass == 'video' && _.has( this_item, 'attr' ) && ( _.has(this_item['attr'], 'css_video_width') || _.has(this_item['attr'], 'css_video_height') ) ){
                            if( _.has(this_item['attr'], 'css_video_width') && _.has(this_item['attr']['css_video_width'], 'val') ){
                                if( _.has(this_item['attr']['css_video_width']['val'], 'desktop') && !isNaN(this_item['attr']['css_video_width']['val']['desktop']) ) this_item['attr']['css_video_width']['val']['desktop'] += 'px';
                                if( _.has(this_item['attr']['css_video_width']['val'], 'laptop') && !isNaN(this_item['attr']['css_video_width']['val']['laptop']) ) this_item['attr']['css_video_width']['val']['laptop'] += 'px';
                                if( _.has(this_item['attr']['css_video_width']['val'], 'tablet') && !isNaN(this_item['attr']['css_video_width']['val']['tablet']) ) this_item['attr']['css_video_width']['val']['tablet'] += 'px';
                                if( _.has(this_item['attr']['css_video_width']['val'], 'mobile') && !isNaN(this_item['attr']['css_video_width']['val']['mobile']) ) this_item['attr']['css_video_width']['val']['mobile'] += 'px';
                            }

                            if( _.has(this_item['attr'], 'css_video_height') && _.has(this_item['attr']['css_video_height'], 'val') ){
                                if( _.has(this_item['attr']['css_video_height']['val'], 'desktop') && !isNaN(this_item['attr']['css_video_height']['val']['desktop']) ) this_item['attr']['css_video_height']['val']['desktop'] += 'px';
                                if( _.has(this_item['attr']['css_video_height']['val'], 'laptop') && !isNaN(this_item['attr']['css_video_height']['val']['laptop']) ) this_item['attr']['css_video_height']['val']['laptop'] += 'px';
                                if( _.has(this_item['attr']['css_video_height']['val'], 'tablet') && !isNaN(this_item['attr']['css_video_height']['val']['tablet']) ) this_item['attr']['css_video_height']['val']['tablet'] += 'px';
                                if( _.has(this_item['attr']['css_video_height']['val'], 'mobile') && !isNaN(this_item['attr']['css_video_height']['val']['mobile']) ) this_item['attr']['css_video_height']['val']['mobile'] += 'px';
                            }
                        }
                        ...
                        this_item['items'] = [];
                        prepareForm.formData[prepareForm.si]['wraps'][prepareForm.wi]['items'][prepareForm.ii] = this_item;
                        prepareForm.nested_items(this_item.uid);
                    }
    ...
    new: {
        wrap: function(id, is_divider, size) {
            ...
            if( $builder.find('.mcb-section-'+id).length ) {
                ...
                if( $builder.find('.mcb-wrap-'+id+' .mcb-wrap-inner-'+id+' .mfn-queryloop-item-wrapper').length ) {
                    $builder.find('.mcb-wrap-'+id+' .mcb-wrap-inner-'+id+' .mfn-queryloop-item-wrapper').first().append( new_element.html );
                }else{
                    $builder.find('.mcb-wrap-'+id+' .mcb-wrap-inner-'+id).append(new_element.html);
                }

                mfn.pagedata.filter( (w) => w.uid == new_element.uid )[0]['item_is_wrap'] = 1;
            ...
            if( $builder.find('.mcb-section-'+id).hasClass('mfn-section-tabber') ){
                sidebar_form.force_open = new_element.uid;
                re_render(id);
            }
        ...
        item: function() {
            ...
            if( be_layout.drag_new.dropped_item != 'placeholder' ) $builder.find('.vb-item[data-uid="'+new_uid+'"]').find('.mfn-header .mfn-element-edit').trigger('click');
            ...
            $builder.find('.mfn-wrapper-for-wraps').sortable({
                ...
                update: function(e, ui) {
                        ...
                        setTimeout(function() {
                            ...
                            setTimeout(function() {
                                if(ui.item.hasClass('mfn-nested-wrap')){
                                    // reset sticky & loop
                                    $('.mfn-form .mfn-form-row.wrap.sticky ul li:first-child a').trigger('click');
                                    $('.mfn-form .mfn-form-row.mfn-loop-switcher.wrap.type ul li:first-child a').trigger('click');
                                }
                            }, 20);
                    ...
                    setTimeout(function() {
                        ...
                        if( ui.item.closest('.vb-item.mfn-section-tabber').length ) {
                            re_render( ui.item.closest('.vb-item.mfn-section-tabber').attr('data-uid') );
                        }
...
let bebuilder_data_updater = {
  ...
  wrapper: false,
  init: function() {
    $(document).on('click', '.mfn-be-css-db-update-action-button', function(e) {
      e.preventDefault();
      bebuilder_data_updater.button = $(this);
      if( bebuilder_data_updater.button.hasClass('loading') ) return;

      if( bebuilder_data_updater.button.closest('.mfn-bebuilder-iframe-error').length ){
        bebuilder_data_updater.wrapper = bebuilder_data_updater.button.closest('.mfn-bebuilder-iframe-error');
        bebuilder_data_updater.wrapper.html('The database update has started. Please wait...');
      }
  ...
  process: function() {
    ...
    $.ajax({
      ...
      if( bebuilder_data_updater.wrapper ){
        bebuilder_data_updater.wrapper.html('BeBuilder: Database updated successfully! Reloading...');
...
var mfn_multifield_field = {

    id: false,

    init: function() {

        $(document).on('click', '.multifield_add_new', function(e) {
            e.preventDefault();
            mfn_multifield_field.id = $(this).closest('.mfn-form-row').attr('data-id');
            mfn_multifield_field.add(e);
        });

        $(document).on('click', '.mfn-multifield-point .mfn-tab-toggle', function(e) {
            e.preventDefault();
            var hash = $(this).closest('.mfn-multifield-point').attr('id');
            if( $(this).closest('.mfn-multifield-point').hasClass('open') ){
                $(this).closest('.mfn-multifield-point').removeClass('open');
                if( $content.find('.hotspot-point-'+hash+' .hotspot-content').length ) $content.find('.hotspot-point-'+hash).removeClass('is-active-force');
                if( $('style.mfn-fields-switcher-multifields-'+hash).length ) $('style.mfn-fields-switcher-multifields-'+hash).remove();
            }else{
                $(this).closest('.mfn-multifield-point').addClass('open');
                if( $content.find('.hotspot-point-'+hash+' .hotspot-content').length ) $content.find('.hotspot-point-'+hash).addClass('is-active-force');
                $('body').append('<style class="mfn-builder-fields-css mfn-fields-switcher mfn-fields-switcher-multifields mfn-fields-switcher-multifields-'+hash+'">.mfn-multifield-form-'+hash+' .mfn-multifield-form-wrapper-'+hash+'{ display: block; } .mfn-multifield-form-'+hash+' .mfn-multifield-point-tab-header .mfn-tab-toggle{ transform: rotate(180deg); }</style>');
            }
            
        });

        $(document).on('click', '.mfn-multifield-point .mfn-tab-delete', function(e) {
            e.preventDefault();
            mfn_multifield_field.id = $(this).closest('.mfn-form-row').attr('data-id');
            var hash = $(this).closest('.mfn-multifield-point').attr('id');
            mfn_multifield_field.delete(hash);
            $(this).closest('.mfn-multifield-point').remove();
        });
        ...
        if( iframe && _.has(iframe.window, 'jQuery') ) {
            iframe.window.jQuery('body').on('click', '.mfn-image-hotspot .hotspot-point', function(e) {
                ...
                if( $('.mfn-form-row.hotspots .mfn-multifield-point').hasClass('open') ) {
                    $('.mfn-form-row.hotspots .mfn-multifield-point.open').each(function() {
                        $(this).find('.mfn-tab-toggle').trigger('click');
                    });
                }
                $('#'+id+'.mfn-multifield-point .mfn-tab-toggle').trigger('click');
            });
        }

        $(document).on('mouseenter', '.filters_fields .mfn-multifield-point', function(e) {
            mfn_multifield_field.id = $(this).closest('.mfn-form-row').attr('data-id');
            mfn_multifield_field.sortable();
        });

    },

    refresh: function(id, field) {
        // mfn_multifield_field.refresh
        // refreshHotspot

        var point_hash = field.closest('.mfn-multifield-point').attr('id');
    ...
    add: function(e) {
        ...
        let i = $('.mfn-multifield-point').length;
        let hash = getUid();

        if( $('.mfn-multifield-point.open').length ) $('.mfn-multifield-point.open').removeClass('open');
        
        if( $(e.target).closest('.mfn-form-row').hasClass('hotspot') ){
            $('.hotspot_add_new').before(mfn_field_hotspot_render({hash: hash}, i));   
        }else{
            $('.multifield_add_new').before(mfn_field_filters_render({hash: hash}, i));   
        }
        

        $('#'+hash+'.mfn-multifield-point .mfn-tab-toggle').trigger('click');

        if( typeof edited_item['attr'] === 'undefined' ) edited_item['attr'] = {};
        if( typeof edited_item['attr'][mfn_multifield_field.id] === 'undefined' ) edited_item['attr'][mfn_multifield_field.id] = [];

        var fields = {hash: hash};

        edited_item['attr'][mfn_multifield_field.id].push(fields);

        mfn_multifield_field.render();
        
        setTimeout(function() {
            mfnoptsinputs.start();
            if( $('.mfn-form .mfn-element-fields-wrapper .mfn-form-row .sliderbar:not(.mfn-initialized)').length ) {
                $('.mfn-form .mfn-element-fields-wrapper .mfn-form-row .sliderbar:not(.mfn-initialized)').each(function() {
    ...
    render: function() {
        if( $builder.find('.mfn-current-editing').closest('.mfn-looped-items').length ){
            re_render( $builder.find('.mfn-current-editing').closest('.mcb-section').attr('data-uid') );
        }else{
    ...
    delete: function(hash) {
        ...
        if( edited_item['attr'][mfn_multifield_field.id].filter( item => item.hash == hash).length ){
            edited_item['attr'][mfn_multifield_field.id] = edited_item['attr'][mfn_multifield_field.id].filter( item => item.hash != hash);
        }
    ...
    sortable: function() {
        $('.filters_fields .mfn_input_wrapper').sortable({
            axis: 'y',
            cursor: 'ns-resize',
            handle: '.mfn-sort-handle',
            opacity: 0.9,
            update: function(e, ui) {

                let updated_order = [];

                $('.filters_fields .mfn_input_wrapper .mfn-multifield-point').each(function(i, el) {
                    let id = $(el).attr('id');
                    updated_order.push(edited_item['attr'][mfn_multifield_field.id].filter( (f) => f.hash == id )[0]);
                    //$(el).find('h6').text('Filter '+(i+1));
                });

                if( updated_order.length == edited_item['attr'][mfn_multifield_field.id].length ) {
                    edited_item['attr'][mfn_multifield_field.id] = updated_order;
...
var mfn_autocomplete = {
    ...
    init: function() {
        ...
        $(document).on('focus paste keyup', '.mfn-autocomplete .mfn-autocomplete-input', function(e) {
            mfn_autocomplete.input = $(this);
            if (e.key !== "ArrowDown" && e.key !== "ArrowUp" && e.key !== "Enter") {
                mfn_autocomplete.show_list();
            }
        });

        document.addEventListener("keydown", (e) => {
            if( !mfn_autocomplete.input || !$('.mfn-autocomplete-ul li').length ) return
            if (e.key === "ArrowDown") {
                e.preventDefault();
                if( $('.mfn-autocomplete-ul li.active').length ) {
                    let next = $('.mfn-autocomplete-ul li.active').next('li:not(.disabled)').length ? $('.mfn-autocomplete-ul li.active').next('li:not(.disabled)') : $('.mfn-autocomplete-ul li').first();
                    $('.mfn-autocomplete-ul li.active').removeClass('active');
                    next.addClass('active');
                }else{
                    $('.mfn-autocomplete-ul li').first().addClass('active')
                }
            } else if (e.key === "ArrowUp") {
                e.preventDefault();
                if( $('.mfn-autocomplete-ul li.active').length ) {
                    let next = $('.mfn-autocomplete-ul li.active').prev('li:not(.disabled)').length ? $('.mfn-autocomplete-ul li.active').prev('li:not(.disabled)') : $('.mfn-autocomplete-ul li').last();
                    $('.mfn-autocomplete-ul li.active').removeClass('active');
                    next.addClass('active');
                }else{
                    $('.mfn-autocomplete-ul li').last().addClass('active')
                }
            } else if (e.key === "Enter") {
                if( $('.mfn-autocomplete-ul li.active').length ) $('.mfn-autocomplete-ul li.active').trigger('click');
            }
        });

        $(document).on('click', '.mfn-autocomplete .mfn-sidebar-classes-show-list', function(e) {
            e.preventDefault();
            mfn_autocomplete.input = $(this).closest('.mfn-autocomplete').find('input.mfn-autocomplete-input');
            mfn_autocomplete.show_list();
        });

        $(document).on('click', '.mfn-autocomplete .mfn-autocomplete-ul li:not(.disabled)', function() {
            $(this).addClass('disabled');
            mfn_autocomplete.to_add = $(this);
            mfn_autocomplete.add();
        });

    },
    add: function() {
        mfn_autocomplete.input.before(`<span data-name="${mfn_autocomplete.to_add.attr('data-id')}" class="sidebar-panel-class mfn-autocomplete-item">${mfn_autocomplete.to_add.text()}<span class="sidebar-panel-class-edit mfn-autocomplete-item-edit">&#9998;</span> <span class="sidebar-panel-class-delete mfn-autocomplete-item-delete">&#10005;</span></span>`);
        
        if( !_.has(edited_item, 'be_classes') ) edited_item['be_classes'] = [];
        if( _.indexOf(edited_item.be_classes, mfn_autocomplete.to_add.attr('data-id')) >= 0 ) return;

        let force_render = false;
        let force_refresh_local_styles = false;

        let added_obj = mfn.classes.filter( cl => cl.uid == mfn_autocomplete.to_add.attr('data-id') )[0];

        let original = edited_item.jsclass == 'section' || edited_item.jsclass == 'wrap' ? JSON.parse( JSON.stringify(mfn.elements[edited_item.jsclass]) ) : JSON.parse( JSON.stringify(renderMfnFields.items[edited_item.jsclass]) );

        if( added_obj ) {
            _.map( added_obj.attr, function(opt, o) {

                if( /*o.includes('css_') && */_.has(original, 'attr') && _.has(edited_item, 'attr') && _.has(original['attr'], o) && _.has(edited_item['attr'], o) && ( edited_item['attr'][o]['val'] == original['attr'][o]['val'] || ( _.has(original['attr'][o]['val'], 'desktop') && original['attr'][o]['val']['desktop'] == edited_item['attr'][o]['val']['desktop'] ) ) ) {
                    // removes stds from edited_item on class add
                    force_refresh_local_styles = true;
                    delete(edited_item['attr'][o]); 
                }
                
                if( !force_render && edited_item.jsclass != 'wrap' && !o.includes('css_') && (_.indexOf(mfn_classes.exceptions, o) >= 0 || _.indexOf(mfn_classes.exceptions, added_obj.jsclass+'_'+o) >= 0 ) && opt != 'default' ) {
                    force_render = true;
                }
            });
        }

        edited_item.be_classes.push( mfn_autocomplete.to_add.attr('data-id') );
        //$builder.find(`.mcb-${edited_item.jsclass}-${edited_item.uid}`).addClass( mfn_autocomplete.to_add.attr('data-id') );
        $builder.find('.'+$('.mfn-element-fields-wrapper').attr('data-element')).addClass( mfn_autocomplete.to_add.attr('data-id') );

        $('.mfn-ui').addClass('mfn-class-editor-full');

        setTimeout(function() {
            sidebar_form.render();
            mfn_autocomplete.exit();
            loopAllStyleFields(edited_item.uid);
            if( force_render ) re_render();
    ...
    show_list: function() {
        ...
        if( mfn_autocomplete.singular && _.has(edited_item, 'be_classes') && edited_item.be_classes.length && mfn.classes.filter( (cl) => cl.uid == edited_item.be_classes[0]).length ) return;

        mfn_autocomplete.exit();
        let list = mfn_autocomplete.input.attr('data-list');
        mfn_autocomplete.data = JSON.parse( JSON.stringify( mfn.classes.filter( (n) => n.uid.includes(edited_item.jsclass) ) ) );
        if( mfn_autocomplete.input.val().length ){
            mfn_autocomplete.data = mfn_autocomplete.data.filter( (n) => n.title.toLowerCase().includes(mfn_autocomplete.input.val().toLowerCase()) );
        }
        if( mfn_autocomplete.data.length ){
            mfn_autocomplete.input.closest('.mfn-autocomplete')
                .append(`<ul class="mfn-autocomplete-ul">
                    ${ _.map(mfn_autocomplete.data, function(l) {
                        return `<li ${ _.has(edited_item, 'be_classes') && edited_item.be_classes.length > 0 && _.indexOf(edited_item.be_classes, l.uid) >= 0 ? 'class="disabled"' : '' } data-id="${ l.uid }">${ l.title }</li>`;
                    }).join('') } </ul>`);
        }else{
            mfn_autocomplete.input.closest('.mfn-autocomplete')
                .append(`<ul class="mfn-autocomplete-ul"><li class="disabled mfn-autocomplete-empty">Empty list</li></ul>`);
        }

        $(document).bind('click', mfn_autocomplete.blur);
    },
    blur: function(e) {
        var div = $('.mfn-autocomplete-list');
        if( !div.is(e.target) && div.has(e.target).length === 0 ) mfn_autocomplete.exit();
    },
    exit: function() {
        $('.mfn-autocomplete .mfn-autocomplete-ul').remove();
        $(document).unbind('click', mfn_autocomplete.blur);
    }
}

mfn_autocomplete.init();


var mfn_classes = {
    uid: false,
    popup_title: "Save styles of a given element as a global style",
    styles: '',
    // has_changed: false,
    original: [],
    edited_item_uid: false,
    confirmed: [],
    refresh: false,
    refresh_all_items_with_class: false,
    backup: false,
    has_classes: false,
    exceptions: ['width_switcher', 'height_switcher', 'visibility', 'banner_box_css_desc_justify', 'banner_box_css_desc_align', 'banner_box_style', 'banner_box_overlay', 'banner_box_hover_effect', 'banner_box_image_height', 'banner_box_hidden_elements_mobile', 'button_icon', 'button_icon_position', 'button_size', 'button_full_width', 'button_style', 'divider_2_type', 'divider_2_align', 'divider_2_addon', 'divider_2_label', 'divider_2_image', 'divider_2_icon', 'icon_2_hover', 'icon_box_2_icon_position', 'icon_box_2_icon_align', 'icon_box_2_hover', 'image_mask_shape_type', 'image_mask_shape_size', 'image_mask_shape_position', 'image_image_height', 'image_image_height_style', 'image_greyscale', 'image_gallery_style', 'image_gallery_layout', 'image_gallery_columns', 'image_gallery_greyscale', 'image_gallery_image_height', 'list_2_type', 'list_2_starting', 'list_2_align', 'list_2_valign', 'list_2_divider', 'list_2_icon', 'list_2_image', 'list_2_background_switcher', 'toggle_type', 'toggle_starting', 'toggle_divider', 'toggle_open_first', 'toggle_open_all', 'toggle_open_more', 'toggle_icon', 'toggle_active_icon', 'toggle_icon_animation'],
    init: function() {

        $(document).on('click', '.mfn-modal .btn-save-new-class', function(e) {
            e.preventDefault();
        ...
        $(document).on('click', '.sidebar-panel-classes-list  .sidebar-panel-class-delete', function(e) {
            let classname = $(this).closest('.sidebar-panel-class').attr('data-name');
            mfn_classes.remove(classname);
        });

        $(document).on('click', '.sidebar-panel-classes-list  .sidebar-panel-class-edit', function(e) {
            let classname = $(this).closest('.sidebar-panel-class').attr('data-name');
            mfn_classes.edited_item_uid = edited_item.uid;
            mfn_classes.edit(classname);
        });

        $(document).on('click', '.mfn-class-editor-update', function(e) {
            e.preventDefault();
            if( $(this).hasClass('loading') ) return;

            $(this).addClass('loading');

            mfn_classes.refresh_all_items_with_class = true;

            mfn_classes.ajax( edited_item );

            if( $builder.find('.vb-item[data-uid="'+mfn_classes.edited_item_uid+'"]').length ) {
                loopAllStyleFields(edited_item.uid);
                let to_click = $builder.find('.vb-item[data-uid="'+mfn_classes.edited_item_uid+'"]').find('.mfn-header').first().find('.mfn-element-edit');
                $builder.find('.mfn-current-editing').removeClass('mfn-current-editing');
                to_click.trigger('click');
            }else{
                backToWidgets();
            }

            mfn_classes.edited_item_uid = false;
        });

        $(document).on('click', '.panel-export-import-classes .mfn-export-classes-button', function(e) {
            e.preventDefault();
            let button = $(this);
            $('#export-classes-data-textarea').select();
            document.execCommand("copy");
            button.find('span').text('Exported').addClass('mfn-icon-check-blue');
            setTimeout(function() { button.find('span').html('Export'); }, 5000);
        });

        $(document).on('click', '.panel-export-import-classes .mfn-import-classes-button', function(e) {
            e.preventDefault();
            let button = $(this);

            if($('#import-classes-data-textarea').hasClass('error')) $('#import-classes-data-textarea').removeClass('error');

            if( !$('#import-classes-data-textarea').val().length ) {
                $('#import-classes-data-textarea').addClass('error');
                return;
            }

            mfn_classes.import.data = JSON.parse($('#import-classes-data-textarea').val());
            mfn_classes.import.init();
        });

        $(document).on('keyup', '.modal-classes-listing .mfn-search', function() {
            let val = $(this).val().toLowerCase();
        
            if( !val.length ) {
                $('.mfn-global-styles-listing li').removeAttr('style');
                return;
            }
            $('.mfn-global-styles-listing li').hide();
            $('.mfn-global-styles-listing li[data-name*="'+val+'"]').show();
            if( $('.mfn-global-styles-listing li li[data-name*="'+val+'"]').parent('ul').parent('li').length ) $('.mfn-global-styles-listing li[data-name*="'+val+'"]').parent('ul').parent('li').show();
            if( $('.mfn-global-styles-listing li[data-name*="'+val+'"]').find('li').length ) $('.mfn-global-styles-listing li[data-name*="'+val+'"]').find('li').show();

        });

        $(document).on('click', '.mfn-class-styles-list li', function(e) {
            e.preventDefault();
            let id = $(this).find('.mfn-class-styles-check').attr('data-id');
            if( $(this).hasClass('active') ){
                $(this).removeClass('active');
                let index = _.indexOf(mfn_classes.confirmed, id);
                if( index >= 0 ) mfn_classes.confirmed.splice(index, 1);
            }else{
                $(this).addClass('active');
                mfn_classes.confirmed.push(id);
            }
        });

        $(document).on('click', '.sidebar-panel-class-create', function(e) {
            e.preventDefault();
            mfn_classes.modal();
        });

        $(document).on('click', '.sidebar-panel-class-list', function(e) {
            e.preventDefault();
            mfn_classes.list_modal.show();
        });

        $(document).on('click', '.mfn-class-editor-dismiss', function(e) {
            e.preventDefault();
            edited_item.attr = mfn_classes.backup['attr'];
            setTimeout(function() {
                loopAllStyleFields(edited_item.uid);

                if( $builder.find('.mfn-current-editing').length ){
                    let to_click = $builder.find('.mfn-current-editing').find('.mfn-header').first().find('.mfn-element-edit');
                    $builder.find('.mfn-current-editing').removeClass('mfn-current-editing');
                    sidebar_form.is_class = false;
                    to_click.trigger('click');
                    // re_render();
    ...
    import: {
        ...
        duplicates: [],
        init: function() {
            mfn_classes.import.duplicates = [];
            let looped = JSON.parse( JSON.stringify(mfn_classes.import.data) );

            _.map(looped, (el, i) => {
                if( mfn.classes && mfn.classes.filter( cl => cl.uid == el.uid ).length ) {
                    el['uid'] = `be_${el.jsclass}_${getUid()}`;
                    el['title'] = el.title+'_clone';
                    mfn_classes.import.duplicates.push(el);
                    //mfn_classes.import.data = mfn_classes.import.data.filter(item => item.uid != el.uid);
                }
            }).join('');

            if( mfn_classes.import.duplicates.length ){
                mfn_classes.import.modal();
            }else{
                mfn_classes.import.import_data();
            }
            
        },
        modal: function() {

            $('.mfn-ui').append(`<div class="mfn-modal modal-confirm mfn-modal-600 show"> <div class="mfn-modalbox mfn-form mfn-shadow-1"> <div class="modalbox-header"> <div class="options-group"> <div class="modalbox-title-group"> <span class="modalbox-icon mfn-icon-global-styles-list"></span> <div class="modalbox-desc"> <h4 class="modalbox-title">Global styles import</h4> </div></div></div><div class="options-group"> <a class="mfn-option-btn mfn-option-blank btn-large btn-modal-close" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a> </div></div><div class="modalbox-content"> <img class="icon" alt="" src="${mfn.themepath}/muffin-options/svg/warning.svg"> <h3>${mfn_classes.import.duplicates.length} duplicated styles detected</h3> <p>What do you want to do with them?</p><a class="mfn-btn btn-wide btn-modal-close" href="#"><span class="btn-wrapper">Cancel</span></a> <a class="mfn-btn btn-wide mfn-btn-red mfn-import-classes-overwrite" href="#"><span class="btn-wrapper">Overwrite</span></a> <a class="mfn-btn btn-wide mfn-btn-green mfn-import-classes-keep-both" href="#"><span class="btn-wrapper">Keep both</span></a> </div></div></div>`);

            $('.mfn-import-classes-overwrite').on('click', function(e) {
                e.preventDefault();
                if( $(this).hasClass('loading') ) return;
                $(this).addClass('loading');
                mfn_classes.import.import_data();
            ...
            $('.mfn-import-classes-keep-both').on('click', function(e) {
                e.preventDefault();
                if( $(this).hasClass('loading') ) return;
                $(this).addClass('loading');
                mfn_classes.import.data = mfn_classes.import.duplicates;
                mfn_classes.import.import_data();
            });

        },
        import_data: function() {
            if( !mfn_classes.import.data.length ) {

                $('.modal-confirm.show').remove();
                $('#mfn-preview-wrapper').append('<div style="display: none;" class="mfn-snackbar"><span class="mfn-icon mfn-icon-information"></span><div class="snackbar-message">Global styles imported.</div></div>');
                $('.mfn-snackbar').fadeIn();
                closeSnackbar();
                ...
                $.ajax({
                    url: mfnajaxurl,
                    data: {
                        action: 'mfnupdateclasses',
                        'mfn-builder-nonce': wpnonce,
                        val: JSON.stringify(mfn_classes.import.data),
                        type: 'import'
                    },
                    type: 'POST',
                    success: function(response) {
    ...
    list_modal: {
        // mfn_classes.list_modal

        confirm: function() {
            $('.mfn-ui')
                .append(`<div class="mfn-modal modal-confirm show"> <div class="mfn-modalbox mfn-form mfn-shadow-1"> <div class="modalbox-header"> <div class="options-group"> <div class="modalbox-title-group"> <span class="modalbox-icon mfn-icon-delete"></span> <div class="modalbox-desc"> <h4 class="modalbox-title">Delete global style</h4> </div></div></div><div class="options-group"> <a class="mfn-option-btn mfn-option-blank btn-large btn-modal-close" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a> </div></div><div class="modalbox-content"> <img class="icon" alt="" src="${mfn.themepath}/muffin-options/svg/warning.svg"> <h3>Delete style?</h3> <p>Please confirm. There is no undo.</p><a class="mfn-btn mfn-btn-red btn-wide btn-modal-confirm" href="#"><span class="btn-wrapper">Delete</span></a> </div></div></div>`);

                $('.btn-modal-confirm').on('click', function(e) {
                    e.preventDefault();
                   if( $(this).hasClass('loading') ) return;
                    $(this).addClass('loading');
                    mfn_classes.list_modal.delete();
                });
        },

        reload: function() {
            mfn_classes.list_modal.close();
            mfn_classes.list_modal.show();
            mfn_classes.refresh = false;
        },
        ...
        delete: function() {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('.mfn-global-styles-listing li[data-id="'+mfn_classes.uid+'"]').remove();
                    ...
                    //$('.mfn-ui').removeClass('mfn-modal-open');
                    mfn_classes.list_modal.reload();
                    $('.modal-confirm.show').remove();

                    $content.find('style.mfn-local-style-'+mfn_classes.uid).remove();

                    $('#mfn-preview-wrapper').append('<div style="display: none;" class="mfn-snackbar"><span class="mfn-icon mfn-icon-information"></span><div class="snackbar-message">Style removed.</div></div>');
                    $('.mfn-snackbar').fadeIn();
                    closeSnackbar();
                }
            });
        ...
        rename: function() {
            let tmp_class = mfn.classes.filter( (n) => n.uid == mfn_classes.uid )[0];
            $('.mfn-ui')
                .append(`<div class="mfn-modal modal-confirm show"> <div class="mfn-modalbox mfn-form mfn-shadow-1"> <div class="modalbox-header"> <div class="options-group"> <div class="modalbox-title-group"> <span class="modalbox-icon mfn-icon-be-class"></span> <div class="modalbox-desc"> <h4 class="modalbox-title">Rename global style</h4> </div></div></div><div class="options-group"> <a class="mfn-option-btn mfn-option-blank btn-large btn-modal-close" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a> </div></div><div class="modalbox-content"><div class="mfn-form-row"><input value="${tmp_class.title}" type="text" class="mfn-form-control mfn-class-new-name"></div><a class="mfn-btn mfn-btn-blue btn-modal-confirm" href="#"><span class="btn-wrapper">Save</span></a> <a class="mfn-btn btn-modal-close" href="#"><span class="btn-wrapper">Cancel</span></a> </div></div></div>`);
                ...
                $('.btn-modal-confirm').on('click', function(e) {
                    ...
                    $('.mfn-class-new-name').removeClass('error');
                    if( !$('.mfn-class-new-name').val().length ) {
                        $('.mfn-class-new-name').addClass('error');
                        return;
                    }

                    $(this).addClass('loading');

                    let new_name = $('.mfn-class-new-name').val();
                    let tmp_class = mfn.classes.filter( (n) => n.uid == mfn_classes.uid )[0];

                    tmp_class.title = new_name;

                    $('.mfn-global-styles-listing li[data-id="'+mfn_classes.uid+'"] .mfn-class-title').text(new_name);

                    setTimeout(mfn_classes.ajax(tmp_class), 10);
                });
        },

        duplicate: function() {
            let tmp_class = JSON.parse( JSON.stringify(mfn.classes.filter( (n) => n.uid == mfn_classes.uid )[0]));
            $('.mfn-ui')
                .append(`<div class="mfn-modal modal-confirm show"> <div class="mfn-modalbox mfn-form mfn-shadow-1"> <div class="modalbox-header"> <div class="options-group"> <div class="modalbox-title-group"> <span class="modalbox-icon mfn-icon-be-class"></span> <div class="modalbox-desc"> <h4 class="modalbox-title">Clone global style</h4> </div></div></div><div class="options-group"> <a class="mfn-option-btn mfn-option-blank btn-large btn-modal-close" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a> </div></div><div class="modalbox-content"><div class="mfn-form-row"><input placeholder="Global style name" type="text" class="mfn-form-control mfn-class-new-name"></div><a class="mfn-btn mfn-btn-blue btn-modal-confirm" href="#"><span class="btn-wrapper">Save</span></a> <a class="mfn-btn btn-modal-close" href="#"><span class="btn-wrapper">Cancel</span></a> </div></div></div>`);
                ...
                $('.btn-modal-confirm').on('click', function(e) {
                    ...
                    let new_name = $('.mfn-class-new-name').val();
                    let new_uid = `be_${tmp_class.jsclass}_${getUid()}`;

                    let check_if_exists = mfn.classes.filter( cl => cl.title == new_name );

                    if( check_if_exists.length ) {
                        new_name += '_copy';

                        let check_if_exists2 = mfn.classes.filter( cl => cl.title.includes(new_name) );
                        if( check_if_exists2.length ) new_name += '_'+check_if_exists2.length;
                    }

                    tmp_class.title = new_name;
                    tmp_class.uid = new_uid;

                    if( _.has(tmp_class, 'attr') ){
                        _.map(tmp_class.attr, function(atr, a) {
                            if( _.has(atr, 'selector') ) {
                                //console.log(atr.selector);
                                atr.selector = atr.selector.replaceAll(mfn_classes.uid, new_uid);
                            }
                        });
                    }

                    mfn_classes.refresh = true;
        ...
        show: function() {
            ...
            const sortedData = Object.fromEntries(
                Object.keys(renderMfnFields)
                    .sort() // Sort the keys alphabetically
                    .map(key => [key, renderMfnFields[key]]) // Map back to key-value pairs
            );

            $('.mfn-ui')
                .addClass('mfn-modal-open')
                .append(`<div class="mfn-modal show modal-classes-listing mfn-modal-900">
                        <div class="mfn-modalbox mfn-form mfn-shadow-1">
                            <div class="modalbox-header">
                                <div class="options-group">
                                    <div class="modalbox-title-group">
                                        <span class="modalbox-icon mfn-icon mfn-icon-global-styles-list"></span> 
                                        <div class="modalbox-desc"> 
                                            <h4 class="modalbox-title">Global styles</h4>
                                        </div>
                                    </div>
                                </div>
                                <div class="options-group right"> <div class="modalbox-search"> <div class="form-control"> <input class="mfn-form-control mfn-form-input mfn-search" type="text" placeholder="Search"> </div> </div> </div>
                                <div class="options-group"><a class="mfn-option-btn mfn-option-blank btn-large btn-modal-close" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a></div>
                            </div>
                            <div class="modalbox-content">
                                ${ mfn.classes.length ? `
                                <ul class="mfn-global-styles-listing">
                                    ${ _.map(sortedData, (item, i) => {

                                        if( i == 'items' ) return '';

                                        let classes_find = mfn.classes.filter( c => c.jsclass == i );

                                        return classes_find.length ? `
                                            <li data-name="${i.toLowerCase().replaceAll('_2', '').replaceAll('_', ' ').replaceAll('-', ' ')}">
                                                <div class="title-group">
                                                    <span class="mfn-icon mfn-icon-${i.replaceAll('_', '-')}"></span>
                                                    <h5>${i.replaceAll('_2', '').replaceAll('_', ' ').replaceAll('-', ' ')}</h5>
                                                </div>
                                                <ul>
                                                    ${classes_find.map( cf => {
                                                        return `<li data-name="${ cf.title.toLowerCase() }" data-id="${cf.uid}"><span class="mfn-class-name-wrapper"><span class="mfn-class-title">${ cf.title }</span><a href="#" data-tooltip="Rename" class="mfn-class-rename mfn-option-btn mfn-option-blank btn-small"><span class="mfn-icon mfn-icon-rename"></span></a><pre class="mfn-class-code">${ cf.uid }</pre></span><div class="modal-classes-listing-tools"><a href="#" data-tooltip="Clone" class="mfn-class-duplicate mfn-option-btn mfn-option-blank btn-small"><span class="mfn-icon mfn-icon-clone"></span></a><a href="#" data-tooltip="Delete" class="mfn-class-remove mfn-option-btn mfn-option-blank btn-small"><span class="mfn-icon mfn-icon-delete"></span></a></div></li>`
                                                    }).join('')}
                                                </ul>
                                            </li>
                                        ` : '';
                ...
                $('.mfn-class-remove').on('click', function(e) {
                    e.preventDefault();
                    mfn_classes.uid = $(this).closest('li').attr('data-id');
                    mfn_classes.list_modal.confirm();
                });

                $('.mfn-class-rename').on('click', function(e) {
                    e.preventDefault();
                    mfn_classes.uid = $(this).closest('li').attr('data-id');
                    mfn_classes.list_modal.rename();
                });

                $('.mfn-class-duplicate').on('click', function(e) {
                    e.preventDefault();
                    mfn_classes.uid = $(this).closest('li').attr('data-id');
                    mfn_classes.list_modal.duplicate();
                });
        ...
        close: function() {
            // mfn_classes.close();
            if( !$('.modal-classes-listing.show').length ) return;
            $('.mfn-ui').removeClass('mfn-modal-open');
            mfn_classes.styles = '';
            $('.modal-classes-listing.show').remove();
        }

    },
    get_styles: function(item) {
        mfn_classes.styles = '';

        _.map( item, function(opt, o) {
            if( !_.isEmpty(opt) && ( o.includes('css_') || _.indexOf(mfn_classes.exceptions, o) >= 0 || _.indexOf(mfn_classes.exceptions, edited_item.jsclass+'_'+o) >= 0 ) ) {

                if( _.has(mfn_classes.original, 'attr') && _.has(mfn_classes.original.attr, o) && ( (opt == mfn_classes.original.attr[o]) || (_.has(mfn_classes.original.attr[o], 'val') && opt.val == mfn_classes.original.attr[o]['val']) || ( _.has(mfn_classes.original.attr[o]['val'], 'desktop') && _.has(opt.val, 'desktop') && opt.val.desktop == mfn_classes.original.attr[o]['val']['desktop'] ) ) ) {
                    return;
                }

                mfn_classes.confirmed.push(o);

                let this_opt = $('.mfn-element-fields-wrapper .mfn-form-row[data-id="'+o+'"]');
                let tab_div = $('.mfn-element-fields-wrapper .mfn-form-row[data-id="'+o+'"]').closest('.modalbox-card');
                let tab = 'Style';
                if( tab_div.hasClass('modalbox-card-content') ) tab = 'Settings';
                if( tab_div.hasClass('modalbox-card-advanced') ) tab = 'Advanced';

                let heading = this_opt.prevAll('.row-header').first().text();

                if( heading === null || heading.match(/^ *$/) !== null ) {
                    heading = this_opt.closest('.mfn-form-row').prevAll('.row-header').first().text();
                   
                    if( this_opt.closest('.mfn-sft').length ) {
                        heading = this_opt.closest('.mfn-sft').closest('.mfn-form-row').prevAll('.row-header').first().text();
                    }

                    if( this_opt.closest('.mfn-sidebar-fields-tabs').length ) {
                        heading = this_opt.closest('.mfn-sidebar-fields-tabs').closest('.mfn-form-row').prevAll('.row-header').first().text();
                    }

                    if( o.includes('_hover') ) heading += ':hover';
                    if( o.includes('_active') ) heading += ':active';
                }

                let this_val = '';

                if( _.has(opt, 'val') && typeof opt.val == 'object' && !_.isEmpty(opt.val) ) {
                    this_val = _.map(opt.val, function(v,s) {
                        if( !_.isEmpty(v) && s != 'string' ) {
                            if( typeof v == 'object' ) {
                                let tv = s+"\r\n"+'<ul>';
                                tv += _.map(v, function(c,x) {
                                    return '<li>'+x+': '+c+'</li>';
                                }).join('');
                                tv += '</ul>'
                                return tv;
                ...
                }else if(!_.isEmpty(opt.val)) {
                    this_val = opt.val;
                }else if(!_.isEmpty(opt)) {
                    if( typeof opt == 'object' ){
                        this_val = _.map(opt, function(v,s) {
                            if( !_.isEmpty(v) && s != 'string' ) {
                                if( typeof v == 'object' ) {
                                    let tv = s+"\r\n"+'<ul>';
                                    tv += _.map(v, function(c,x) {
                ...
                //let this_val = _.has(opt, 'val') && typeof opt.val == 'object' ? _.map(opt.val, (v,s) => `${s.trim()}: ${v}<br>`).join('') : `${opt.val}`;

                if( this_val.length && !_.isEmpty(this_val) ) mfn_classes.styles += `<li class="active"><span data-id="${o}" class="mfn-class-styles-check"></span><span class="mfn-class-styles-list-cat">${tab} / ${heading.trim()}</span><span class="mfn-class-styles-list-title">${this_opt.find('label').first().text().trim()}</span><span title="${ this_val.replaceAll('<br>', '; ').replaceAll('<ul>', '').replaceAll('</ul>', "").replaceAll('<li>', "").replaceAll('</li>', "\r\n") }" class="mfn-class-styles-list-val">${ this_val }</span></li>`;
    ...
    modal: function() {
        ...
        mfn_classes.confirmed = [];

        if( _.has(edited_item, 'be_classes') && edited_item.be_classes.length > 0 ) {
            mfn_classes.has_classes = true;
        }else{
            mfn_classes.has_classes = false;
        }

        mfn_classes.original = edited_item.jsclass == 'section' || edited_item.jsclass == 'wrap' ? JSON.parse( JSON.stringify(mfn.elements[edited_item.jsclass]) ) : JSON.parse( JSON.stringify(renderMfnFields.items[edited_item.jsclass]) );
        mfn_classes.get_styles(edited_item.attr);

        $('.mfn-ui')
            .addClass('mfn-modal-open')
            .append(`<div class="mfn-modal show modal-classes mfn-modal-900">
                    <div class="mfn-modalbox mfn-form mfn-shadow-1">
                        <div class="modalbox-header">
                            <div class="options-group">
                                <div class="modalbox-title-group">
                                    <span class="modalbox-icon mfn-icon-be-class"></span> 
                                    <div class="modalbox-desc"> 
                                        <h4 class="modalbox-title">Global styles</h4>
                                    </div>
                                </div>
                        ...
                        <div class="modalbox-content">
                            ${ mfn_classes.styles.length ? 
                            `<h5>${mfn_classes.popup_title}</h5>
                            <div class="mfn-form-row">
                                <label>Options to save</label>
                                <ul class="mfn-class-styles-list">
                                    ${mfn_classes.styles}
                                </ul>
                            </div>
                            <div class="mfn-form-row">
                                <label>Create new global style</label>
                                <input class="mfn-form-control mfn-form-input mfn-new-class-name-input" type="text" placeholder="type-style-name" autocomplete="off">
                            </div>
                            <a class="mfn-btn mfn-btn-blue btn-save-new-class btn-modal-confirm" ${mfn.view == 'demo' ? 'data-tooltip="Unavailable in Demo"' : ''} href="#"><span class="btn-wrapper">Save</span></a> 
                            <a class="mfn-btn mfn-new-class-toggle btn-modal-close" href="#"><span class="btn-wrapper">Cancel</span></a>
                        ` : '<h5>No CSS given</h5>' }
                        </div>
                    </div>
                </div>`
    ...
    close: function() {
        ...
        if( !$('.modal-classes.show').length ) return;
        $('.mfn-ui').removeClass('mfn-modal-open');
        mfn_classes.styles = '';
        $('.modal-classes.show').remove();
    },
    ...
    create: function() {
        ...
        if( mfn.view == 'demo' ) return;
        $('.mfn-new-class-name-input').removeClass('error');
        $('.modal-classes .mfn-input-error-msg').remove();

        var class_name_input = $('.mfn-new-class-name-input');
        var class_name = `be_${edited_item.jsclass}_${getUid()}`;

        if( !class_name_input.val().length ) {
            class_name_input.addClass('error');
            $('.modal-classes.show .mfn-btn.loading').before('<span class="mfn-input-error-msg">Make sure the title is not empty and not taken</span>').removeClass('loading disabled');
            return;
        }

        if( mfn.classes.length && mfn.classes.filter( (n) => n.title == class_name_input.val() ).length ) {
            class_name_input.addClass('error');
            $('.modal-classes.show .mfn-btn.loading').before('<span class="mfn-input-error-msg">This name is already taken. Please choose another.</span>').removeClass('loading disabled');
            return;
        }

        if( !_.has(edited_item, 'be_classes') ) edited_item['be_classes'] = [];
        if( _.indexOf(edited_item.be_classes, class_name) >= 0 ) return;

        let copy_ei = JSON.parse( JSON.stringify(edited_item) );

        let tmp_class = {
            jsclass: copy_ei.jsclass,
            type: 'be_class',
            title: class_name_input.val(),
            uid: class_name,
            attr: {}
        };

        _.map( copy_ei.attr, function(opt, o) {
            if( ( o.includes('css_') && _.indexOf(mfn_classes.confirmed, o) >= 0 ) || _.indexOf(mfn_classes.exceptions, o) >= 0 || _.indexOf(mfn_classes.exceptions, copy_ei.jsclass+'_'+o) >= 0 ) {
                tmp_class['attr'][o] = opt;
                if( _.indexOf(mfn_classes.exceptions, o) < 0 ) delete(edited_item.attr[o]);
            }
        });

        edited_item['be_classes'] = [];
        
        edited_item['be_classes'].push(class_name);
        $('.sidebar-panel-classes-list  .sidebar-panel-classes-input').before('<span data-name="'+class_name+'" class="sidebar-panel-class mfn-autocomplete-item">'+class_name_input.val()+'<span class="sidebar-panel-class-edit mfn-autocomplete-item-edit">&#9998;</span> <span class="sidebar-panel-class-delete mfn-autocomplete-item-delete">&#10005;</span></span>');
        //$builder.find(`.mcb-${edited_item.jsclass}-${edited_item.uid}`).addClass(class_name);
        $builder.find('.'+$('.mfn-element-fields-wrapper').attr('data-element')).addClass(class_name);

        setTimeout(mfn_classes.ajax(tmp_class), 10);
    },

    ajax: function(tmp_class) {

        // mfn_classes.ajax();

        // mfn_classes.has_changed = false;

        _.map( tmp_class.attr, function(opt, o) {
            if( (o.includes('css_') && _.has(opt, 'selector')) || _.indexOf(mfn_classes.exceptions, o) < 0 || _.indexOf(mfn_classes.exceptions, tmp_class.jsclass+'_'+o) < 0 ) {
                if( o.includes('css_') && _.has(opt, 'selector') ) opt.selector = opt.selector.replaceAll('.mcb-section .mcb-wrap .mcb-item-mfnuidelement', '.column.'+tmp_class.uid).replaceAll('.mcb-section .mcb-wrap-mfnuidelement', '.'+tmp_class.uid).replaceAll('mcb-section-mfnuidelement', tmp_class.uid);
                tmp_class['attr'][o] = opt;
            }
        });
        ...
        $.ajax({
            ...
            success: function(response) {
                mfn_classes.close();
                mfn.classes = response;
                mfn_classes.uid = false;
                mfn_classes.confirmed = [];

                //sidebar_form.open( $content.find('.mfn-current-editing'), false );
                sidebar_form.render();
                $content.find('style.mfn-local-style-'+edited_item.uid).remove();
                loopAllStyleFields();

                if( mfn_classes.refresh_all_items_with_class ) {
                    if(  $builder.find('.'+tmp_class.uid).length ) {
                        $builder.find('.vb-item.'+tmp_class.uid).each(function() {
                    ...
                    mfn_classes.refresh_all_items_with_class = false;
                }
    
                if($('.mfn-class-editor-update').length) $('.mfn-class-editor-update').removeClass('loading');
                if( $('.modal-confirm.show').length ) $('.modal-confirm.show').remove();

                if( mfn_classes.refresh ) mfn_classes.list_modal.reload();

                $('#mfn-preview-wrapper').append('<div style="display: none;" class="mfn-snackbar"><span class="mfn-icon mfn-icon-information"></span><div class="snackbar-message">Global styles updated.</div></div>');
                $('.mfn-snackbar').fadeIn();
                closeSnackbar();
            }
        });
    ...
    remove: function(classname) {
        $('.sidebar-panel-classes-list  .sidebar-panel-class[data-name="'+classname+'"]').remove();

        if($('.mfn-ui').hasClass('mfn-class-editor-full')) $('.mfn-ui').removeClass('mfn-class-editor-full');
        
        if( _.has(edited_item, 'be_classes') ) {
            let index = _.indexOf(edited_item.be_classes, classname);
            if( index >= 0 ) edited_item.be_classes.splice(index, 1);

            $builder.find('.'+$('.mfn-element-fields-wrapper').attr('data-element')).removeClass(classname);
            sidebar_form.render();
        }

        if( _.has(edited_item, 'be_classes') && !edited_item.be_classes.length ) {
            $('.mfn-classes-used-info').remove();
            delete(edited_item.be_classes);
        }
    },

    edit: function(classname) {
        
        let this_class = mfn.classes.filter( (c) => c.uid == classname )[0];

        if( !this_class || !_.has(this_class, 'jsclass') ) {
            console.error('There is no such class on the list. Please refresh the page.');
            return false;
        }

        $('.sidebar-panel-classes').remove();
        //$('#header-edit-item .sidebar-panel-title').text('.'+this_class.nicename);

        edited_item = this_class;
        sidebar_form.is_class = classname;

        sidebar_form.open($content.find('.mfn-current-editing'), true);
...
mfn_live_css = {
    ...
    init: function() {
        ...
        // if( localStorage.getItem("be_live_css_history") ) localStorage.removeItem("be_live_css_history");

        $(document).on('click', '.btn-live-css-switcher:not(.disabled)', function(e) {
            e.preventDefault();
            mfn_live_css.restore.index = false;
            if( $('.mfn-ui').hasClass('mfn-live-css-editor-active') ) {
                mfn_live_css.hide();
            }else{
                if( localStorage.getItem("be_live_css") ) mfn_live_css.tab = localStorage.getItem("be_live_css");
                mfn_live_css.show();
            }
        });

        if( localStorage.getItem("be_live_css_history") ) {
            let helpr = JSON.parse(localStorage.getItem("be_live_css_history"));
            mfn_live_css['history'] = JSON.parse( JSON.stringify(helpr) );
        }else{
            mfn_live_css.backup('global_css');
            mfn_live_css.backup('page_css');
            mfn_live_css.backup('css_variables');
        }

        document.addEventListener('visibilitychange', function () {
            if( document.visibilityState === 'visible' ) {
                mfn_live_css.refresh();
            }
        });

        $(document).on('mouseup', '.mfn-live-css-editor .mfn-builder-tabs li', function(e) {
            e.preventDefault();
            if( $(this).hasClass('active') ) return;
            mfn_live_css.restore.index = false;
            mfn_live_css.tab = $(this).attr('data-tab');
            $(this).siblings('li').removeClass('active');
            $('.mfn-live-css-editor .editor-wrapper.active').removeClass('active');
            $(this).addClass('active');
            localStorage.setItem("be_live_css", mfn_live_css.tab);
            mfn_live_css[mfn_live_css.tab]();
        });

        $(document).on('change', '.mfn-live-css-editor input#live-css-update', function() {
            if( $(this).prop('checked') == true ) {
                mfn_live_css.live_update = true;
                $('.mfn-live-css-editor').addClass('mfn-live-css-editor-live-update');
                $('.mfn-live-css-editor .footer-buttons .mfn-btn').addClass('disabled');
            }else{
                mfn_live_css.live_update = false;
                $('.mfn-live-css-editor').removeClass('mfn-live-css-editor-live-update');
                $('.mfn-live-css-editor .footer-buttons .mfn-btn').removeClass('disabled');
            }
        });

        $(document).on('click', '.mfn-live-css-editor .variable-title', function(e) {
            e.preventDefault();
            let button = $(this);
            navigator.clipboard.writeText(button.text())
            button.attr('data-tooltip', 'Copied');
            setTimeout(function() {
                button.attr('data-tooltip', 'Click to copy');
            }, 3000);
        });

        $(document).on('click', '.mfn-live-css-editor .editor-toolbar .mfn-editor-append', function(e) {
            e.preventDefault();
            let val = $(this).attr('data-val');
            let cm = mfn_live_css.editors['mfn-css-live-cm-editor-'+mfn_live_css.tab].codemirror;

            const lastLine = cm.lastLine();
            const scrollInfo = cm.getScrollInfo();

            // Dodaj nową linię z deklaracją
            const from = { line: lastLine + 1, ch: 0 };
            cm.replaceRange('\n' + val, from);

            // Znajdź linię, którą właśnie wstawiliśmy
            const insertedLine = cm.getLine(from.line);

            // Znajdź pozycję nawiasu klamrowego `{`
            const braceIndex = insertedLine.indexOf('{');
            const selectorEnd = braceIndex > -1 ? braceIndex : insertedLine.length;

            // Zaznacz tylko ".class" (bez nawiasu)
            const selectionFrom = { line: from.line, ch: 0 };
            const selectionTo = { line: from.line, ch: selectorEnd };
            cm.setSelection(selectionFrom, selectionTo);
            cm.focus();

            // Przywróć scroll
            cm.scrollTo(scrollInfo.left, scrollInfo.top+70);
        });

        $(document).on('click', '.mfn-live-css-editor .btn-live-css-editor-save', function(e) {
            e.preventDefault();
            if( mfn_live_css.pending ) return;
            mfn_live_css.pending = true;
            $(this).addClass('loading disabled');
            let cm = mfn_live_css.editors['mfn-css-live-cm-editor-'+mfn_live_css.tab].codemirror;
            let val = cm.getValue();
            mfn_live_css['update'][mfn_live_css.tab](val);
        });

        $(document).on('click', '.css-variables-form .mfn-css-variable-save', function(e) {
            e.preventDefault();
            mfn_live_css.new_var = {};
            let button = $(this);
            let title_input = $('.css-variables-form .mfn-css-var-title');
            let value_input = $('.css-variables-form .mfn-css-var-value');

            if( button.hasClass('loading') ) return;

            $('.css-variables-form .mfn-form-input').removeClass('error');

            if( !title_input.val().length || !value_input.val().length ) {
                if( !title_input.val().length ) title_input.addClass('error');
                if( !value_input.val().length ) value_input.addClass('error');
                return;
            }

            const regex = /^[a-z0-9-]+$/;
            if( !regex.test(title_input.val()) ) {
                title_input.addClass('error');
                return;
            }

            if( !title_input.val().startsWith('--') ) {
                title_input.val('--'+title_input.val());

                if( title_input.val().includes('---') ){
                    title_input.val( title_input.val().replace('---', '--') );
                }
            }

            button.addClass('loading');

            mfn_live_css.new_var.title = title_input.val();
            mfn_live_css.new_var.value = value_input.val();

            mfn_live_css.update.css_variables();
        });

        $(document).on('click', '.mfn-live-css-editor .mfn-css-var-action-delete', function(e) {
            e.preventDefault();
            let title = $(this).attr('data-title');
            mfn.css_variables = mfn.css_variables.filter( (x) => x.title != title );
            mfn_live_css.variables_list();
            mfn_live_css['update']['variables_ajax']();
        });

        $(document).on('click', '.mfn-live-css-editor .mfn-css-var-action-edit', function(e) {
            e.preventDefault();
            let title = $(this).attr('data-title');
            mfn_live_css.is_editing = title;
            $('.css-variables-form .mfn-css-var-title').val(title);
            $('.css-variables-form .mfn-css-var-value').val( mfn.css_variables.filter( (x) => x.title == title )[0]['value'] );

        });

        $(document).on('click', '.mfn-live-css-editor .btn-live-css-editor-history-undo', function(e) {
            e.preventDefault();
            mfn_live_css.restore.undo();
        });

        $(document).on('click', '.mfn-live-css-editor .btn-live-css-editor-history-redo', function(e) {
            e.preventDefault();
            mfn_live_css.restore.redo();
        });

        $(document).on('keyup focus', '.panel-edit-item .object-css-input input.mfn-form-input', function(e) {
            if( !mfn['css_variables'] || !mfn['css_variables'].length ) return;

            if( e.key === "ArrowDown" || e.key === "ArrowUp" || e.key === "Enter" ) return;

            mfn_live_css.input = $(this);
            mfn_live_css.row = mfn_live_css.input.closest('.mfn-form-row');

            if( mfn_live_css.row.find('.mfn-css-variables-dropdown').length ) mfn_live_css.row.find('.mfn-css-variables-dropdown').remove();
            if( mfn_live_css.input.hasClass('mfn-css-variable-input-active') ) mfn_live_css.input.removeClass('mfn-css-variable-input-active');

            if( !mfn_live_css.input.val().length ) return;

            let val_search = mfn_live_css.input.val().replace('var(', '').replace(')', '');
            
            let filter = mfn.css_variables.filter( (x) => x.title.includes(val_search) );

            if( !filter.length ) return;
            mfn_live_css.input.addClass('mfn-css-variable-input-active');

            let html = '<ul class="mfn-css-variables-dropdown">';
            
                filter.map(function(cv) {
                    html += `<li><a class="mfn-css-variable-item" href="#"><span class="mfn-css-variable-item-title">${cv.title}</span><code class="mfn-css-variable-item-val variable-value ${cv.value.includes('#') ? 'variable-color' : ''}">${cv.value.includes('#') ? '<span style="background-color: '+cv.value+';"></span>' : ''} ${cv.value}</code></a></li>`;
                }).join('');

            html += '</ul>';

            mfn_live_css.row.append(html);

            //if( mfnPickr && mfn_live_css.input.hasClass('color-picker-vb') ) mfnPickr.hide();

            $(document).bind('click', mfn_live_css.hide_classes_dropdown);

        });


        document.addEventListener("keyup", (e) => {
            if( !mfn_live_css.input || !$('.mfn-css-variables-dropdown li').length ) return
            if (e.key === "ArrowDown") {
                e.preventDefault();
                if( $('.mfn-css-variables-dropdown li.active').length ) {
                    let next = $('.mfn-css-variables-dropdown li.active').next('li').length ? $('.mfn-css-variables-dropdown li.active').next('li') : $('.mfn-css-variables-dropdown li').first();
                    $('.mfn-css-variables-dropdown li.active').removeClass('active');
                    next.addClass('active');
                }else{
                    $('.mfn-css-variables-dropdown li').first().addClass('active')
                }
            } else if (e.key === "ArrowUp") {
                e.preventDefault();
                if( $('.mfn-css-variables-dropdown li.active').length ) {
                    let next = $('.mfn-css-variables-dropdown li.active').prev('li').length ? $('.mfn-css-variables-dropdown li.active').prev('li') : $('.mfn-css-variables-dropdown li').last();
                    $('.mfn-css-variables-dropdown li.active').removeClass('active');
                    next.addClass('active');
                }else{
                    $('.mfn-css-variables-dropdown li').last().addClass('active')
                }
            } else if (e.key === "Enter") {
                if( $('.mfn-css-variables-dropdown li.active').length ) $('.mfn-css-variables-dropdown li.active > a.mfn-css-variable-item').trigger('click');
            }
        });


        $(document).on('click', '.mfn-css-variables-dropdown li a.mfn-css-variable-item', function(e) {
            e.preventDefault();

            let val = $(this).find('.mfn-css-variable-item-title').text();
            mfn_live_css.input.val(val).trigger('change');

            if( mfn_live_css.input.hasClass('mfn-css-variable-input-active') ) mfn_live_css.input.removeClass('mfn-css-variable-input-active');
            if( mfn_live_css.row.find('.mfn-css-variables-dropdown').length ) mfn_live_css.row.find('.mfn-css-variables-dropdown').remove();

            if( mfn_live_css.input.hasClass('color-picker-vb') && $(this).find('code').text().includes('#') ) {
                mfn_live_css.input.closest('.color-picker-group').find('.form-addon-prepend .label').css('border-color', val).css('background-color', val);
                mfn_live_css.input.closest('.color-picker-group').find('.form-addon-prepend .label').removeClass('light dark').addClass(getContrastYIQ( $(this).find('code').text().replace(' ', '') ));
            }

            mfn_live_css.input = false;
            mfn_live_css.row = false;

            $(document).unbind('click', mfn_live_css.hide_classes_dropdown);

        });

    },

    hide_classes_dropdown: function(e) {
        var div = $('.mfn-css-variables-dropdown');

        if( !mfn_live_css.input ){
            $(document).unbind('click', mfn_live_css.hide_classes_dropdown);
            return;
        }

        if(!div.is(e.target) && !mfn_live_css.input.is(e.target)) {
            if( mfn_live_css.input.hasClass('mfn-css-variable-input-active') ) mfn_live_css.input.removeClass('mfn-css-variable-input-active');
            if( mfn_live_css.row.find('.mfn-css-variables-dropdown').length ) mfn_live_css.row.find('.mfn-css-variables-dropdown').remove();
    ...
    refresh: function() {
        ...
        $.ajax( mfnajaxurl, {
            ...
            data: {
              'mfn-builder-nonce': wpnonce,
              action: 'mfn_vb_refresh_live_css',
              postid: mfn.pageid 
            },
            success: function(response) {

                let force_hr = response.themecss == mfn['themeoptions']['custom-css'] ? false : true;
                
                mfn['css_variables'] = response.variables;
                mfn['page_options']['mfn-post-css'] = response.postcss;
                mfn['themeoptions']['custom-css'] = response.themecss;

                if( force_hr && localStorage.getItem("be_live_css_history") ) {
                    let helpr = JSON.parse(localStorage.getItem("be_live_css_history"));
                    mfn_live_css['history'] = JSON.parse( JSON.stringify(helpr) );
                }

                $content.find('style.mfn-be-page-custom-css').html(response.postcss);
                $content.find('style.mfn-be-theme-custom-css').html(response.themecss);

                if( $('.mfn-ui').hasClass('mfn-live-css-editor-active') ) {
                    mfn_live_css.hide();
                    $('.btn-live-css-switcher:not(.disabled)').trigger('click');
                }

                mfn_live_css.pending = false;
                
            }
        });
    },

    backup: function( tab = false ) {

        let tabb = tab ? tab : mfn_live_css.tab;

        if( !_.has(mfn_live_css.history, tabb) ) mfn_live_css.history[tabb] = [];

        if( tabb == 'global_css' && _.has(mfn['themeoptions'], 'custom-css') ) {
            mfn_live_css.history[tabb].push(mfn['themeoptions']['custom-css']);
        }else if( tabb == 'page_css' && _.has(mfn['page_options'], 'mfn-post-css') ) {
            mfn_live_css.history[tabb].push(mfn['page_options']['mfn-post-css']);
        }else if( tabb == 'css_variables' && _.has(mfn, 'css_variables') ) {
            mfn_live_css.history[tabb].push( JSON.parse(JSON.stringify(mfn['css_variables'])) );
        }

        if( $('.mfn-live-css-editor .btn-live-css-editor-history-undo').hasClass('disabled') && mfn_live_css.history[tabb].length ) $('.mfn-live-css-editor .btn-live-css-editor-history-undo').removeClass('disabled');

        localStorage.setItem("be_live_css_history", JSON.stringify(mfn_live_css.history));
    },
    ...
    restore: {
        ...
        undo: function() {
            ...
            if( !_.has(mfn_live_css.history, mfn_live_css.tab) || !mfn_live_css.history[mfn_live_css.tab].length ) return;

            let total = mfn_live_css.history[mfn_live_css.tab].length > 1 ? mfn_live_css.history[mfn_live_css.tab].length - 1 : mfn_live_css.history[mfn_live_css.tab].length;

            if( mfn_live_css.restore.index === false ) mfn_live_css.restore.index = total;

            mfn_live_css.restore.index--;

            if( mfn_live_css.restore.index >= 0 ) {
                mfn_live_css.restore.replace();
                if( mfn_live_css.restore.index == 0 ) $('.mfn-live-css-editor .btn-live-css-editor-history-undo').addClass('disabled');
            }else{
                mfn_live_css.restore.index = 0;
                $('.mfn-live-css-editor .btn-live-css-editor-history-undo').addClass('disabled');
            }

            if( $('.mfn-live-css-editor .btn-live-css-editor-history-redo').hasClass('disabled') ) $('.mfn-live-css-editor .btn-live-css-editor-history-redo').removeClass('disabled');

            //console.log(mfn_live_css.restore.index);
        ...
        redo: function() {
            ...
            let total = mfn_live_css.history[mfn_live_css.tab].length - 1;

            mfn_live_css.restore.index++;

            if( mfn_live_css.restore.index <= total  ) {
                mfn_live_css.restore.replace();
                if( mfn_live_css.restore.index == total ) $('.mfn-live-css-editor .btn-live-css-editor-history-redo').addClass('disabled');
            }else{
                mfn_live_css.restore.index = total;
                $('.mfn-live-css-editor .btn-live-css-editor-history-redo').addClass('disabled');
            }

            if( $('.mfn-live-css-editor .btn-live-css-editor-history-undo').hasClass('disabled') ) $('.mfn-live-css-editor .btn-live-css-editor-history-undo').removeClass('disabled');

            //console.log(mfn_live_css.restore.index);

        },

        replace: function( val = false ) {

            let to_restore = val ? val : mfn_live_css.history[mfn_live_css.tab][mfn_live_css.restore.index];

            if( !to_restore ) return;

            // console.log({'restoring': to_restore});

            if( mfn_live_css.tab == 'global_css' || mfn_live_css.tab == 'page_css' ) {

                let cm = mfn_live_css.editors['mfn-css-live-cm-editor-'+mfn_live_css.tab].codemirror;
                let new_val = to_restore;
                cm.setValue(new_val);
                
            }else if( mfn_live_css.tab == 'css_variables' ) {
                mfn_live_css.variables_list(to_restore);
            }
    ...
    show: function() {
        $('.mfn-ui').addClass('mfn-live-css-editor-active');

        if( edited_item && ( ( _.has(edited_item, 'uid') && edited_item.uid == 'pageoptions' ) || _.has(edited_item, 'last_tab') ) ) backToWidgets();

        let html = `<div class="mfn-live-css-editor ${mfn_live_css.live_update ? `mfn-live-css-editor-live-update` : ''}">
        
        <header class="editor-header">
            <span>Custom:</span>
            <ul class="mfn-builder-tabs">
                <li ${mfn_live_css.tab == 'global_css' ? 'class="active"' : ''} data-tab="global_css">Global CSS</li>
                <li ${mfn_live_css.tab == 'page_css' ? 'class="active"' : ''}  data-tab="page_css">Current Page CSS</li>
                <li ${mfn_live_css.tab == 'css_variables' ? 'class="active"' : ''}  data-tab="css_variables">CSS Variables <span class="mfn-badge badge-filled">${mfn.css_variables ? mfn.css_variables.length : 0}</span></li>
            </ul>
            <div class="editor-header-tools">
                <a class="mfn-option-btn mfn-option-blank btn-large btn-live-css-editor-history-undo" title="Undo" href="#"><span class="mfn-icon mfn-icon-undo"></span></a>
                <a class="mfn-option-btn mfn-option-blank btn-large btn-live-css-editor-history-redo" title="Redo" href="#"><span class="mfn-icon mfn-icon-redo"></span></a>
                <a class="mfn-option-btn mfn-option-blank btn-large btn-live-css-editor-close btn-live-css-switcher" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a>
            </div>
        </header>

        <div class="editor-toolbar-wrapper">
            <p class="warning"><span>Warning:</span> Improper CSS modifications may result in visual or functional issues on your WordPress site. Proceed with caution.</p>
        </div>
    ...
    css_editor_tollbar: function() {

        if( $('.mfn-live-css-editor .editor-toolbar').length ) return;

        $('.mfn-live-css-editor .editor-toolbar-wrapper').append(`
            <div class="editor-toolbar">
                <p>Toolset:</p>
                <div class="mfn-option-dropdown mfn-option-dropdown-left">
                    <a href="#" class="mfn-option-btn btn-large mfn-option-text btn-icon-right mfn-option-blank"><span class="text">@media</span><span class="mfn-icon mfn-icon-arrow-down"></span></a>
                    <div class="dropdown-wrapper">
                    <a class="mfn-dropdown-item mfn-editor-append" data-val="@media only screen and (min-width: 1441px) {\r\n\r\n}" href="#">Desktop <span class="res">1441px</span> <</a>
                    <a class="mfn-dropdown-item mfn-editor-append" data-val="@media only screen and (min-width: 960px) and (max-width: 1440px) {\r\n\r\n}" href="#">Laptop <span class="res">960</span> - <span class="res">1440px</span></a>
                    <a class="mfn-dropdown-item mfn-editor-append" data-val="@media only screen and (min-width: 768px) and (max-width: 959px) {\r\n\r\n}" href="#">Tablet <span class="res">768</span> - <span class="res">959px</span></a>
                    <a class="mfn-dropdown-item mfn-editor-append" data-val="@media only screen and (max-width: 767px) {\r\n\r\n}" href="#">Mobile < <span class="res">767px</span></a>
                    <a class="mfn-dropdown-item mfn-editor-append" data-val="@media only screen and (max-width: 959px) {\r\n\r\n}" href="#">Mobile & Tablet < <span class="res">959px</span></a>
                </div>
            </div>
            <a href="#" class="mfn-option-btn btn-large mfn-option-text mfn-option-blank mfn-editor-append" data-val="#id{\r\n\r\n}"><span class="text">#id</span></a>
            <a href="#" class="mfn-option-btn btn-large mfn-option-text mfn-option-blank mfn-editor-append" data-val=".class{\r\n\r\n}"><span class="text">.class</span></a>
            <div class="live-update">
                <div class="mfn-switch">
                    <input id="live-css-update" type="checkbox" ${mfn_live_css.live_update ? `checked` : ''} />
                    <label for="live-css-update" class="switch"></label>
                </div>  
                Live Update
            </div>
            </div>
    ...
    css_editor_footer: function() {
        
        if( $('.mfn-live-css-editor .editor-footer').length ) return;

        $('.mfn-live-css-editor').append(`
            <footer class="editor-footer">
                <span class="editor-footer-theme-options-info">Custom CSS is available also in <a href="admin.php?page=be-options#css">Theme Options</a></span>
                <div class="footer-buttons">
                    <a class="mfn-btn mfn-btn btn-live-css-switcher ${mfn_live_css.live_update ? 'disabled' : ''}" href="#"><span class="btn-wrapper">Cancel</span></a>
                    <a class="mfn-btn mfn-btn-blue btn-live-css-editor-save ${mfn_live_css.live_update ? 'disabled' : ''}" href="#"><span class="btn-wrapper">Save changes</span></a>
                </div>
            </footer>
        `);
    ...
    init_cm: function() {
        ...
        if( !$('.mfn-live-css-editor .editor-wrapper.active .css-live-cm-editor').length ) return;
        if( $('.mfn-live-css-editor .editor-wrapper.active .css-live-cm-editor.cm-initialized').length ) return;

        let id = 'mfn-css-live-cm-editor-'+mfn_live_css.tab;
        let type = 'css';
        let editor = $('.mfn-live-css-editor .editor-wrapper.active .css-live-cm-editor');

        editor.attr('id', id);
        editor.addClass('cm-initialized');

        wp.codeEditor.defaultSettings.codemirror.mode = 'text/'+type;

        mfn_live_css.editors[id] = wp.codeEditor.initialize( id, mfn_cm[type] );
        mfn_live_css.editors[id].codemirror.setOption( 'lint', true );
        mfn_live_css.editors[id].codemirror.refresh();

        /*mfn_live_css.editors[id].codemirror.setOption('extraKeys', { 'Ctrl-Space': 'autocomplete' });
        mfn_live_css.editors[id].codemirror.setOption('hintOptions', { completeSingle: false });*/

        /*mfn_live_css.editors[id].codemirror.on('inputRead', function(editor, change) {
          if (change.text[0].match(/[\w-]/)) {
            editor.showHint();
          }
        });*/

        if( mfn_live_css.live_update ) {
            mfn_live_css.editors[id].codemirror.on('blur', function(cm, change) {

                if( mfn_live_css.pending ) return;
                mfn_live_css.pending = true;

                mfn_live_css['update'][mfn_live_css.tab](cm.getValue());
            });
        }
    ...
    update: {
        global_css: function(val) {

            mfn['themeoptions']['custom-css'] = val;
            $content.find('style.mfn-be-theme-custom-css').html(val);
            $('.mfn-live-css-editor .btn-live-css-editor-history-undo').addClass('loading');
            ...
            $.ajax( mfnajaxurl, {
                ...
                data: {
                  ...
                  betheme: mfn['themeoptions']
                },
                success: function(response) {
                    mfn_live_css.pending = false;
                    if( $('.mfn-live-css-editor .btn-live-css-editor-save').hasClass('loading') ) $('.mfn-live-css-editor .btn-live-css-editor-save').removeClass('loading disabled');
                    mfn_live_css.backup();
                    $('.mfn-live-css-editor .btn-live-css-editor-history-undo').removeClass('loading');

                    let now_date = new Date();
                    localStorage.setItem('be_live_css_history_time', now_date.getTime());
                }
            });

        },

        page_css: function(val) {

            $('.mfn-live-css-editor .btn-live-css-editor-history-undo').addClass('loading');

            $content.find('style.mfn-be-page-custom-css').html(val);
            mfn['page_options']['mfn-post-css'] = val;
            updatePageOpt('mfn-post-css', val);

        },

        css_variables: function() {

            let title = mfn_live_css.new_var.title;

            if( mfn_live_css.is_editing ) title = mfn_live_css.is_editing;

            let find = mfn.css_variables.filter( (x) => x.title == title );

            if( mfn_live_css.is_editing ) {
                find[0]['value'] = mfn_live_css.new_var.value;
                find[0]['title'] = mfn_live_css.new_var.title;
            }else if(find.length) {
                find[0]['value'] = mfn_live_css.new_var.value;
            }else{
                mfn.css_variables.push(mfn_live_css.new_var);
            }
            
            mfn_live_css.is_editing = false;

            mfn_live_css.new_var = {};
            mfn_live_css.variables_list();

            $('.css-variables-form .mfn-css-variable-save').removeClass('loading');
            $('.css-variables-form .mfn-form-input').val('');

            $('.mfn-builder-tabs .mfn-badge').html( mfn.css_variables.length );

            mfn_live_css['update']['variables_ajax']();
        },

        variables_ajax: function() {
            $('.mfn-live-css-editor .btn-live-css-editor-history-undo').addClass('loading');
            $.ajax( mfnajaxurl, {
                type: "POST",
                data: {
                  'mfn-builder-nonce': wpnonce,
                  action: 'mfn_vb_css_variables',
                  val: mfn['css_variables']
                },
                success: function(response){
                    mfn_live_css.backup();
                    $('.mfn-live-css-editor .btn-live-css-editor-history-undo').removeClass('loading');
    ...
    global_css: function() {
        if( $('.mfn-live-css-editor .editor-wrapper.global-css-wrapper').length ){
            $('.mfn-live-css-editor .editor-wrapper.global-css-wrapper').addClass('active');
        }else{
            let html = `<div class="editor-wrapper css-live-cm-editor global-css-wrapper active">
                <textarea class="mfn-global-css-live-editor css-live-cm-editor">${_.has( mfn.themeoptions, 'custom-css' ) ? mfn['themeoptions']['custom-css'] : ''}</textarea>
            </div>`;
            if( $('.mfn-live-css-editor .editor-toolbar-wrapper').length ){
                $('.mfn-live-css-editor .editor-toolbar-wrapper').after(html);
            }else{
                $('.mfn-live-css-editor .editor-header').after(html);
            }
        }
        mfn_live_css.css_editor_tollbar();
        mfn_live_css.css_editor_footer();

        if( !_.has(mfn_live_css.history, mfn_live_css.tab) || mfn_live_css.history[mfn_live_css.tab].length <= 1 ) {$('.mfn-live-css-editor .btn-live-css-editor-history-undo').addClass('disabled');}else{ $('.mfn-live-css-editor .btn-live-css-editor-history-undo').removeClass('disabled'); }
        if( !$('.mfn-live-css-editor .btn-live-css-editor-history-redo').hasClass('disabled') ) $('.mfn-live-css-editor .btn-live-css-editor-history-redo').addClass('disabled');
        if( $('.mfn-live-css-editor .editor-wrapper.active .css-live-cm-editor').length ) mfn_live_css.init_cm();

        $('.mfn-live-css-editor').removeClass('mfn-live-page-css-editor mfn-live-css-variables-editor').addClass('mfn-live-global-css-editor');
    },

    page_css: function() {
        if( $('.mfn-live-css-editor .editor-wrapper.current-page-css-wrapper').length ){
            $('.mfn-live-css-editor .editor-wrapper.current-page-css-wrapper').addClass('active');
        }else{
            let html = `<div class="editor-wrapper css-live-cm-editor current-page-css-wrapper active">
                <textarea class="mfn-page-css-live-editor css-live-cm-editor">${_.has( mfn.page_options, 'mfn-post-css' ) ? mfn['page_options']['mfn-post-css'] : ''}</textarea>
            </div>`;
            if( $('.mfn-live-css-editor .editor-toolbar-wrapper').length ){
                $('.mfn-live-css-editor .editor-toolbar-wrapper').after(html);
            }else{
        ...
        $('.mfn-live-css-editor').removeClass('mfn-live-css-variables-editor mfn-live-global-css-editor').addClass('mfn-live-page-css-editor');
    },

    css_variables: function() {

        if( $('.mfn-live-css-editor .editor-toolbar').length ) $('.mfn-live-css-editor .editor-toolbar').remove();

        if( $('.mfn-live-css-editor .editor-wrapper.css-variables-wrapper').length ) {
            $('.mfn-live-css-editor .editor-wrapper.css-variables-wrapper').addClass('active');
        }else{
            $('.mfn-live-css-editor').append(`
                <div class="editor-wrapper css-variables-wrapper active">

                <div class="mfn-css-variables">

                    <p class="body-tag">body {</p>

                    <ul class="mfn-css-variables-list"></ul>

                    <div class="mfn-form css-variables-form">
                        <input class="mfn-form-control mfn-form-input mfn-css-var-title" placeholder="--variable-title" type="text" />
                        <span class="variable-sep">=</span>
                        <input class="mfn-form-control mfn-form-input mfn-css-var-value" placeholder="value" type="text" />
                        <a href="#" class="mfn-btn mfn-css-variable-save mfn-btn-blue">Save variable</a>
                    </div>
                    <p class="body-tag">}</p>
                </div>
        ...
        if( !_.has(mfn_live_css.history, mfn_live_css.tab) || mfn_live_css.history[mfn_live_css.tab].length <= 1 ) {$('.mfn-live-css-editor .btn-live-css-editor-history-undo').addClass('disabled');}else{ $('.mfn-live-css-editor .btn-live-css-editor-history-undo').removeClass('disabled'); }
        if( !$('.mfn-live-css-editor .btn-live-css-editor-history-redo').hasClass('disabled') ) $('.mfn-live-css-editor .btn-live-css-editor-history-redo').addClass('disabled');
        if( $('.mfn-live-css-editor .editor-footer').length ) $('.mfn-live-css-editor .editor-footer').remove();
        mfn_live_css.variables_list();

        $('.mfn-live-css-editor').removeClass('mfn-live-page-css-editor mfn-live-global-css-editor').addClass('mfn-live-css-variables-editor');
    },

    sortable: function() {
        $('.mfn-live-css-editor .mfn-css-variables-list').sortable({
            axis: 'y',
            cursor: 'ns-resize',
            handle: '.mfn-sort-handle',
            opacity: 0.9,
            update: function(e, ui) {
                ...
                $('.mfn-live-css-editor .mfn-css-variables-list li').each(function(i, el) {
                    let title = $(el).find('.variable-title').text();
                    updated_vars.push(mfn['css_variables'].filter( (f) => f.title == title )[0]);
                });

                if( updated_vars.length == mfn['css_variables'].length ) {
                    mfn['css_variables'] = updated_vars;
                    setTimeout(function() {
                        mfn_live_css['update']['variables_ajax']();
                    },10);
                }else{
                    console.log('Something went wrong with sorting of variables.');
                }
    ...
    variables_list: function( arr = false ) {

        let css_html = '';
        let html = '';

        let array = arr ? arr : mfn.css_variables;

        if( array && array.length ) {
            array.map(function(cv) {
                html += `<li>
                  <a class="mfn-option-btn mfn-option-blank mfn-sort-handle" href="#"><span class="mfn-icon mfn-icon-sort"></span></a>
                  <span data-tooltip="Click to copy" data-position="right" class="variable-title">${cv.title}</span>
                  <span class="variable-sep">=</span>
                  <code class="variable-value ${cv.value.includes('#') ? 'variable-color' : ''}">${cv.value.includes('#') ? '<span style="background-color: '+cv.value+';"></span>' : ''} ${cv.value}</code>
                  <div>
                    <a class="mfn-option-btn mfn-option-blank mfn-css-var-action-edit" data-title="${cv.title}" href="#"><span class="mfn-icon mfn-icon-edit"></span></a>
                    <a class="mfn-option-btn mfn-option-blank mfn-css-var-action-delete" data-title="${cv.title}" href="#"><span class="mfn-icon mfn-icon-delete"></span></a>
                  </div>
                </li>`;
                css_html += cv.title+': '+cv.value+';';
            }).join('');
        }

        $('.mfn-live-css-editor ul.mfn-css-variables-list').html(html);
        $content.find('style.mfn-be-variables-custom-css').html(`body{${css_html}}`);

        if( !$('body style.mfn-css-be-variables').length ) $('body').append('<style class="mfn-css-be-variables"></style>');
        $('body style.mfn-css-be-variables').html(`body{${css_html}}`);

        mfn_live_css.sortable();
    },

    hide: function() {
        $('.mfn-ui').removeClass('mfn-live-css-editor-active');
        $('.mfn-live-css-editor').remove();
    }
}
...
$(document).on('focus', '.mfn-placeholder-inherited-class input[type="text"], .mfn-placeholder-inherited-std input[type="text"], .mfn-placeholder-inherited-rwd input[type="text"], .mfn-placeholder-inherited-class input[type="number"], .mfn-placeholder-inherited-std input[type="number"], .mfn-placeholder-inherited-rwd input[type="number"]', function() {
    $(this).attr('placeholder', $(this).val()).val('');
}).on('blur', '.mfn-placeholder-inherited-std input[type="text"], .mfn-placeholder-inherited-class input[type="text"], .mfn-placeholder-inherited-rwd input[type="text"], .mfn-placeholder-inherited-class input[type="number"], .mfn-placeholder-inherited-std input[type="number"], .mfn-placeholder-inherited-rwd input[type="number"]', function() {
    if( !$(this).val().length && typeof $(this).attr('placeholder') != 'undefined' ) $(this).val( $(this).attr('placeholder') );
});
...
var mfn_conditional_logic = {
    ...
    ui: function() {
        ...
        $(document).on('click', '.modal-conditional-logic .mfn-cl-remove', function(e) {
            ...
            let this_condition = $(this).closest('.mfn-cl-condition');
            let cl_rows = this_condition.find('.mfn-cl-row').length;
            ...
            if( cl_rows == 2 ) this_condition.addClass('mfn-cl-condition-single').removeClass('mfn-cl-condition-multiple');
    ...
    form: function(ver = false) {
        ...
        if( !mfn.is_woocommerce ) delete(mfn_conditional_logic.options.woocommerce);

        html += `<div class="mfn-cl-row">
            <div class="mfn-cl-row-col">
                <select class="mfn-form-control mfn-form-select mfn-form-select mfn-cl-row-input mfn-cl-row-input-type">
            ...
            <div class="mfn-cl-row-col mfn-cl-row-col-variable mfn-cl-row-col-is-or-no ${mfn_conditional_logic.fill && mfn_conditional_logic.fill.key.length ? 'mfn-cl-row-col-visible' : '' }">
                ${ _.map( mfn_conditional_logic.options, (opt, o) => 
                    _.map( opt.options, (field) => 
                        `<select class="mfn-form-control mfn-form-select mfn-cl-row-input mfn-cl-row-input-variable mfn-cl-row-input-is mfn-cl-row-input-${field.id} ${mfn_conditional_logic.fill && mfn_conditional_logic.fill.key == field.id ? 'mfn-cl-row-input-visible' : '' }">
                ...
                ${ _.map( mfn_conditional_logic.options, (opt, o) => `
                    ${ _.map( opt.options, function (field) { 
                        ...
                        if( field.input == 'text' ){
                            ...
                            field_html = `<select class="mfn-form-control mfn-form-select mfn-cl-row-input mfn-cl-row-input-value mfn-cl-row-input-variable mfn-cl-row-input-${field.id} ${mfn_conditional_logic.fill && mfn_conditional_logic.fill.key == field.id ? 'mfn-cl-row-input-visible' : '' }">
                                ${ _.map( field.options, (field_opt, f) =>
                                typeof field_opt == 'object' ? `<option ${mfn_conditional_logic.fill && mfn_conditional_logic.fill.value == field_opt.name ? 'selected' : '' } value="${field_opt.name}">${field_opt.label}</option>` : `<option ${mfn_conditional_logic.fill && mfn_conditional_logic.fill.value == f ? 'selected' : '' } value="${f}">${field_opt}</option>`
    ...
    options: {
        'singular': {
            label: 'Singular',
            options: [
                { id: 'post_type',          label: 'Post type',         input: 'select',            conditions: { 'is': 'is', 'isnt': 'is not'},       options: mfn.post_types },
        ...
        'archive': {
            label: 'Archive',
            options: [
                { id: 'post_type_archive',              label: 'Post type archive',         input: 'select',        conditions: { 'is': 'is', 'isnt': 'is not'},     options: mfn.post_types },
        ...
        'user': {
            ...
            options: [
                ...
                { id: 'user_role',              label: 'User role',                 input: 'select',        conditions: { 'is': 'is', 'isnt': 'is not'},        options: mfn.user_roles },
            ]
        },
        'woocommerce': {
            label: 'Woocommerce',
...
$editpanel.on('change', '.mfn-form-row .browse-icon .mfn-field-value, .mfn-form-row .browse-icon .field-to-object', function() {
    ...
    if( icon != '' ){
        ...
        }else if($editrow.hasClass('wrap tab_icon')){

            if( !edited_item.attr?.tab_title ) $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-title').text('');

            if( !$content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-icon').length ) {
                $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-wrapper').prepend('<div class="mfn-tabber-nav-li-icon" />')
            }else if( $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-icon img').length ){
                return false;
            }

            $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-icon').html('<i class="'+icon+'" aria-hidden="true"></i>');
        ...
        if( isBlocks() ){
          $content.find('.'+it+' .item-preview-icon i').removeAttr('class');
            ...
            if( !$content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-icon img').length ){
                $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-icon').remove();
            }

            $('.mfn-form-row.tab_title .mfn-field-value').trigger('change');
...
var presets = {
    ...
    set: function() {
        $editpanel.on('click', '.mfn-ui .mfn-presets-list a.mfn-load-preset', function(e) {
            ...
            var styles = mfn.presets.filter( (item) => item.uid == uid ).length ? JSON.parse( JSON.stringify(mfn.presets.filter( (item) => item.uid == uid )[0] )) : false;
            ...
            if( typeof presets_keys[edited_item.jsclass] !== 'undefined' && presets_keys[edited_item.jsclass].length ){
                $.each( presets_keys[edited_item.jsclass], function(i, el) {
                    if( $editpanel.find('.panel-edit-item .mfn-form-row.'+el+'.re_render').length ){
                        rerender = true;
                    }else if( $editpanel.find('.panel-edit-item .mfn-form-row.'+el+' .segmented-options').length ){
                        $editpanel.find('.panel-edit-item .mfn-form-row.'+el+' .segmented-options li.active a').trigger('click');
                    }else{
                        $editpanel.find('.panel-edit-item .mfn-form-row.'+el+' .mfn-field-value').trigger('change');
                    }
                });
            }
    ...
    show: function() {
        $('.mfn-ui:not(.mfn-editing-section, .mfn-editing-wrap) .mfn-presets-list').on('mouseenter', function() {
            ...
            if( mfn.presets.length ){
                var el_presets = mfn.presets.filter( (it) => it.item == edited_item.jsclass );
            ...
            $('.mfn-presets-list .dropdown-wrapper span.mfn-preset-remove').on('click', function() {
                ...
                $('.mfn-ui').addClass('mfn-modal-open').append('<div class="mfn-modal modal-confirm show"> <div class="mfn-modalbox mfn-form mfn-shadow-1"> <div class="modalbox-header"> <div class="options-group"> <div class="modalbox-title-group"> <span class="modalbox-icon mfn-icon-delete"></span> <div class="modalbox-desc"> <h4 class="modalbox-title">Delete preset</h4> </div></div></div><div class="options-group"> <a class="mfn-option-btn mfn-option-blank btn-large btn-modal-close" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a> </div></div><div class="modalbox-content"> <img class="icon" alt="" src="'+mfn.themepath+'/muffin-options/svg/warning.svg"> <h3>Delete preset?</h3> <p>Please confirm. There is no undo.</p><a class="mfn-btn mfn-btn-red btn-wide btn-modal-confirm" href="#"><span class="btn-wrapper">Delete</span></a> </div></div></div>');
                ...
                $('.btn-modal-confirm').on('click', function(e){
                    ...
                    $.ajax({
                        ...
                        success: function(response){
                            $a.parent('li').remove();
                            mfn.presets = mfn.presets.filter( (it) => it.uid != uid );
                            $('.mfn-ui').removeClass('mfn-modal-open');
                            $('.modal-confirm.show').remove();
                        }
                    });
    ...
    modal: function(uid) {
        $('.mfn-ui').addClass('mfn-modal-open').append('<div class="mfn-modal modal-confirm show"> <div class="mfn-modalbox mfn-form mfn-shadow-1"> <div class="modalbox-header"> <div class="options-group"> <div class="modalbox-title-group"> <span class="modalbox-icon mfn-icon-preset"></span> <div class="modalbox-desc"> <h4 class="modalbox-title">Add new preset</h4> </div></div></div><div class="options-group"> <a class="mfn-option-btn mfn-option-blank btn-large btn-modal-close" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a> </div></div><div class="modalbox-content"><div class="mfn-form-row"><input placeholder="Type preset name" type="text" class="mfn-form-control mfn-preset-name"></div><a class="mfn-btn mfn-btn-blue btn-modal-confirm" '+( mfn.view == 'demo' ? 'data-tooltip="Unavailable in Demo"' : '' )+' href="#"><span class="btn-wrapper">Save</span></a> <a class="mfn-btn btn-modal-close" href="#"><span class="btn-wrapper">Cancel</span></a> </div></div></div>');
...
sliderInput = {
    ...
    init: function($slider) {
        ...
        var $editbox = $slider.closest('.mfn-form-row');
        ...
        $slider.slider({
            ...
            slide: function(event, ui) {
                ...
                if( typeof $input.attr('data-pointobj') !== 'undefined' && $content.find('.'+it+' .mfn-hotspot-style-line .hotspot-image').length ){
                    ...
                    mfn_multifield_field.refresh(c_id, $input);
                }
    ...
    unitChange: function() {
        $editpanel.on('click', 'ul.mfn-slider-unit li a', function(e) {
            ...
            if(!$li.hasClass('active')){
                ...
                if($editbox.find('.mfn-sliderbar-value').val().length) $editbox.find('.mfn-sliderbar-value').trigger('change');
    ...
    search: function($field) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if( mfn.is_woocommerce && ( mfn.builder_type == 'single-product' || mfn.post_type == "product" ) && $field.attr('data-get') == 'products' ) {
                    response.product.options.push( {id: 'current-id', title: 'Current product'} );
                }

                if( $field.attr('data-get') == 'taxonomies' ) {
                    if(mfn.is_woocommerce) response.product_cat.options.push( {id: 'parent_taxonomy', title: 'Parent category'}, {id: 'any_taxonomy', title: 'Any category'} );
...
var select_ajax = {
  ...
  init: function() {
    ...
    $(document).on('keyup focus', '.mfn-select-ajax .mfn-select-ajax-input', function() {
        ...
        select_ajax.type = typeof $(this).attr('data-selecttype') != 'undefined' ? $(this).attr('data-selecttype') : 'posts';
        select_ajax.post_type = typeof $(this).attr('data-posttype') != 'undefined' && $(this).attr('data-posttype').length ? $(this).attr('data-posttype') : mfn.current_post_type;

        select_ajax.val = $(this).val().length ? $(this).val() : ''; 
  ...
  ajax: function() {
    ...
    $.ajax({
        ...
        data: {
          'mfn-builder-nonce': wpnonce,
          action: 'mfn_dynamic_get_items',
          type: select_ajax.type, // taxonomies, posts, postmeta
          post_type: select_ajax.post_type, // post type
          search: select_ajax.val,
        },
        type: 'POST',
        success: function(response) {

        $('.mfn-select-ajax .mfn-select-ajax-hints').remove();
          ...
          if( select_ajax.type == 'postmeta' ){

            response.map( item => {

                if( select_ajax.val.length && item.includes(select_ajax.val) ){
                    html += `<li data-id="${item}">${item}</li>`;
                }else if( !select_ajax.val.length ){
                    html += `<li data-id="${item}">${item}</li>`;
                }
              
            });

          }else if( _.has(response, 'page') && _.has(response.page, 'options') && response.page['options'].length ){

            response.page['options'].map( item => {
              html += `<li class="${item.id == select_ajax.val ? 'disabled' : ''}" data-id="${item.id}">${item.title}</li>`;
            });
  ...
  exit: function() {
    ...
    $(document).unbind('click', select_ajax.blur);
  }
...
var multiSelectField = {
    ...
    init: function() {
        ...
        $(document).on('focus', '.mfn-multiselect-field-wrapper .form-control .mfn-multiselect-input', function(e) {
          e.preventDefault();
        ...
        $(document).on('click', '.mfn-multiselect-field-wrapper .mfn-multiselect-options li:not(.selected)', function() {
            //e.preventDefault();
        ...
        $(document).on('click', '.mfn-ui .mfn-form .form-group.mfn-multiselect-field-wrapper .form-control span', function(e) {
            e.preventDefault();
            multiSelectField.remove($(this));
        });

        $(document).on('keyup paste change', '.mfn-ui .mfn-form .form-group.mfn-multiselect-field-wrapper .form-control .mfn-multiselect-input', function() {
...
var multiTextField = {
    init: function() {
        ...
        $editpanel.on('click', '.sidebar-add .added-sidebars .mfn-btn-delete', function(e) {
          ...
          var name = $(this).closest('.mfn-form-row').attr('data-name');
          var $input = $(this).siblings('input');

          if( typeof mfn.themeoptions[name][key] !== undefined ) delete(mfn.themeoptions[name][key]);
        ...
        $('.panel-edit-item .tabs-wrapper:not(.mfn-initialized)').each(function() {
            ...
            $(this).sortable({
                ...
                update: function(e, ui) {
                    ...
                    }else if($editbox.hasClass('tabber_content_order')) {

                        let tabs_backup = $edited_div.find('.section_wrapper').clone(true);
                        let new_tabs_nav = $('<ul class="mfn-tabber-nav" />');
                        let new_tabs_content = $('<ul class="mfn-tabber-nav" />');


                        $('.panel-edit-item .tabber_content_order .tabs-wrapper li').each(function() {
                            let uid = $(this).attr('data-uid');
                            let wrap_clone = $edited_div.find('.mcb-wrap-'+uid).clone(true);

                            $edited_div.find('.mcb-wrap-'+uid).remove();

                            $edited_div.find('.section_wrapper').append(wrap_clone);
                            new_tabs_nav.append( $edited_div.find('.mfn-tabber-nav .mfn-nav-tab-'+uid).clone(true) );
                        });

                        $edited_div.find('.mfn-tabber-nav').replaceWith(new_tabs_nav);
...
$editpanel.on('keyup change paste', '.filters_fields .filters.label .mfn-field-value', function() {
    let val = $(this).val();
    let hea = $(this).closest('.mfn-multifield-point').find('.mfn-multifield-point-tab-header h6');
    let index = $(this).closest('.mfn-multifield-point').index();

    if( val.length ){
        hea.text(val);
    }else{
        hea.text(`Filter ${index+1}`);
    }
...
$(document).on('keyup paste change', '.content-txt-edit .mfn-field-value', function() {
    fieldUpdate($(this));
});
...
$editpanel.on('keyup paste change', '.wrap.tab_title .mfn-field-value', function() {
    let val = $(this).val();

    $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-title').text(val);

    if( !val.length && !$content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-icon').length ) {
        $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-title').text('[ Fill the title or icon ]');
    }

});

$editpanel.on('keyup paste change', '.wrap.tab_desc .mfn-field-value', function() {
    let val = $(this).val();

    if( !$content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-desc').length ) {
        $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-desc').append('<span class="mfn-tabber-nav-desc" />')
    }
    $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-desc').text(val);
});
...
function updateColumnsRwd() {
    ...
    if( val_desktop > 0 ) {
        ...
        $editpanel.find('.mfn-form-row.query_item_default_width .mfn-field-value').val(percent_d+"%").trigger('change');
    }
...
$editpanel.on("click", '.mfn-modal .segmented-options.single-segmented-option li a, .sidebar-panel-content .panel:not(.panel-settings) .segmented-options.single-segmented-option li a', function(e) {
    e.preventDefault();

    let $li = $(this).closest('li');
    /*let is_unclickable = false;
    if( $li.closest('.segmented-options').hasClass('mfn-placeholder-inherited-class') ) is_unclickable = true;*/
    ...
    /*else{
        ...
        $li.find('input').prop('checked', false).trigger('change');
    }*/
    ...
    if( $(this).closest('.mfn-element-fields-wrapper').length && !val.length && $editbox.hasClass('mfn-placeholder-inherited-class') ) return;
    ...
    if( $editbox.hasClass('tabber_nav_position') ){

        if( screen == 'desktop' ){
            $content.find('.'+it).removeClass('mfn-tabber-nav-left mfn-tabber-nav-top mfn-tabber-nav-right mfn-tabber-nav-bottom');
            $content.find('.'+it).addClass('mfn-tabber-nav-'+val);
        }else{
            $content.find('.'+it).removeClass('mfn-tabber-nav-'+screen+'-left mfn-tabber-nav-'+screen+'-top mfn-tabber-nav-'+screen+'-right mfn-tabber-nav-'+screen+'-bottom');
            $content.find('.'+it).addClass('mfn-tabber-nav-'+screen+'-'+val);
        }
    }

    if( $editbox.hasClass('tabber_icon_position') ){

        if( screen == 'desktop' ){
            $content.find('.'+it).removeClass('mfn-tabber-icon-left mfn-tabber-icon-top mfn-tabber-icon-right mfn-tabber-icon-bottom mfn-tabber-icon-center-left mfn-tabber-icon-center-right');
            $content.find('.'+it).addClass('mfn-tabber-icon-'+val);
        }else{
            $content.find('.'+it).removeClass('mfn-tabber-icon-'+screen+'-left mfn-tabber-icon-'+screen+'-top mfn-tabber-icon-'+screen+'-right mfn-tabber-icon-'+screen+'-bottom mfn-tabber-icon-'+screen+'-center-right mfn-tabber-icon-'+screen+'-center-left');
            $content.find('.'+it).addClass('mfn-tabber-icon-'+screen+'-'+val);
        }
    }

    

    if( $editbox.hasClass('wrap grid') ) {
        if( val.length ){
            $content.find('.'+it).addClass('mcb-wrap-grid');
        }else{
            $content.find('.'+it).removeClass('mcb-wrap-grid');
        }
    }

    if( $editbox.hasClass('wrap') && $editbox.hasClass('grid_columns_switcher') ) {

        $content.find('.'+it).removeClass('mcb-wrap-grid-col-custom');

        if( val.length ){
            $content.find('.'+it).addClass('mcb-wrap-grid-col-custom');
        }

    }


    if( $editbox.hasClass('tabber_nav_width_switcher') ) {

        if( !val.length ){
            delete(edited_item.attr?.css_tabber_nav_width);
            loopAllStyleFields(edited_item.uid);
        }

    }

    if( $editbox.hasClass('tabber_nav_open_opt') ) {

        $content.find('.'+it).removeClass('mfn-tabber-open-on-click mfn-tabber-open-on-hover');

        if( val.length ){
            $content.find('.'+it).addClass('mfn-tabber-open-on-'+val);
        }else{
            $content.find('.'+it).addClass('mfn-tabber-open-on-click');
        }
    ...
    if( $editbox.hasClass('divider_2') && $editbox.hasClass('align') ) {


        if( screen == 'desktop' ){
            $content.find('.'+it+' .mfn-divider').removeClass('mfn-divider-start mfn-divider-center  mfn-divider-end');
        }else{
            $content.find('.'+it+' .mfn-divider').removeClass('mfn-divider-'+screen+'-start mfn-divider-'+screen+'-center  mfn-divider-'+screen+'-end');
        }

        if( val.length ){
            if( screen == 'desktop' ){
                $content.find('.'+it+' .mfn-divider').addClass('mfn-divider-'+val);
            }else{
                $content.find('.'+it+' .mfn-divider').addClass('mfn-divider-'+screen+'-'+val);
            }
        }else{
            if( screen == 'desktop' ){
                $content.find('.'+it+' .mfn-divider').addClass('mfn-divider-start');
            }else{
                $content.find('.'+it+' .mfn-divider').addClass('mfn-divider-'+screen+'-start');
            }
        }
    ...
    if( $editbox.hasClass('section type') ) {
        if( !val.length ) {
            ...
            $content.find('.'+it).removeClass('mfn-looped-items mfn-section-tabber');
            $('.mfn-element-fields-wrapper').removeClass('mfn-is-query-looped');
            $('.mfn-element-fields-wrapper').removeClass('mfn-section-is-tabber');
            ...
            if( mfn.preview_post_type.includes('archive-') ){
                $('.mfn-element-fields-wrapper .query_post_type .mfn-field-value').val(mfn.preview_post_type.replace('archive-', '')).trigger('change');
            }

            //re_render2( $content.find('.'+it).attr('data-uid'), 'section' );
        } else if( val == 'tabber' ) {
            $content.find('.'+it).addClass('mfn-section-tabber');
            $('.mfn-element-fields-wrapper .query_post_type .mfn-field-value').val('').trigger('change');
            $('.mfn-element-fields-wrapper .query_terms_taxonomy .mfn-field-value').val('').trigger('change');
            $('.mfn-element-fields-wrapper').removeClass('mfn-is-query-looped');
            $('.mfn-ui.mfn-editing-section .mfn-form .mfn-element-fields-wrapper').removeClass('mfn-is-query-looped');
    ...
    if( $editbox.hasClass('product_cart_button') && $editbox.hasClass('style') ){

        $content.find('.'+it+' .mfn-product-add-to-cart').removeClass(`mfn-add-to-cart-layout-inline-${screen} mfn-add-to-cart-layout-wrap-${screen}`);

        if( val.length ){
            $content.find('.'+it+' .mfn-product-add-to-cart').addClass(`mfn-add-to-cart-layout-${val}-${screen}`);
        }
    }
    ...
    if( $editbox.hasClass('product_star_rating_stats') ) {

        if( $content.find('.'+it+' ul.products').length ) {
            $content.find('.'+it+' ul.products').removeClass('mfn-stars-visible-rate');
            if( val.length ) $content.find('.'+it+' ul.products').addClass('mfn-stars-'+val);
        }else if( $content.find('.'+it+' .woocommerce-product-rating').length ) {
            $content.find('.'+it+' .woocommerce-product-rating').removeClass('mfn-stars-visible-rate');
            if( val.length ) $content.find('.'+it+' .woocommerce-product-rating').addClass('mfn-stars-'+val);
        }
    }
    ...
    if( $editbox.hasClass('filters') && $editbox.hasClass('form_layout') ) {

        $content.find('.'+it+' .mfn-advanced-filters').removeClass('mfn-advanced-filters-horizontal mfn-advanced-filters-vertical');

        if( val.length ) {
            $content.find('.'+it+' .mfn-advanced-filters').addClass('mfn-advanced-filters-'+val);
        }else{
            $content.find('.'+it+' .mfn-advanced-filters').addClass('mfn-advanced-filters-vertical');
        }

    }

    if( $editbox.hasClass('filters') && $editbox.hasClass('form_submit') ) {

        $content.find('.'+it+' .mfn-advanced-filters').removeClass('mfn-advanced-filters-auto-submit mfn-advanced-filters-ajax-submit');
    ...
    if( $editbox.hasClass('tabber_nav_items_hover_type') ) {

        if( val.length ){
            let li =  $content.find('.'+it+' ul.mfn-tabber-nav li:first-child');
            $content.find('.'+it+' ul.mfn-tabber-nav').addClass('mfn-tabber-nav-hover-sliding');
            $content.find('.'+it+' ul.mfn-tabber-nav').prepend('<span class="mfn-tabber-nav-slide-helper"></span>');
            $content.find('.'+it+' ul.mfn-tabber-nav .mfn-tabber-nav-slide-helper').css({ width: li.outerWidth(), height: li.outerHeight() });

            runAjaxElements();

        }else{
            $content.find('.'+it+' ul.mfn-tabber-nav').removeClass('mfn-tabber-nav-hover-sliding');
            $content.find('.'+it+' ul.mfn-tabber-nav .mfn-tabber-nav-slide-helper').remove();
        }
    } 


    if( $editbox.hasClass('product_brands') && $editbox.hasClass('logo_pos') ) {

        $content.find('.'+it+' .mfn-product-brands').removeClass('mfn-product-brands-logo-top mfn-product-brands-logo-left');

        if( val.length ) {
            $content.find('.'+it+' .mfn-product-brands').addClass('mfn-product-brands-logo-'+val);
        }else{
            $content.find('.'+it+' .mfn-product-brands').addClass('mfn-product-brands-logo-left');
        }

    }

    if( $editbox.hasClass('product_brands') && $editbox.hasClass('layout') ) {

        $content.find('.'+it+' .mfn-product-brands').removeClass('mfn-product-brands-horizontal mfn-product-brands-vertical');

        if( val.length ) {
            $content.find('.'+it+' .mfn-product-brands').addClass('mfn-product-brands-'+val);
        }else{
            $content.find('.'+it+' .mfn-product-brands').addClass('mfn-product-brands-horizontal');
        }

    }

    if( $editbox.hasClass('product_brands') && $editbox.hasClass('column_switcher') ) {

        $content.find('.'+it+' .mfn-product-brands').removeClass('mfn-product-brands-horizontal-custom-cols');

        if( val.length ) {
            $content.find('.'+it+' .mfn-product-brands').addClass('mfn-product-brands-horizontal-'+val+'-cols');
        }
    ...
    if( $editbox.hasClass('header_menu submenu_icon_display') ) {
        ...
        if( val == 'on' ) $content.find('.'+it+' .mfn-header-menu').removeClass('mfn-menu-submenu-icon-off');
    }
    ...
    if( $editbox.hasClass('filters filter_by') ) {
        sidebar_form.render();
    }
    ...
    if( $editbox.hasClass('product_images product_badge_onsale_position') ) {
        $content.find('.'+it+' .mfn-product-images-wrapper').removeClass('mfn-product-badges-left mfn-product-badges-right');
        if( val.length ) {
            $content.find('.'+it+' .mfn-product-images-wrapper').addClass('mfn-product-badges-right');
        }else{
            $content.find('.'+it+' .mfn-product-images-wrapper').addClass('mfn-product-badges-left');
        }
    }

    if( $editbox.hasClass('ql_badge_onsale_position') ) {
        $content.find('.'+it+' .mfn-img-box').removeClass('mfn-product-badges-left mfn-product-badges-right');
        if( val.length ) {
            $content.find('.'+it+' .mfn-img-box').addClass('mfn-product-badges-right');
        }else{
            $content.find('.'+it+' .mfn-img-box').addClass('mfn-product-badges-left');
        }
    }
    ...
    if( $editbox.hasClass('product_cart_button') && $editbox.hasClass('layout') ) {
        $content.find('.'+it+' .mfn-product-add-to-cart').removeClass(`mfn-add-to-cart-layout-wrap-${screen} mfn-add-to-cart-layout-inline-${screen}`);
        if( val.length ) $content.find('.'+it+' .mfn-product-add-to-cart').addClass(`mfn-add-to-cart-layout-${val}-${screen}`);
    }
    ...
    if( $editbox.hasClass('hotspot') && $editbox.hasClass('content_position') ) {
        ...
        if( screen == 'desktop' ){
            ...
            $content.find('.hotspot-point-'+hash).removeClass('hotspot-point-'+screen+'-bottom hotspot-point-'+screen+'-top hotspot-point-'+screen+'-left hotspot-point-'+screen+'-right').addClass('hotspot-point-'+screen+'-'+val);
        }
    }
    ...
    if($editbox.hasClass('icon_box_2 icon_position') || $editbox.hasClass('header_icon icon_position') || $editbox.hasClass('header_burger icon_position') ){
        if( screen == 'desktop' ){
            ...
            $content.find('.'+it+' .mfn-icon-box').removeClass('mfn-icon-box-'+screen+'-top mfn-icon-box-'+screen+'-bottom mfn-icon-box-'+screen+'-left mfn-icon-box-'+screen+'-right');
            val = screen+'-'+val;
        }
    ...
    if($editbox.hasClass('icon_box_2 icon_align') || $editbox.hasClass('header_icon icon_align')){
        if( screen == 'desktop' ){
            ...
            $content.find('.'+it+' .mfn-icon-box').removeClass('mfn-icon-box-'+screen+'-center mfn-icon-box-'+screen+'-start mfn-icon-box-'+screen+'-end');
            val = screen+'-'+val;
        }
        $content.find('.'+it+' .mfn-icon-box').addClass('mfn-icon-box-'+val);
    }
    ...
    if( $('.mfn-ui').hasClass('mfn-editing-wrap') && $editbox.hasClass('sticky') ){
        ...
        if( val == 1 ){
            ...
            $content.find('.'+it).removeClass('sticky');
...
$(document).on('keyup', '.multiple-inputs .field input', function() {
...
}).on('focus', '.multiple-inputs.separated-fields .field input:not(.readonly), .mfn-form-row.border-radius .multiple-inputs .field input:not(.readonly), .mfn-form-row.border-width .multiple-inputs .field input:not(.readonly)', function() {
    let input = $(this);
    let input_wrapper = input.closest('.field');
    let row = input.closest('.multiple-inputs');

    if( input_wrapper.hasClass('focused') ) return;

    if( row.find('.focused').length ) row.find('.focused').removeClass('focused');

    if( !row.hasClass('mfn-field-active') ) row.addClass('mfn-field-active');
    if( !input_wrapper.hasClass('focused') ) input_wrapper.addClass('focused');

}).on('change', '.multiple-inputs .field input:not(.readonly)', function() {
    var $input = $(this);
    var $editbox = $input.closest('.multiple-inputs');
    var $box = $input.closest('.mfn-form-row');
    var val = $input.val();
    ...
    if( val.length && $input.hasClass('numeral') ){
        $.each( units, function( i, el ) {
            if( val == 'initial' || val == 'auto' || val.includes(el) || val.includes('--') ){
                units_check = true;
                return;
            }
        });
    ...
    if( val.includes('--') && !val.includes('var(') && val != '--' ) {
        val = 'var('+val+')';
        $input.val( val );
    }
...
$editpanel.on('change', '#label_visibility.filters .mfn-field-value', function(e) {
    let it = $(this).closest('.mfn-element-fields-wrapper').attr('data-element');

    $content.find('.'+it+' .mfn-advanced-filters').removeClass('mfn-advanced-filters-labels-hidden');
    if( $(this).is(':checked') ) $content.find('.'+it+' .mfn-advanced-filters').addClass('mfn-advanced-filters-labels-hidden');
});
...
$editpanel.on('change', '.height_switcher .mfn-field-value', function(e) {
    ...
    if( !val.length && _.has(edited_item.attr, 'css_advanced_height') ) {
        delete( edited_item['attr']['css_advanced_height'] );
        loopAllStyleFields(edited_item.uid);
    }
...
$editpanel.on('change', '.width_switcher .mfn-field-value', function(e) {
    ...
    if( !val.length && _.has(edited_item.attr, 'css_advanced_flex') ) delete(edited_item['attr']['css_advanced_flex']);

    if( !val.length || val == 'inline' ){
        ...
        $('.mfn-ui .panel-edit-item .mfn-form .modalbox-card-advanced .advanced_flex .mfn-field-value').val('').trigger('change');

        delete(edited_item['attr']['css_advanced_flex']);
...
$editpanel.on('change', '.mfn-form-row.bgposopt select', function() {

    if( $(this).val() != 'custom' && _.has(edited_item['attr'], 'css_advanced_background_position_v2') ){
        delete(edited_item['attr']['css_advanced_background_position_v2']);
        sidebar_form.render();
        loopAllStyleFields(edited_item.uid);
    }
...
$editpanel.on('change', '.mfn-form-row.custombgsize select', function() {

    if( $(this).val() != 'custom' && _.has(edited_item['attr'], 'css_advanced_background_size_v2') ) {
        delete(edited_item['attr']['css_advanced_background_size_v2']);
        sidebar_form.render();
        loopAllStyleFields(edited_item.uid);
    }
...
$editpanel.on('change', '.banner_box.hover_effect .mfn-field-value', function() {
    ...
    $builder.find('.'+it+' .mfn-banner-box').removeClass('mfn-banner-box-image-toggle mfn-banner-box-image-zoom-in mfn-banner-box-image-zoom-out mfn-banner-box-image-blur mfn-banner-box-image-grayscale-10 mfn-banner-box-image-grayscale-01');
...
$editpanel.on('change', '.shape_divider_type_top, .shape_divider_type_bottom', function() {
    ...
    let key = 'svg';
    if( invertValSet && ( 'invert' in mfn.shape_dividers[inputValue] ) ) {
      key = 'invert';
    } else {
      invertValSet = 0;
    }
    ...
    if( 'viewbox' in mfn.shape_dividers[inputValue] ){
      viewbox = mfn.shape_dividers[inputValue]['viewbox'];
    }
    ...
    let pathVal = mfn.shape_dividers[inputValue][key].replace( regex, 'mfn-uid-'+ rand );
...
$editpanel.on('click', '.multiple-segmented-options.segmented-options ul li a', function(e) {
    ...
    if( $li.closest('.segmented-option').hasClass('mfn-placeholder-inherited-class') ) $li.closest('.segmented-option').removeClass('mfn-placeholder-inherited-class');
    if( $li.closest('.segmented-option').hasClass('mfn-placeholder-inherited-rwd') ) $li.closest('.segmented-option').removeClass('mfn-placeholder-inherited-rwd');
...
$editpanel.on('change', '.mfn-form-row.gradient .mfn-form-control', function() {
    let $editbox = $(this).closest('.mfn-form-row');
...
$(document).on('click', '.mfn-form-row .color-picker .form-control .color-picker-vb', function () {

    const this_input   = $(this);
    const $edit_field  = this_input.closest('.mfn-form-row');
    const $edit_wrapper = this_input.closest('.mfn-element-fields-wrapper');
    const element      = $edit_wrapper.attr('data-element');

    let css_style = $edit_field.attr('data-name');
    const css_path  = $edit_field.attr('data-csspath');

    if ($edit_field.hasClass('themeoption')) {
        css_style = $edit_field.attr('data-style');
    }

    if (this_input.val().length && this_input.val().includes('--')) return;

    const default_color = this_input.val().length ? this_input.val() : '#000';

    this_input.addClass('mfn-initialized');

    const inputElement = this_input[0]; // zamiast kombinacji z klasą
    ...
    .on('show', (color, instance) => {
        if (this_input.val().length && this_input.val().includes('#') && this_input.val() !== color.toHEXA().toString(0)) {
            instance.setColor(this_input.val());
            instance.show();
        }
        sidebar_form.no_render = true;
        $('.sidebar-wrapper').addClass('mfn-vb-sidebar-overlay');
    ...
    .on('save', (color, instance) => {
        instance.hide(); // ZAMIast mfnPickr.hide()
    })
    ...
    .on('change', (color, source, instance) => {
        ...
        let cl = color.toHEXA().toString(0);
        ...
        if ($edit_field.hasClass('fancy_divider color_top')) {
        ...
        $(inputElement)
            .closest('.color-picker-group')
            .find('.form-addon-prepend .label')
            .css({
                'border-color': cl,
                'background-color': cl
            })
            .removeClass('light dark')
            .addClass(getContrastYIQ(color.toHEXA().toString(0)));
    })

    .on('changestop', (source, instance) => {

        let cl = instance._color.toHEXA().toString(0);
        ...
        if (instance._color.a < 1) {
            cl = instance._color.toRGBA().toString(0).replaceAll(' ', '');
        }
    ...
    // jeśli chcesz mieć dostęp do instancji później:
    this_input.data('pickr', pickr);
});
...
$editpanel.on('change', '.mfn-form-row .color-picker-vb', function(e){
    e.preventDefault();
    var $field = $(this);
    var color = $field.val();
...
$editpanel.on('click', '.color-picker-clear', function(e){
    ...
    if( !$group.find('.color-picker-vb').val().length ) return
...
$(document).on('click', '.mfn-fr-help-icon', function(e) {
    e.preventDefault();
    if( $(this).closest('div').find('.desc-group').is(':visible') ){
        $(this).closest('div').find('.desc-group').slideUp(300);
    }else{
        $(this).closest('div').find('.desc-group').slideDown(300);
    }
});
...
function updatePseudoField($box) {
    ...
    $box.find('input.pseudo-field').val(is_empty ? '' : value).trigger('change');
...
// autosave
if( !mfn.autosave ) setInterval(autosave, 300000);
...
var copypaste = {
    ...
    copy_to_localstorage: function() {
        ...
        copypaste.ls_obj['obj'].push( mfn.pagedata.filter( (item) => item.uid == copypaste.uid )[0] );
        ...
        if( $content.find('.vb-item[data-uid="'+copypaste.uid+'"] .vb-item').length ) {
            $content.find('.vb-item[data-uid="'+copypaste.uid+'"] .vb-item').each(function() {
                var uid = $(this).attr('data-uid');
                copypaste.ls_obj['obj'].push( mfn.pagedata.filter( (item) => item.uid == uid )[0] );
            });
        }
    ...
    paste: function() {
        ...
        edited_item = mfn.pagedata.filter( (item) => item.uid == newuid )[0];
        ...
        setTimeout(function() {
            ...
            // re render tabber
            if( !$content.find('.vb-item[data-uid="'+newuid+'"]').hasClass('mfn-nested-wrap') && $content.find('.vb-item[data-uid="'+newuid+'"]').hasClass('mcb-wrap') && $content.find('.vb-item[data-uid="'+newuid+'"]').closest('.mfn-section-tabber').length ) re_render( $content.find('.vb-item[data-uid="'+newuid+'"]').closest('.mfn-section-tabber').attr('data-uid') );
...
function addLocalStyle(u, v, s, r, uid) {
    ...
    // console.log(u+' / '+s+' / '+v+' / '+r+' / '+uid);
    // console.log(addLocalStyle.caller);

    if( sidebar_form.is_class ){
        u = u.replaceAll('.mcb-section .mcb-wrap .be_', '.column.be_');
    }
    ...
    if( !uid.includes('be_') ) {
        ...
        selector_string = 'html '+selector_string;
    ...
    if( r == 'laptop' ) {
        $content.find('.mfn-be-local-css-laptop').append('<style class="mfn-local-style mfn-local-style-'+uid+' '+el+r+'">@media(max-width: 1440px){'+selector_string+' { '+(s.replace('_laptop', ''))+': '+v+' }}</style>');
    }else if( r == 'tablet' ) {
        $content.find('.mfn-be-local-css-tablet').append('<style class="mfn-local-style mfn-local-style-'+uid+' '+el+r+'">@media(max-width: 959px){'+selector_string+' { '+(s.replace('_tablet', ''))+': '+v+' }}</style>');
    }else if( r == 'mobile' ) {
        $content.find('.mfn-be-local-css-mobile').append('<style class="mfn-local-style mfn-local-style-'+uid+' '+el+r+'">@media(max-width: 767px){'+selector_string+' { '+(s.replace('_mobile', ''))+': '+v+' }}</style>');
    }else{
        $content.find('.mfn-be-local-css-desktop').append('<style class="mfn-local-style mfn-local-style-'+uid+' '+el+r+'">'+selector_string+' { '+s+': '+v+' }</style>');
    }
...
function changeInlineStyles(u, s, v){
    ...
    if( _.has(edited_item, 'uid') && u.includes('mfnuidelement') && !sidebar_form.is_class ) u = u.replaceAll('mfnuidelement', edited_item.uid);
    if( _.has(edited_item, 'uid') && edited_item.uid == 'pageoptions' && u.includes('postid') ) u = u.replaceAll('postid', mfn.pageid);
    if( _.has(edited_item, 'uid') && sidebar_form.is_class ) u = u.replaceAll('mcb-item-mfnuidelement', 'mcb-column.'+sidebar_form.is_class).replaceAll('mcb-wrap-mfnuidelement', 'mcb-wrap.'+sidebar_form.is_class).replaceAll('mcb-section-mfnuidelement', 'mcb-section.'+sidebar_form.is_class)
...
function imageForWidget(u, v, p){
    ...
    if( isBlocks() ){
      if( p != 'after' && p != 'hoverimg' ){
    ...
    }else if($content.find('.'+u).hasClass('column_zoom_box')){
        // zoom box desc
        if(v){
        ...
        // banner box
        $content.find('.'+u+' .banner-image img').replaceWith('<img src="'+v+'" alt="">');
...
var uploader = {
    ...
    browse: function() {
        $editpanel.on('click', '.panel .mfn-form .mfn-form-row .browse-image .mfn-button-upload', function(e) {
            e.preventDefault();
            var frame,
                addImgLink = $(this),
            ...
            if(multipleImgs && multipleImgs == 'add' && metaBox.find( '.upload-input' ).length) {
                ...
                frame.on( 'select', function() {
                    ...
                    if( addImgLink.closest('.dynamic_items_wrapper').length ){
                        ...
                        if( $modulebox.attr('data-id') == '_thumbnail_id' ){
                            imgIdInput.val( attachment.id ).trigger('change');
                            mfn.featured_image = attachment.url;
                            setTimeout(function() { imgContainer.html('<img src="'+attachment.url+'">') }, 200);
                        }else if( $modulebox.attr('data-id') == 'mfn-meta-seo-og-image' ){
    ...
    itemsUpdate: function(attachment, imgIdInput) {
        ...
        if( $modulebox.hasClass('themeoption logo-img') ){
            ...
            if(imgIdInput.hasClass('preview-bg_imageinput')){
            ...
            }else if($modulebox.hasClass('wrap') && $modulebox.hasClass('tab_image') ){
                
                if( !$content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-icon').length ) {
                    $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-wrapper').prepend('<div class="mfn-tabber-nav-li-icon" />')
                }

                $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-icon').html('<img src="'+attachment+'" alt="" />');
                ...
                imageForWidget(eluid, attachment);
            }else if($modulebox.hasClass('zoom_box') && $modulebox.hasClass('image') && imgIdInput.hasClass('preview-imageinput')){
                imageForWidget(eluid, attachment, 'main');
            }else if($modulebox.hasClass('zoom_box') && $modulebox.hasClass('content_image')){
    ...
    delete: function() {
        $editpanel.on('click', '.mfn-form .mfn-form-row .browse-image .mfn-button-delete', function(e) {
            e.preventDefault();
            ...
            if(multipleImgs == 'add'){
                ...
                if( $modulebox.hasClass('themeoption logo-img') ){
                    ...
                    if( $content.find('#Top_bar .logo img.logo-main').length ) {
                        $content.find('#Top_bar .logo img.logo-main').attr('src', mfn.themepath+'/images/logo/logo.png').removeClass('svg');
                    }else{
                        $content.find('#Header_creative .logo img.logo-main').attr('src', mfn.themepath+'/images/logo/logo.png').removeClass('svg');
                    }

                }else if( $modulebox.hasClass('themeoption retina-logo-img') ){

                    if( $content.find('#Top_bar .logo img.logo-mobile').length ) {
                        $content.find('#Top_bar .logo img.logo-mobile').attr('src', mfn.themepath+'/images/logo/logo-retina.png');
                    }else if($content.find('#Header_creative .logo img.logo-mobile').length) {
                        $content.find('#Header_creative .logo img.logo-mobile').attr('src', mfn.themepath+'/images/logo/logo-retina.png');
                    }

                }else if( $modulebox.hasClass('themeoption sticky-logo-img') ){

                    if( $content.find('#Top_bar .logo img.logo-sticky').length ) {
                        $content.find('#Top_bar .logo img.logo-sticky').attr('src', mfn.themepath+'/images/logo/logo.png');
                    }else if($content.find('#Header_creative .logo img.logo-sticky').length) {
                        $content.find('#Header_creative .logo img.logo-sticky').attr('src', mfn.themepath+'/images/logo/logo.png');
                    }
                }else if( $modulebox.hasClass('themeoption gdpr-content-image') ){

                    if( $content.find('.mfn-gdpr-image img').length ) {
                        $content.find('.mfn-gdpr-image img').attr('src', mfn.themepath+'/images/cookies.png');
                    }
                    ...
                    if(imgIdInput.hasClass('preview-bg_imageinput')){
                        ...
                        if( $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-icon img').length ) {
                            $content.find('.mfn-nav-tab-'+edited_item.uid+' .mfn-tabber-nav-li-wrapper .mfn-tabber-nav-li-icon img').remove();
                        }

                        $('.mfn-field-icon.wrap.tab_icon .mfn-field-value').trigger('change');
    ...
    deleteAllGallery: function() {
        $editpanel.on('click', '.mfn-form .mfn-form-row .browse-image .mfn-button-delete-all', function(e) {
            e.preventDefault();
...
function re_render( uid = false ) {
    ...
    if( uid && uid.includes('be_') ) return; // class editor will be reloaded upon class update

    if( !uid ) {
        uid = edited_item.uid;
    }else {
        var looped_obj = mfn.pagedata.filter( (item) => item.uid == uid )[0];
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if( sidebar_form.force_open ){
                $content.find('.vb-item[data-uid="'+sidebar_form.force_open+'"]').find('.mfn-header').first().find('.mfn-element-edit').trigger('click');
                sidebar_form.force_open = false;
            }
...
var sidebar_form = {
    ...
    is_class: false,
    is_pageoptions: false,
    is_themeoptions: false,
    ...
    before: function() {
        ...
        if( sidebar_form.wyswig_active && !copypaste.uid ) {
            if( $editpanel.find( '.mfn-form .html-editor' ).length ) MfnFieldTextarea.destroy();
            if( $editpanel.find( '.mfn-form .visual-editor' ).length ) tinymce.execCommand( 'mceRemoveEditor', false, 'mfn-editor' );
        }

        if(sidebar_form.wyswig_active) sidebar_form.wyswig_active = false;
    },

    open: function($edited_div, scroll = false) {

        $('style.mfn-builder-fields-css').remove();
        $('.mfn_classes_footer').remove();

        if( $('.mfn-ui').hasClass('mfn-reset-margin-placeholders') ) $('.mfn-ui').removeClass('mfn-reset-margin-placeholders');

        sidebar_form.is_pageoptions = false;
        sidebar_form.is_themeoptions = false;
        // sidebar_form.open

        // if( mfn_classes.has_changed ) setTimeout(mfn_classes.ajax(mfn_classes.has_changed), 10);

        if( sidebar_form.wyswig_active ) {
            // destroy editors has to be here and in before
            if( $editpanel.find( '.mfn-form .html-editor' ).length ) MfnFieldTextarea.destroy();
            if( $editpanel.find( '.mfn-form .visual-editor' ).length ) tinymce.execCommand( 'mceRemoveEditor', false, 'mfn-editor' );

            if( $content.find('.mcb-item-'+edited_item['uid']).length && $content.find('.mcb-item-'+edited_item['uid']).closest('.mfn-looped-items').length ) {
                re_render( $content.find('.mcb-item-'+edited_item['uid']).closest('.vb-item.mcb-section').attr('data-uid') );
            }else if($content.find('.mcb-item-'+edited_item['uid']).length){
                re_render( edited_item['uid'] );
            }
        }
        ...
        // console.log(sidebar_form.open.caller);

        if( $content.find('.vb-item.mfn-current-editing').length ) $content.find('.vb-item.mfn-current-editing').removeClass('mfn-current-editing');

        sidebar_form.edited_div = $edited_div;

        sidebar_form.edited_div.addClass('mfn-current-editing');

        if( !sidebar_form.is_class ){
            var id = sidebar_form.edited_div.attr('data-uid');
            edited_item = mfn.pagedata.filter( (item) => item.uid == id )[0];
        }

        // Global Wraps -> BeSections, PBL, Prevent opening edit form for sections
        if ( $('body').hasClass('mfn-template-wrap') && sidebar_form.edited_div.hasClass('section') ) return;

        $('.mfn-ui').removeClass('modalbox-card-active-content modalbox-card-active-style modalbox-card-active-advanced');
        $('.mfn-element-fields-wrapper').removeClass('mfn-section-is-tabber');
        $('.mfn-ui').removeClass('mfn-is-tabber');


        $('.sidebar-panel-classes-list  .sidebar-panel-class').remove();

        // Global Wraps -> BeWraps, PBL, hide all elements except for global wraps
        if( sidebar_form.edited_div.hasClass('mfn-global-wrap') ) {
            $('.sidebar-panel').addClass('mfn-global-wrap-edit');
            if( _.has(edited_item, 'attr') && _.has(edited_item['attr'], 'global_wraps_select') && typeof edited_item['attr']['global_wraps_select'] == 'number' ) edited_item['attr']['global_wraps_select'] = edited_item['attr']['global_wraps_select'].toString();
        } else {
        ...
        sidebar_form.type = 'item';

        if( edited_item.jsclass == 'section' ) {
            sidebar_form.type = 'section';
        }else if(edited_item.jsclass == 'wrap') {
            sidebar_form.type = 'wrap';
            if( _.has(edited_item, 'item_is_wrap') || sidebar_form.edited_div.hasClass('mfn-nested-wrap') ) $('.mfn-ui').addClass('mfn-editing-nested-wrap');
        }
        ...
        if( edited_item.jsclass != 'wrap' && edited_item.jsclass != 'section' ) {
            ...
            if( sidebar_form.edited_div.closest('.mcb-wrap').hasClass('mcb-wrap-grid') ) $('.mfn-ui').addClass('mfn-reset-margin-placeholders');
        }

        $('.mfn-ui .panel-edit-item .mfn-form').html(`<div class="mfn-element-fields-wrapper" data-item="${edited_item.jsclass}" data-element="${ sidebar_form.is_class ? edited_item.uid : `mcb-${sidebar_form.type}-${edited_item.uid}` }"><ul class="mfn-form-row sidebar-panel-content-tabs"></ul>`);
        ...
        mfn.current_post_type = _.has(edited_item, 'attr') && _.has( edited_item['attr'], 'query_post_type' ) ? edited_item['attr']['query_post_type'] : 'post';
        mfn.current_tax = _.has(edited_item, 'attr') && _.has( edited_item['attr'], 'query_terms_taxonomy' ) ? edited_item['attr']['query_terms_taxonomy'] : 'category';

        if( edited_item.jsclass == 'filters' ){
            mfn.current_post_type =
              edited_item?.attr?.post_type
              ?? (mfn.builder_type?.startsWith('archive-')
                    ? mfn.builder_type.slice(8)
                    : 'post');
        }
        ...
        if( edited_item?.uid && $content.find('.vb-item[data-uid="'+edited_item.uid+'"]').hasClass('mfn-tabber-content') && $content.find('.mfn-section-tabber .mfn-tabber-nav li.mfn-nav-tab-'+edited_item.uid+':not(.active)').length ) {
            $content.find('.mfn-section-tabber .mfn-tabber-nav li.mfn-nav-tab-'+edited_item.uid).addClass('active').siblings().removeClass('active');
            $content.find('.vb-item[data-uid="'+edited_item.uid+'"]').addClass('active').siblings().removeClass('active');
        }
        ...
        if( !sidebar_form.is_class ) {
            ...
            if( !$content.find('body').hasClass('mfn-class-editor-preview') ) $content.find('body').addClass('mfn-class-editor-preview');
            if( !$('.mfn-ui').hasClass('mfn-class-editor') ) $('.mfn-ui').addClass('mfn-class-editor');
            if($('.mfn-ui').hasClass('mfn-class-editor-full')) $('.mfn-ui').removeClass('mfn-class-editor-full');
            if( !$('.mfn-ui .panel-edit-item .mfn-form .modalbox-card-content').hasClass('mfn-class-editor-card-visible') ) {
                $('.mfn-ui .panel-edit-item .mfn-form .spct-li-content').remove();
                $('.mfn-ui .panel-edit-item .mfn-form .modalbox-card-content').remove();
            }
            if( edited_item.jsclass == 'section' || edited_item.jsclass == 'wrap' ) {
                $('.mfn-ui .panel-edit-item .mfn-form .spct-li-style').remove();
                $('.mfn-ui .panel-edit-item .mfn-form .modalbox-card-style').remove();
            }
            
            $('.mfn-ui .panel-edit-item .mfn-form').append(`<div class="mfn_classes_footer"><a href="#" class="mfn-btn mfn-class-editor-dismiss">Cancel</a><a href="#" class="mfn-btn mfn-btn-green mfn-class-editor-update">Update style</a></div>`);

            mfn_classes.backup = JSON.parse( JSON.stringify(edited_item) );
        }

        $('.mfn-ui .panel-edit-item .mfn-form .sidebar-panel-content-tabs li:first-child').trigger('click');
        ...
        $('.mfn-visualbuilder .sidebar-panel .sidebar-panel-header .header-edit-item .title-group .sidebar-panel-icon').attr('class', 'sidebar-panel-icon mfn-icon-'+( !sidebar_form.is_class ? edited_item.icon : 'global-styles-list'));
        ...
        if( !sidebar_form.is_class && sidebar_form.edited_div.length && scroll && builder_type !== 'header' ) {
            $content.find('html, body').animate({ scrollTop: sidebar_form.edited_div.offset().top - 50 }, 1000);
        }
        ...
        setTimeout(function() {
            $('body').append('<style class="mfn-builder-fields-css mfn-fields-switcher-normal-hover">.mfn-ui .mfn-form .mfn-sft-nav li:first-child a{background: var(--mfn-ui-sft-nav-tab-bg-active); color: var(--mfn-ui-sft-nav-tab-color-active);} .mfn-ui .mfn-form .mfn-sft.mfn-sft-normal{display: block;}</style>');
    ...
    render: function() {
        ...
        if( sidebar_form.no_render ) return;

        // console.log('sidebar re render');

        sidebar_form.before();

        if( $('.mfn-ui .panel-edit-item .mfn-form .mfn-element-fields-wrapper .modalbox-card').length ) $('.mfn-ui .panel-edit-item .mfn-form .mfn-element-fields-wrapper .modalbox-card').remove();

        if( !sidebar_form.is_class ) {

            if( $('.mfn-element-fields-wrapper .mfn-classes-used-info') ) $('.mfn-element-fields-wrapper .mfn-classes-used-info').remove();

            if( _.has(edited_item, 'be_classes') && mfn.classes.length && !mfn.classes.filter( cl => cl.uid == edited_item.be_classes[0] ).length ) delete( edited_item.be_classes );
            if( $('.mfn-ui .panel-edit-item .mfn-form .sidebar-panel-classes').length ) $('.mfn-ui .panel-edit-item .mfn-form .sidebar-panel-classes').remove();

            if( edited_item.jsclass != 'placeholder' && !sidebar_form.edited_div.hasClass('mfn-global-wrap') ) $('.mfn-ui .panel-edit-item .mfn-form').prepend('<div class="sidebar-panel-classes mfn-autocomplete"><label class="form-label form-label-wrapper">Global styles<a class="mfn-option-btn mfn-option-blank mfn-fr-help-icon" target="_blank" data-tooltip="Toggle description" data-position="left" href="#"><span class="mfn-icon mfn-icon-desc"></span></a><a href="#" class="sidebar-panel-class-create mfn-option-btn mfn-option-blank mfn-vb-label-button" title="Add new global style" data-tooltip="Add new global style" data-position="left"><span class="mfn-icon mfn-icon-add"></span></a><a href="#" class="sidebar-panel-class-list mfn-option-btn mfn-option-blank mfn-vb-label-button" title="Show list" data-tooltip="Show list" data-position="left"><span class="mfn-icon mfn-icon-global-styles-list"></span></a></label><div class="desc-group"><span class="description">Save the current styles so you can use them later anywhere. Check a <a href="https://www.youtube.com/watch?v=SO1EfBu24Oo" rel="nofollow" target="_blank">quick introduction</a> or watch a <a href="https://www.youtube.com/watch?v=bN1gSP009Vc" rel="nofollow" target="_blank">detailed tutorial</a>.</span></div><div class="sidebar-panel-classes-list mfn-autocomplete-list"><a class="mfn-option-btn mfn-sidebar-classes-show-list" href="#"><span class="mfn-icon mfn-icon-classes"></span></a><input data-list="mfn-classes" class="sidebar-panel-classes-el sidebar-panel-classes-input mfn-autocomplete-input" placeholder="Choose style"></div></div>');

            if( _.has(edited_item, 'be_classes') && edited_item.be_classes.length && mfn.classes.length && mfn.classes.filter( cl => cl.uid == edited_item.be_classes[0] ) ) {
                $('.mfn-ui').addClass('mfn-class-editor-full');
            }else{
                $('.mfn-ui').removeClass('mfn-class-editor-full');
            }

            if( _.has(edited_item, 'be_classes') && edited_item.be_classes.length && mfn.classes.length && !sidebar_form.edited_div.hasClass('mfn-global-wrap') ) {
                let classes_html = '';
                
                _.map( edited_item.be_classes, function(clas, c) {

                    let nm = mfn.classes.filter( (c) => c.uid == clas );
                    
                    if( nm.length ) {
                        classes_html += `<span data-name="${clas}" class="sidebar-panel-class mfn-autocomplete-item">${ nm[0]['title'] }<span class="sidebar-panel-class-edit mfn-autocomplete-item-edit">&#9998;</span> <span class="sidebar-panel-class-delete mfn-autocomplete-item-delete">&#10005;</span></span>`;
                    }else{
                        edited_item.be_classes.splice(c, 1);
                    }

                }).join('');
                    
                $('.sidebar-panel-classes .sidebar-panel-classes-input').before(classes_html);
            }

            if( mfn.classes.length && _.has(edited_item, 'be_classes') && edited_item.be_classes.length && !$('.mfn-ui .panel-edit-item .mfn-form .mfn-element-fields-wrapper .mfn-classes-used-info').length && mfn.classes.filter( (cl) => cl.uid == edited_item.be_classes[0]).length ){
                $('.mfn-ui .panel-edit-item .mfn-form .mfn-element-fields-wrapper').append('<div class="mfn-form-row mfn-alert mfn-classes-used-info placeholder"><div class="alert-icon mfn-icon-information"></div> <div class="alert-content"><p>This element uses global style. Any local changes will overwrite the global styles.</p></div> </div>');
            }

        }


        if( builder_type == 'archive-product' && edited_item.jsclass == 'filters' && !$('.mfn-ui .panel-edit-item .mfn-form .mfn-element-fields-wrapper .mfn-shop-products-alert').length ){
            $('.mfn-ui .panel-edit-item .mfn-form .mfn-element-fields-wrapper').append('<div class="mfn-form-row mfn-alert mfn-shop-products-alert"><div class="alert-icon mfn-icon-information"></div> <div class="alert-content"><p>This element works with Shop Products element and Query Loops. It does not work with Shop element.</p></div> </div>');
        }
        ...
        if( sidebar_form.edited_div && !sidebar_form.edited_div.hasClass('mfn-looped-items') && (sidebar_form.edited_div.closest('.mfn-looped-items').length || sidebar_form.edited_div.find('.mfn-looped-items').length) ) {
            ...
            $('.mfn-element-fields-wrapper .wrap.type.re_render').addClass('mfn-disabled-field');

            if( edited_item?.attr?.type == 'tabber' ){
                $('.mfn-element-fields-wrapper .section.type.re_render li:nth-child(2)').remove();
            }else{
                $('.mfn-element-fields-wrapper .section.type.re_render').addClass('mfn-disabled-field');
            }

        }

         if( edited_item.jsclass == 'wrap' && !_.has(edited_item, 'item_is_wrap') && sidebar_form.edited_div.closest('.mcb-section').hasClass('mfn-section-tabber') ) {
            $('.mfn-ui').addClass('mfn-is-tabber');
         }

         if( edited_item.jsclass == 'section' && typeof edited_item['attr']['type'] !== 'undefined' && edited_item['attr']['type'] == 'tabber' ) {
            $('.mfn-element-fields-wrapper').addClass('mfn-section-is-tabber');
        }
        ...
        /*if( sidebar_form.edited_div && sidebar_form.edited_div.find('.mfn-nested-wrap').length ) {
            $('.mfn-loop-switcher.type ul li:first-child a').trigger('click');
            $('.mfn-form-row.mfn-loop-switcher').addClass('mfn-disabled-field');
        }*/

        if( edited_item.jsclass != 'placeholder' && $('.mfn-ui').hasClass('mfn-editing-element') && !$('.mfn-element-fields-wrapper .modalbox-card-advanced').length ) {
        ...
        if( sidebar_form.focus ) {
            $('.sidebar-panel-content .mfn-form').find(sidebar_form.focus).focus();
            sidebar_form.focus = false;
        }

        sidebar_form.after();
        mfnoptsinputs.start();

        //console.log( mfn.placeholders );
    },
    ...
    after() {
        ...
        * */
        ...
        if( edited_item.jsclass == 'wrap' || edited_item.jsclass == 'section' ){
            ...
            if( typeof edited_item['attr']['type'] === 'undefined' || edited_item['attr']['type'] == '' || edited_item['attr']['type'] == 'tabber' ) {
        ...
        if( builder_type.includes('archive-') && (edited_item.jsclass == 'section' || edited_item.jsclass == 'wrap') && _.has(edited_item['attr'], 'type') && edited_item['attr']['type'] == 'query' ) {
            let ql_posts = builder_type.replace('archive-', '');
            if( $('.panel-edit-item .mfn-form .mfn-form-row.query_post_type .mfn-field-value').val() != ql_posts ){
                $('.panel-edit-item .mfn-form .mfn-form-row.query_type .mfn-field-value').val('posts').trigger('change');
                $('.panel-edit-item .mfn-form .mfn-form-row.query_post_type .mfn-field-value').val(ql_posts).trigger('change');
            }
        }
        ...
        * */
        ...
        if( $('.panel-edit-item .mfn-element-fields-wrapper .mfn-form-row.mfn-deprecated').length ){
            if( !$('.panel-edit-item .mfn-element-fields-wrapper .mfn-form-row.mfn-deprecated .mfn-field-value').length ) $('.panel-edit-item .mfn-element-fields-wrapper .mfn-form-row.mfn-deprecated').remove();
        }

        //$('.panel-edit-item .mfn-element-fields-wrapper .mfn-form-row .checkboxes.pseudo .mfn-deprecated:not(.active)').remove();
        ...
        * */
        ...
        if( $('.mfn-form .mfn-element-fields-wrapper .mfn-form-row.typography .mfn-toggle-fields-wrapper').is(':visible') ){
            $('.mfn-form .mfn-element-fields-wrapper .mfn-form-row.typography').each(function() {
                initFontSelect($(this));
            });
        }
        ...
        if( $( '.mfn-form .mfn-element-fields-wrapper .html-editor' ).length ) {
            sidebar_form.wyswig_active = true;
            preventEdit = true;
            MfnFieldTextarea.create();
            //$(document).trigger('mfn:vb:edit');
        }

        if( $( '.mfn-form .mfn-element-fields-wrapper .visual-editor' ).length ) {
            sidebar_form.wyswig_active = true;
            preventEdit = true;
            MfnFieldVisual.init();
        }
...
let inputDrag = {
    ...
    init: function () {
        $(document).on( 'mousedown', '.mfn-slider-input .mfn-form-input', function(e) {
            inputDrag.i = $(this);
            if( !inputDrag.i.val().includes('--') ) inputDrag.start(e);
        });
...
var settings = {
    ...
    start: function(){
        ...
        if( $("body").hasClass('mfn-default-ui') ){
        ...
        if( $("body").hasClass('mfn-presets-enabled') ){
          $('.mfn-ui .panel-settings .mfn-form .settings .form-control[data-option="enable-presets"] li:last-child').addClass('active')
            .siblings().removeClass('active');
        }
    ...
    run: function() {
        $('.mfn-ui .panel-settings .single-segmented-option.segmented-options ul li a').on('click', function(e) {
            ...
            if( $row.hasClass('mfn-reload-required') ){
                $('.mfn-ui').addClass('mfn-modal-open').append('<div class="mfn-modal modal-confirm mfn-modal-600 show"> <div class="mfn-modalbox mfn-form mfn-shadow-1"> <div class="modalbox-header"> <div class="options-group"> <div class="modalbox-title-group"> <span class="modalbox-icon mfn-icon-settings"></span> <div class="modalbox-desc"> <h4 class="modalbox-title">Confirmation box</h4> </div></div></div><div class="options-group"> <a class="mfn-option-btn mfn-option-blank btn-large btn-modal-close" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a> </div></div><div class="modalbox-content"> <img class="icon" alt="" src="'+mfn.themepath+'/muffin-options/svg/warning.svg"> <h3>Page reload is required</h3> <p>Continue?</p><a class="mfn-btn mfn-btn-red btn-wide btn-modal-close" href="#"><span class="btn-wrapper">Cancel</span></a> <a class="mfn-btn btn-wide btn-modal-confirm" href="#"><span class="btn-wrapper">Reload</span></a> <a class="mfn-btn mfn-btn-green btn-wide btn-modal-confirm-with-save" href="#"><span class="btn-wrapper">Save & Reload</span></a> </div></div></div>');
    ...
    continue: function() {
        ...
        var option = settings.option.closest('.form-control').attr('data-option'),
            ...
            }else if( option == 'enable-presets' ){

                $("body").removeClass('mfn-presets-enabled');

                if( value == 'enable' ) $("body").addClass('mfn-presets-enabled');
    ...
    detectOsTheme: function() {
        $('body').removeClass('mfn-ui-dark mfn-ui-light');
...
const GlobalWraps = {
    ...
    convertToGlobal: function(uid) {
        ...
        $('.mfn-ui').addClass('mfn-modal-open').append('<div class="mfn-modal modal-confirm show"> <div class="mfn-modalbox mfn-form mfn-shadow-1"> <div class="modalbox-header"> <div class="options-group"> <div class="modalbox-title-group"> <span class="modalbox-icon mfn-icon-preset"></span> <div class="modalbox-desc"> <h4 class="modalbox-title">Convert to Global Wrap</h4> </div></div></div><div class="options-group"> <a class="mfn-option-btn mfn-option-blank btn-large btn-modal-close" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a> </div></div><div class="modalbox-content"><div class="mfn-form-row"><input placeholder="Name it" type="text" class="mfn-form-control mfn-global-section-name"></div><a class="mfn-btn mfn-btn-blue btn-modal-confirm" '+( mfn.view == 'demo' ? 'data-tooltip="Unavailable in Demo"' : '' )+' href="#"><span class="btn-wrapper">Save</span></a> <a class="mfn-btn btn-modal-close" href="#"><span class="btn-wrapper">Cancel</span></a> </div></div></div>');
        ...
        $('.btn-modal-confirm').on('click', function(e){
            ...
            if( $('.mfn-global-section-name').val() != '' ){
                ...
                prepareForm.save = true;
                prepareForm.is_global_wrap = true;
                ...
                success: function(response) {
                    ...
                    if( typeof mfn['global_wraps'] !== 'undefined' && mfn['global_wraps'].length < 2 ) {
                        mfn['global_wraps'] = { '0': '- Default -' };
                    }

                    mfn['global_wraps'][response.key] = response.title;
                    ...
                    if( mfn.pagedata.filter((x) => x.uid == GlobalWraps.selectedUid).length ) {
                        mfn.pagedata.filter((x) => x.uid == GlobalWraps.selectedUid)[0]['attr']['global_wraps_select'] = response.key;

                        const wrap_helpr = mfn.pagedata.filter((x) => x.uid == GlobalWraps.selectedUid)[0];

                        if( wrap_helpr.attr ) {
                            Object.keys(wrap_helpr.attr).forEach(key => {
                              if (key.startsWith('css_')) {
                                delete wrap_helpr.attr[key];
                              }
                            });
                        }
    ...
    getGlobalWrap: function () {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if( $builder.find('.mfn-current-editing-global').length ) $builder.find('.mfn-current-editing-global').removeClass('mfn-current-editing-global');

                that.wrapTriggered.replaceWith(response.html);
                $builder.find('.mfn-current-editing-global').attr('data-uid', that.selectedUid);
                ...
                setTimeout(function() {
                    ...
                    sidebar_form.open($builder.find('.mfn-current-editing-global'), false);
                    GlobalWraps.selectedUid = false;
                }, 100);


                /*let retrieve_uid = false;

                if( $content.find('.mfn-current-editing').length ) $content.find('.mfn-current-editing').removeClass('mfn-current-editing');
                if( $content.find('.mfn-current-editing-global').length ) $content.find('.mfn-current-editing-global').removeClass('mfn-current-editing-global'); */

                // Imported content, insert it
                //that.wrapTriggered.replaceWith(response.html);

                //if( !$builder.find('.mfn-global-wrap.vb-item[data-uid="'+tmp_uid+'"]').length ) mfn.pagedata = mfn.pagedata.filter((i) => i.uid != tmp_uid );

                /*response.form.forEach(it => {
                  if( _.has(it, 'jsclass') && it.jsclass == 'wrap' ){
                    if( !mfn.pagedata.filter( (x) => x.uid == it.uid ).length ) {
                        mfn.pagedata.push(it);
                    }else{
                        mfn.pagedata.filter( (x) => x.uid == it.uid )[0] = it;
                    }
                    console.log(it);
                    retrieve_uid = it.uid;
                  }
                ...
                setTimeout(function() {
                    console.log(retrieve_uid);
                    if( $('body').hasClass('mfn-navigator-active') ) be_navigator.show( $(response.html).attr('data-uid') ); 
                    sidebar_form.open($builder.find('.mcb-wrap[data-mfn-global="'+that.selectedWrapTemplate+'"]'), false);
                    //$builder.find('.mfn-current-editing .mcb-wrap-inner').html($(response.html).find('.mcb-wrap-inner').html());
                    GlobalWraps.selectedUid = false;
                }, 100);*/
    ...
    resetWrapToDefaults: function() {
        ...
        queueMicrotask(() => sidebar_form.open( this.wrapTriggered ));
    },
}

$(document).on('change', '.global_wraps_select .mfn-field-value', function() {
    ...
    GlobalWraps.selectedUid = edited_item.uid;
    ...
    GlobalWraps.selectedWrapTemplate = $(this).val();
    GlobalWraps.wrapTriggered = $content.find(`.mfn-current-editing`).length ? $content.find(`.mfn-current-editing`) : $content.find(`.vb-item[data-uid="${edited_item.uid}"]`);
...
const GlobalSections = {
        ...
        $('.btn-modal-confirm').on('click', function(e){
            ...
            if( $('.mfn-global-section-name').val() != '' ) {
                ...
                success: function(response){
                    ...
                    if( typeof mfn['global_sections'] !== 'undefined' && mfn['global_sections'].length < 2 ){
                        mfn['global_sections'] = { '0': '- Default -' };
                    }

                    mfn['global_sections'][response.key] = response.title;
                    ...
                    if( mfn.pagedata.filter((x) => x.uid == GlobalSections.selectedUid).length ) {

                        const section_helpr = mfn.pagedata.filter((x) => x.uid == GlobalSections.selectedUid)[0];

                        console.log(section_helpr);

                        if( section_helpr.attr ) {
                            Object.keys(section_helpr.attr).forEach(key => {
                              if (key.startsWith('css_')) {
                                delete section_helpr.attr[key];
                              }
                            });
                        }


                        if( section_helpr.wraps ) {
                             section_helpr.wraps.forEach(wrap => {

                                if( wrap.items ){
                                    wrap.items.forEach(item => {
                                        if( !item.attr ) return;
                                        Object.keys(item.attr).forEach(key => {
                                            if (key.startsWith('css_')) {
                                              delete item.attr[key];
                                ...
                                if( !wrap.attr ) return;
                                Object.keys(wrap.attr).forEach(key => {
                                    if (key.startsWith('css_')) {
                                      delete wrap.attr[key];
                                    }
                                  });
                              });
                        }
    ...
    getGlobalSection: function () {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response.form, function(i, el) {
                    if( _.has(el, 'jsclass') && el.jsclass == 'section' ) {
                        el['ver'] = elements_ver;
                        response.html = response.html.replaceAll('mfn-default-section', 'mfn-'+elements_ver+'-section');
                    }
                    mfn.pagedata.push(el);
                });
    ...
    resetSectionToDefaults: function() {
        ...
        queueMicrotask(() => sidebar_form.open( this.sectionTriggered ));
    },
}
...
$('.mfn-export-import-opt').on('click', function(e) {
    ...
    let filtr = $(this).attr('data-filter');
    if( filtr == 'panel-export-import-presets' ){
        $('#export-presets-data-textarea').val( JSON.stringify(mfn.presets.filter( (it) => it.type == 'custom' )) );
    }
    if( filtr == 'panel-export-import-classes' ){
...
$('.pre-built-opt').on('click', function(e) {
    ...
    $(this).addClass('active');
    $('.header-prebuilt-sections .pre-built-current').text($(this).text());
...
var dynamicData = {
    ...
    init: function() {
        ...
        $(document).on('click', '.is_dynamic_data .mfn-button-dynamic-data', function(e) {
            e.preventDefault();
            dynamicData.id = $(this).closest('.mfn-form-row').attr('data-id');
            dynamicData.type = $(this).closest('.mfn-form-row').attr('data-dynamic');
            dynamicData.post_type = mfn.preview_post_type;
            dynamicData.open();
        });
        ...
        $editpanel.on('click', '.modal-dynamic-data:not(.mfn-dd-element-column, .mfn-dd-element-visual) .modalbox-content ul li a', function(e) {
            ...
            let this_val = $editpanel.find('.mfn-form-row[data-id="'+dynamicData.id+'"] .mfn-field-value').val();

            $('.modal-dynamic-data').removeClass('show');
            if($editpanel.find('.mfn-form-row[data-id="'+dynamicData.id+'"] .mfn-field-value').length) {

                if( type == 'featured_image' ){
                    $editpanel.find('.mfn-form-row[data-id="'+dynamicData.id+'"] .mfn-field-value').val(dd_code).trigger('change');
                }else{
                    $editpanel.find('.mfn-form-row[data-id="'+dynamicData.id+'"] .mfn-field-value').val(this_val+dd_code).trigger('change');
                }
            }
        });
    ...
    open: function() {
        ...
        if( $queried.length ) {
            ...
            if( dd_query_type == 'terms' ){
                ...
                label_prefix = dd_posttype;
                dynamicData.post_type = dd_posttype;
            }
        }
        ...
        // set dynamic options
        dd_fields = dynamic_data.dynamic[dd_query_type][dynamicData.type];

        if( typeof dynamic_data.dynamic[dd_query_type] !== 'undefined' && typeof dynamic_data.dynamic[dd_query_type][edited_item.type] !== 'undefined' && typeof dynamic_data.dynamic[dd_query_type][edited_item.type][dynamicData.type] !== 'undefined' ) {
            dd_fields = dd_fields.concat( dynamic_data.dynamic[dd_query_type][edited_item.type][dynamicData.type] );
        }

        if( dd_posttype != '' && typeof dynamic_data.dynamic[dd_query_type][dd_posttype] !== 'undefined' ) {

            if( typeof dynamic_data.dynamic[dd_query_type][dd_posttype][dynamicData.type] !== 'undefined' && dynamic_data.dynamic[dd_query_type][dd_posttype][dynamicData.type].length ) {
                // dynamic_data.dynamic[dd_query_type][dd_posttype][dynamicData.type].map( (el, i) => $('.modal-dynamic-data .mfn-dd-dynamic-ul').append('<li><span class="mfn-dd-copy" data-tooltip="Copy"><i class="far fa-copy"></i></span><a data-type="'+type+'" href="#"><span class="mfn-dd-label">'+label_prefix+' '+el.label+'</span><span class="mfn-dd-code">{'+el.key+'}</span></a></li>') );
                dd_fields = dd_fields.concat(dynamic_data.dynamic[dd_query_type][dd_posttype][dynamicData.type]);
            }
        ...
        if( dd_fields.length ) {
            ...
            dd_fields.map( (el, i) => $('.modal-dynamic-data .mfn-dd-dynamic-ul').append('<li class="mfn-dd-li-'+el.key.replaceAll(':', '-').replaceAll('_', '-')+'" data-name="'+label_prefix+' '+el.label+' '+el.key+'"><span class="mfn-dd-copy" data-tooltip="Copy"><i class="far fa-copy"></i></span><a data-type="'+dynamicData.type+'" href="#"><span class="mfn-dd-label">'+label_prefix+' '+el.label+'</span><span class="mfn-dd-code">{'+el.key+'}</span></a></li>') );
        }

        if( typeof dynamic_data.user[dynamicData.type] != 'undefined' ) {
            dynamic_data.user[dynamicData.type].map( (el, i) => $('.modal-dynamic-data .mfn-dd-user-ul').append('<li><span class="mfn-dd-copy" data-tooltip="Copy"><i class="far fa-copy"></i></span><a data-type="'+dynamicData.type+'" href="#"><span class="mfn-dd-label">'+el.label+'</span><span class="mfn-dd-code">{'+el.key+'}</span></a></li>') );

        }

        if( dd_query_type == 'posts' && typeof dynamic_data.author[dynamicData.type] != 'undefined' ) {
            dynamic_data.author[dynamicData.type].map( (el, i) => $('.modal-dynamic-data .mfn-dd-author-ul').append('<li><span class="mfn-dd-copy" data-tooltip="Copy"><i class="far fa-copy"></i></span><a data-type="'+dynamicData.type+'" href="#"><span class="mfn-dd-label">'+el.label+'</span><span class="mfn-dd-code">{'+el.key+'}</span></a></li>') );
        }

        if( typeof dynamic_data.global[dynamicData.type] != 'undefined' ) {
            dynamic_data.global[dynamicData.type].map( (el, i) => $('.modal-dynamic-data .mfn-dd-global-ul').append('<li><span class="mfn-dd-copy" data-tooltip="Copy"><i class="far fa-copy"></i></span><a data-type="'+dynamicData.type+'" href="#"><span class="mfn-dd-label">'+el.label+'</span><span class="mfn-dd-code">{'+el.key+'}</span></a></li>') );
        }
        ...
        if( dynamicData.post_type ){
            dynamicData.ajax(dynamicData.post_type, 'postmeta');
            content_html += `
              <div class="mfn-dd-notbg mfn-dd-type-wrapper mfn-dd-type-wrapper-cleared mfn-dd-type-wrapper-${dynamicData.post_type}-postmeta mfn-dd-type-wrapper-custom-fields">
                <h5>Custom fields</h5>
                <ul>
                </ul>
              </div>`;
        ...
        if( mfn.post_types.length && !$queried.length ) {
          mfn.post_types.forEach(pt => {

            // get posts ajax

            dynamicData.ajax(pt.name, 'posts');
            
            content_html += `
              <div class="mfn-dd-notbg mfn-dd-type-wrapper mfn-dd-type-wrapper-cleared mfn-dd-dynamic-set mfn-dd-type-wrapper-${pt.name}-posts">
                <h5>${pt.label}</h5>
                <ul></ul>
              </div>`;

            if( _.has(pt, 'mfn_tax') ) {
              _.map(pt['mfn_tax'], function(tx) {
                if( _.has(tx, 'items') && tx.items.length ) {
                  content_html += `
                    <div class="mfn-dd-notbg mfn-dd-type-wrapper mfn-dd-type-wrapper-cleared mfn-dd-type-wrapper-${pt.name}-terms">
                      <h5>${tx.label}</h5>
                      <ul>
                        ${ _.map(tx.items, (i) => `
                          <li data-name="${i.name.toLowerCase()}">
                            <span class="mfn-dd-copy" data-tooltip="Copy">
                              <i class="far fa-copy"></i>
                            </span>
                            <a href="#" data-subtype=":${i.term_id}:term" data-type="${dynamicData.type}">
                              <span class="mfn-dd-label">${i.name}</span>
                              <span class="mfn-dd-code">{${dynamicData.type}:${i.term_id}:term}</span>
                            </a>
                          </li>` ).join('') }
                      </ul>
                    </div>`;
        ...
        $('.modal-dynamic-data .modalbox-content').append(content_html);
        ...
        $('.modal-dynamic-data .modalbox-content .mfn-dd-dynamic-set ul li a').each(function() {
            ...
            $(this).attr('data-type', dynamicData.type);
            $(this).find('.mfn-dd-code').text('{'+dynamicData.type+subtype+'}');

            if( $(this).closest('.mfn-dd-type-wrapper').hasClass('mfn-dd-type-wrapper-global') && label == 'image' ){
                // exception for logo
                $(this).find('.mfn-dd-label span').text('logo');
            }else{
                $(this).find('.mfn-dd-label span').text(dynamicData.type);
            }
    ...
    search: function() {
        ...
        $(document).on('keyup paste change', '.modal-dynamic-data .modalbox-header .mfn-search', function() {
            var val = $(this).val().toLowerCase();
    ...
    ajax: function(post_type, data_type) {
        $.ajax({
            url: mfnajaxurl,
            data: {
                action: 'mfn_dynamic_get_items',
                ...
                type: data_type,
                post_type: post_type,
                search: $('.modal-dynamic-data .modalbox-header .mfn-search').val()
            },
            type: 'POST',
            success: function(response) {

                let obj_to_loop = _.has(response, post_type) && _.has(response[post_type], 'options') ? response[post_type]['options'] : response;

                if( obj_to_loop.length ) {
                    _.map( obj_to_loop, function(pt) {
                        if( data_type != 'postmeta' || ( !pt.startsWith('_') || ( pt.startsWith('_') && !response.includes(pt.slice(1)) ) ) ){

                        $('.modal-dynamic-data .mfn-dd-type-wrapper-'+post_type+'-'+data_type+' ul').append(`<li data-name="${ data_type == 'postmeta' ? pt : pt.title.toLowerCase() }">
                            <span class="mfn-dd-copy" data-tooltip="Copy">
                              <i class="far fa-copy"></i>
                            </span>
                            <a href="#" data-subtype=":${ data_type == 'postmeta' ? pt : pt.id }" data-type="${ data_type == 'postmeta' ? 'postmeta' : dynamicData.type }">
                              <span class="mfn-dd-label">${ data_type == 'postmeta' ? pt : pt.title }</span>
                              <span class="mfn-dd-code">{${ data_type == 'postmeta' ? 'postmeta' : dynamicData.type }:${ data_type == 'postmeta' ? pt : pt.id }}</span>
                            </a>
                          </li>`);
                        }
                    }).join('');
                }else{
                    $('.modal-dynamic-data .mfn-dd-type-wrapper-'+post_type+'-'+data_type).remove();
                }
            }
...
function switchPreview(preview_type) {
    ...
    if( sidebar_form.is_themeoptions ){
        sidebar_themeoptions.render();
    }else if( sidebar_form.is_pageoptions ){
        page_options_tab.render();
    }else{
        sidebar_form.render();
    }
    ...
    if( $builder.find('.mfn-looped-items-slider-wrapper').length ){
        $builder.find('.mfn-looped-items-slider-wrapper').each(function() {
            re_render($(this).closest('.mcb-section').attr('data-uid'));
        });
    }
...
let be_navigator = {
                            ...
                            if( !$wrap.hasClass('mfn-global-wrap') ) {
                                ...
                                if( $wrap.find('.mfn-module.vb-item').length ) {
                                    ...
                                    $wrap_childs.children('.mfn-module.vb-item').each(function() {
                                        let $item = $(this);
                                        let itemObj = mfn.pagedata.filter( (item) => item.uid == $item.attr('data-uid') )[0];
                                            ...
                                            if( $item.find('.vb-item').length ){
                                                ...
                                                let $submenu_nested_wrap = $('.mfn-navigator .navigator-tree li.nav-'+$item.attr('data-uid')+' .mfn-sub-nav');
                                                 ...
                                                 $item.find('.vb-item').each(function() {
                                                    ...
                                                    if( !mfn.pagedata.filter( (item) => item.uid == $nested_item.attr('data-uid') ).length ) return;

                                                    let n_itemObj = mfn.pagedata.filter( (item) => item.uid == $nested_item.attr('data-uid') )[0];
        ...
        modules: function() {
            $(".mfn-navigator .mfn-sub-nav-modules").sortable({
                ...
                update: function( event, ui ) {
                    ...
                    if( $item.closest('.mcb-section').hasClass('mfn-section-tabber') ){

                        setTimeout(function() {
                            re_render($item.closest('.mfn-section-tabber').attr('data-uid'));
                        }, 200);
    ...
    ui: function() {
        ...
        $(document).on('click', '.mfn-navigator .navigator-tree li > a', function(e) {
            ...
            if( !$(this).closest('.toggle-disabled').length ) {
                sidebar_form.open($edited_div, true);
            } else {
                backToWidgets();
            }
    ...
    item: function(type, uid, size = false) {
        ...
        let el = mfn.pagedata.filter( (item) => item.uid == uid )[0];
...
let page_options_tab = {
    init: function() {
        $(document).on('click', '.mfn-view-options-tab', function(e) {
            e.preventDefault();
            if (mfn.view == 'demo') return;
            ...
            if( sidebar_form.wyswig_active && !copypaste.uid ) {
                if( $editpanel.find( '.sidebar-panel .mfn-form .html-editor' ).length ) MfnFieldTextarea.destroy();
                if( $editpanel.find( '.sidebar-panel .mfn-form .visual-editor' ).length ) tinymce.execCommand( 'mceRemoveEditor', false, 'mfn-editor' );
            }

            sidebar_form.wyswig_active = false;
            ...
            sidebar_form.is_themeoptions = false;
            sidebar_form.is_class = false;
            sidebar_form.is_pageoptions = true;

            page_options_tab.render();

            $('style.mfn-fields-switcher-normal-hover').remove();
            $('body').append('<style class="mfn-builder-fields-css mfn-fields-switcher-normal-hover">.mfn-ui .mfn-form .mfn-sft-nav li:first-child a{background: var(--mfn-ui-sft-nav-tab-bg-active); color: var(--mfn-ui-sft-nav-tab-color-active);} .mfn-ui .mfn-form .mfn-sft.mfn-sft-normal{display: block;}</style>');
    ...
    render: function() {
        if( sidebar_form.no_render ) return;
        //console.log('page option render');

        $('.mfn-ui .panel-view-options .mfn-form.mfn-form-options').html('');

        const mfn_form_po = new MfnForm( mfn.pageoptions );
        ...
        * */
        ...
        if( $('.panel-view-options .mfn-form .mfn-form-row .sliderbar:not(.mfn-initialized)').length ) {
            $('.panel-view-options .mfn-form .mfn-form-row .sliderbar:not(.mfn-initialized)').each(function() {
                sliderInput.init($(this));
            });
        }
...
var mfnNewFont = {
    ...
    html: function() {
        ...
        return `<div class="mfn-form-row mfn-form-row themeoption"><h5 class="row-header-title">Font ${i}</h5></div>

            <div class="mfn-form-row mfn-form-row themeoption font-custom${i}" data-id="font-custom${i}" data-name="font-custom${i}">
        ...
        <div class="mfn-form-row mfn-form-row themeoption font-custom${i}-woff" data-id="font-custom${i}-woff" data-name="font-custom${i}-woff">
    ...
    <div class="mfn-form-row mfn-form-row themeoption font-custom${i}-ttf" data-id="font-custom${i}-ttf" data-name="font-custom${i}-ttf">
...
$(document).on('click', '.mfn-checkbox', function(e) {
    e.preventDefault();
    let opt = $(this);
    if( opt.hasClass('active') ) {
        opt.removeClass('active');
        if( opt.find('input[type="checkbox"]').length ) opt.find('input[type="checkbox"]').prop('checked', false).trigger('change');
        if( opt.closest('.mfn-publication-opt-wrapper').length ) {
            opt.closest('.mfn-publication-opt-wrapper').removeClass('mfn-opt-active'); // popup publication dates
            opt.closest('.mfn-publication-opt-wrapper').find('.mfn-form-control').val('').trigger('change');
        }

    }else{
        opt.addClass('active');
        if( opt.find('input[type="checkbox"]').length ) opt.find('input[type="checkbox"]').prop('checked', true).trigger('change');
        if( opt.closest('.mfn-publication-opt-wrapper').length ) opt.closest('.mfn-publication-opt-wrapper').addClass('mfn-opt-active'); // popup publication dates
    }
});
...
var sidebar_themeoptions = {
    tab: false,
    // theme options theme_options theme-options
    init: function() {

        $(document).on('click', 'li.menu-themeoptions a', function(e) {
            e.preventDefault();

            if( mfn.view == 'demo' ) return;
            if( !_.has(renderMfnFields, 'themeoptions') ) return;

            sidebar_form.is_themeoptions = true;
            sidebar_form.is_class = false;
            sidebar_form.is_pageoptions = false;
        ...
        $editpanel.on('click', '.vb-themeoptions .vb-to-header a', function(e) {
            e.preventDefault();
            sidebar_themeoptions.tab = $(this).closest('.vb-to');
            ...
            if( $(this).hasClass('vb-themeoptions-link-expander') ){
                if( sidebar_themeoptions.tab.hasClass('active') ){
                    sidebar_themeoptions.tab.removeClass('active');
                    ...
                    sidebar_themeoptions.tab.addClass('active');
                    ...
                    if( sidebar_themeoptions.tab.find('.vb-to-content').length ) sidebar_themeoptions.render();
                ...
                if($tabler.hasClass('vb-to-form-view')) {
                    ...
                    sidebar_themeoptions.tab.find(href).addClass('vb-to-active');

                    mfnoptsinputs.start();
                    if( $('.panel-view-themeoptions .mfn-form .mfn-form-row .sliderbar:not(.mfn-initialized)').length ) {
                        $('.panel-view-themeoptions .mfn-form .mfn-form-row .sliderbar:not(.mfn-initialized)').each(function() {
                            sliderInput.init($(this));
                        });
                    }

                    initSocialSorting(sidebar_themeoptions.tab);
                    initFontSelect(sidebar_themeoptions.tab);
                    if( !$tabler.find('.CodeMirror').length ) MfnFieldTextarea.initForCSSandJS();
    ...
    render: function() {
        sidebar_themeoptions.tab.find('.vb-to-content').each(function(i, el) {
...
if( mfn.current_user_roles.includes('administrator')  ) sidebar_themeoptions.init();
...
function runAjaxElements(){
    ...
    var iframeWin = document.getElementById('mfn-vb-ifr').contentWindow;
    iframeWin.jQuery(iframeWin.document).trigger('mfn:ajax:refresh');
    ...
    if( $content.find('.mfn-advanced-filters').length ) {
...
let mfnAdvancedFilters = {
    form: false,
    init: function() {
      if( !$content.find('.mfn-advanced-filters').length ) return;

      $content.find('.mfn-advanced-filters:not(.initialized)').each(function(i, el) {
    ...
    slider_range: function() {
      mfnAdvancedFilters.form.find('.mfn-advanced-filters-range-slider').each(function(i, slider) {
        ...
        let input_min = $(this).closest('.mfn-form-row-wrapper').find('input.mfn-ui-slider-input-min');
        let input_max = $(this).closest('.mfn-form-row-wrapper').find('input.mfn-ui-slider-input-max');

        let min = input_min.attr('min').length ? input_min.attr('min') : '0';
        let max = input_max.attr('max').length ? input_max.attr('max') : '500';
        ...
        var ToolsTypography = MediumEditor.extensions.form.extend({
            ...
            showForm: function( typo ) {
                ...
                if( !$(input.fontfamily).find('optgroup[label="Google"]').length ){
                    var g_fonts = '';
                    mfn.mfn_google_fonts.map((value) => {
...
function mfnBannerBox() {
    ...
    if( !$content.find('.column_banner_box .hidden-desc').length ) return;
      ...
      $content.find(".mfn-banner-box").each( function() {
        ...
        let container = $(this);
              ...
              if( container.find('.hidden-desc').length ) {
                if( container.find('.hidden-desc').prev().length ) container.find('.hidden-desc').prev().addClass("last-visible-el");

                var hd_h = container.find('.hidden-wrapper').outerHeight();

                container.find('.hidden-desc').css( { '--mfn-banner-box-height': hd_h + 'px' } );
...
$editpanel.on('click', '.panel-edit-item .row-header.toggled_header', function(e) {
    e.preventDefault();
    var header = $(this);
    let component = header.closest('.mfn-component-wrapper');

    if( $('style.mfn-fields-switcher-'+component.attr('data-id')).length ) {
        $('style.mfn-builder-fields-css:not(.mfn-fields-switcher-normal-hover)').remove();
    }else{
        $('style.mfn-builder-fields-css:not(.mfn-fields-switcher-normal-hover)').remove();
        $('body').append('<style class="mfn-builder-fields-css mfn-fields-switcher mfn-fields-switcher-'+component.attr('data-id')+'">.mfn-ui .mfn-form .mfn-section-is-tabber .mfn-component-wrapper-'+component.attr('data-id')+' .mfn-form-row.mfn-alert{ display: flex; } .mfn-ui .mfn-form .mfn-component-wrapper-'+component.attr('data-id')+' .mfn-form-row.conditionally-show{ display: block; } .mfn-class-editor .mfn-element-fields-wrapper .modalbox-card-style .mfn-component-wrapper-'+component.attr('data-id')+' .mfn-form-row.mfn-class-editor-form-row-visible{display: block;} .mfn-element-fields-wrapper .mfn-component-wrapper-'+component.attr('data-id')+' .mfn-toggled{display: block;} .mfn-element-fields-wrapper .mfn-component-wrapper-'+component.attr('data-id')+' .toggled_header h5:before{ transform: rotate(180deg); color: var(--mfn-ui-heading-arrow-active); }</style>');
    }
});

var mfn_label_fields = {
    id: false,
    class: '',
    init: function() {
        $(document).on('mousedown', '.mfn-form-row.toggle_fields > label > .mfn-vb-label-button', function(e) {
            e.preventDefault();

            var box = $(this).closest('.mfn-form-row');
            mfn_label_fields.id = box.attr('data-id');
            mfn_label_fields.class = mfn_label_fields.id.replaceAll(' ', '.');

            initFontSelect(box);

            $('body').append('<style class="mfn-builder-fields-css mfn-builder-label-fields mfn-builder-fields-css-'+ mfn_label_fields.id.replaceAll(' ', '')+'">.mfn-ui .mfn-form .mfn-form-row[data-id="'+mfn_label_fields.class+'"] > label .mfn-vb-label-button {background-color: #72a5d8;} .mfn-ui .mfn-form .mfn-form-row[data-id="'+mfn_label_fields.class+'"] > label .mfn-vb-label-button .mfn-icon-edit { background-image: url('+mfn.themepath+'/visual-builder/assets/svg/options/edit-light.svg); }.mfn-ui .mfn-form .mfn-form-row[data-id="'+mfn_label_fields.class+'"] .mfn-toggle-fields-wrapper{display: block;}</style>');
            $('.sidebar-wrapper').addClass('mfn-vb-sidebar-overlay');
            $(document).bind('mousedown', mfn_label_fields.hide);

        });
    },
    hide: function(e) {
        var div = $('.mfn-element-fields-wrapper .mfn-form-row[data-id="'+mfn_label_fields.class+'"] .mfn-toggle-fields-wrapper');
        var tools_switcher = $('.mfn-element-fields-wrapper .mfn-form-row[data-id="'+mfn_label_fields.class+'"] .mfn-vb-label-button');

        if ( (!div.is(e.target) && div.has(e.target).length === 0 ) || tools_switcher.is(e.target) ) {
            $('style.mfn-builder-label-fields').remove();
            $('.sidebar-wrapper').removeClass('mfn-vb-sidebar-overlay');

            $(document).unbind('mousedown', mfn_label_fields.hide);
        }
    }
}

mfn_label_fields.init();
...
function initFontSelect(box) {
    ...
    if( box.find('.form-group.font-family-select select').length ) {
        box.find('.form-group.font-family-select select').each(function() {
            ...
            if( typeof mfn.mfn_google_fonts !== 'undefined' ) {
              mfn.mfn_google_fonts.forEach((entry) => {
...
$editpanel.on('click', '.mfn-form-row.toggle_fields > label > a.reset-big-field', function(e) {
    e.preventDefault();

    var $editrow = $(this).closest('.mfn-form-row');
    let id = $editrow.attr('data-id');

    if( !_.has(edited_item, 'attr') || !_.has(edited_item['attr'], id) ) return;
...
$editpanel.on('click', '.mfn-form-row.mfn-sidebar-fields-tabs > ul.mfn-sft-nav li a', function(e) {
    ...
    $('style.mfn-fields-switcher-normal-hover').remove();
    $('body').append('<style class="mfn-builder-fields-css mfn-fields-switcher-normal-hover">.mfn-ui .mfn-form .mfn-sft-nav li:nth-child('+index+') a{background: var(--mfn-ui-sft-nav-tab-bg-active); color: var(--mfn-ui-sft-nav-tab-color-active);} .mfn-ui .mfn-form .mfn-sft.mfn-sft-'+tab+'{display: block;}</style>');


    /*$box.find('ul.mfn-sft-nav li').removeClass('active');
    $box.find('.mfn-sft').removeClass('mfn-tabs-fields-active');
    $box.find('.mfn-sft .mfn-form-row').addClass('mfn-toggled');

    $box.find('.mfn-sft-'+tab).addClass('mfn-tabs-fields-active');
    $box.find('.mfn-sft-'+tab+' .mfn-form-row').removeClass('mfn-toggled');

    if( !$box.find('.mfn-sft-'+tab+' .mfn-form-row.mfn-fields-switcher .segmented-options li.active').length ){
        $box.find('.mfn-sft-'+tab+' .mfn-form-row.mfn-fields-switcher .segmented-options li:first-child a').trigger('click');
    }
...
$editpanel.on('click', '.panel-edit-item ul.sidebar-panel-content-tabs > li', function(e) {
    ...
    var tab = $(this).attr('data-tab');
    $('.mfn-ui').removeClass('modalbox-card-active-content modalbox-card-active-style modalbox-card-active-advanced').addClass('modalbox-card-active-'+tab);
    ...
    if( !$(this).hasClass('active') ) {
        ...
        if( $(this).hasClass('spct-li-advanced') || $(this).hasClass('spct-li-style') ){
            let fid = $('.mfn-element-fields-wrapper .modalbox-card.modalbox-card-'+tab+' .mfn-component-wrapper:first-child');

            if( !fid.find('.toggled_header:not(.conditionally-hide)').length ){
                fid = $('.mfn-element-fields-wrapper .modalbox-card.modalbox-card-'+tab+' .mfn-component-wrapper:nth-child(2)');
            }

            if( !fid.find('.toggled_header:not(.conditionally-hide)').length ){
                fid = $('.mfn-element-fields-wrapper .modalbox-card.modalbox-card-'+tab+' .mfn-component-wrapper:nth-child(3)');
            }

            if( $('.mfn-element-fields-wrapper .modalbox-card.modalbox-card-'+tab+' .mfn-component-wrapper').length ){
                if( !$('style.mfn-fields-switcher-'+fid.attr('data-id')).length ){
                    fid.find('.toggled_header:not(.conditionally-hide)').trigger('click');
                }
            }
        }
...
$editpanel.on('change', '.has-default-unit.mfn-field-value', function() {
    ...
    if( $field.val().length ) {
        ...
        if(units_check == false) $field.val(val+$field.attr('data-unit')).trigger('change');
    }
...
$editpanel.on('change', '.inline-style-input .mfn-field-value, .inline-style-input input[type="checkbox"]', function() {
    ...
    if( $field.closest('.mfn-form-row').hasClass('custom-background-size-section') && val == 'cover-ultrawide' ) {
        $content.find('.'+it).addClass('bg-cover-ultrawide');
    }else if( $field.closest('.mfn-form-row').hasClass('custom-background-size-section') && val != 'cover-ultrawide' ) {
...
function prepareCss(csspath, cssstyle, val, uid, rwd) {
...
function grabFieldStyle($field){
    ...
    let $box = $field.closest('.mfn-form-row');
    let val = $field.attr('type') == 'checkbox' && !$field.prop('checked') ? '' : $field.val();
    let it =  $box.closest('.mfn-element-fields-wrapper').length ? $box.closest('.mfn-element-fields-wrapper').attr('data-element') : 'pageopt';
    //let uid = it == 'pageopt' ? mfn.pageid : $content.find('.'+it).attr('data-uid');
    let uid = edited_item.uid == 'pageoptions' ? mfn.pageid : edited_item.uid;
    let rwd = 'desktop';

    if( $box.hasClass('mfn_field_rwd') ) rwd = screen;

    if( $box.closest('.transform_field').length ){
        return;
    }

    if( $box.closest('.inline-style-input.gradient').length ) $box = $box.closest('.inline-style-input.gradient');
    ...
    if( sidebar_form.is_class ) {
        it =  edited_item.uid;
        uid = edited_item.uid;
        csspath = csspath.replaceAll('mcb-item-mfnuidelement', uid).replaceAll('mcb-wrap-mfnuidelement', ''+uid).replaceAll('mcb-section-mfnuidelement', uid);
    }

    if( _.has( additional_css, id) ) {
        if( val.length && _.has( additional_css[id]['rewrites'], val) ) {
            addLocalStyle(additional_css[id]['selector'].replace('mfnuidelement', edited_item.uid), additional_css[id]['rewrites'][val], additional_css[id]['style'], rwd, uid);
        }else{
            addLocalStyle(additional_css[id]['selector'].replace('mfnuidelement', edited_item.uid), '', additional_css[id]['style'], rwd, uid);
        }
    }
    ...
    if( val == 'cover-ultrawide' ){
        val = '';
    }

    if( csspath == '.mcb-section .mcb-wrap .mcb-item-mfnuidelement' && cssstyle == 'width' ) {
        if($('body').hasClass('mfn-navigator-active') ) be_navigator.show(edited_item.uid);
    }
    ...
    if( ( cssstyle == 'background-image' || cssstyle == '-webkit-mask-image' ) && val.length && !$box.hasClass('gradient') && !$builder.find('.'+it).closest('.mfn-looped-items').length ) {
    ...
    if( cssstyle == 'background-size' && val.length && val == 'custom' ) {
        val = '';
    }
    
    if( cssstyle == 'background-position' && val.length && val == 'custom' ) {
        val = '';
    }
    ...
    /*if( sidebar_form.is_class ){
        setTimeout(function() {
            mfn_classes.has_changed = edited_item;
        }, 100);
    }*/
...
function setSizeLabels(){
    $content.find('.mfn-size-label .mfn-element-size-label').each(function() {
        ...
        let curr_object = mfn.pagedata.filter( (item) => item.uid == uid )[0];
...
var mfnoptsinputs = {
        ...
        items.each(function() {
            ...
            if (Array.isArray(mfnoptsinputs.condition)) {
                ...
                mfnoptsinputs.condition.map( (c) => {
                    if( !$('.mfn-ui .mfn-form #'+c.id+'.watchChanges').length ){
                        $(this).addClass('conditionally-hide');
                        $('.mfn-ui .mfn-form  #'+c.id).addClass('watchChanges');
                        prepareValues = true;
                    }
                });

            }else{
                if( !$('.mfn-ui .mfn-form #'+mfnoptsinputs.condition.id+'.watchChanges').length ){
                    $(this).addClass('conditionally-hide');
                    $('.mfn-ui .mfn-form  #'+mfnoptsinputs.condition.id).addClass('watchChanges');
                    prepareValues = true;
                }
            }
  ...
  watchChanges: function() {
    ...
    $(document).on('change', '.watchChanges .mfn-field-value, .watchChanges .condition-field, .watchChanges .field-to-object', function() {
  ...
  showhidefields: function(formrow) {
    ...
    if (Array.isArray(mfnoptsinputs.condition)) {
        ...
        mfnoptsinputs.condition.map( (c) => {
            ...
            if( $('#'+c.id+'.mfn-form-row .single-segmented-option.segmented-options').length || $('#'+c.id+'.mfn-form-row .visual-options').length || $('#'+c.id+'.mfn-form-row .mfn-switch').length ) {
                val = $('#'+c.id+'.mfn-form-row input:checked').val();
            }else{
                val = $('#'+c.id+'.mfn-form-row .mfn-field-value, #'+c.id+'.mfn-form-row .condition-field, #'+c.id+'.mfn-form-row .field-to-object').val();
            }
...
const Transforms = {
    ...
    inputs: [
        ...
        { key: 'scaleX',      domLocation_value: 'input.mfn-field-value[data-key="scaleX"]', defaultValue: 1 },
        { key: 'skewY',       domLocation_value: 'input.mfn-field-value[data-key="skewY"]',  defaultValue: 0  },
        { key: 'skewX',       domLocation_value: 'input.mfn-field-value[data-key="skewX"]',  defaultValue: 0  },
        { key: 'scaleY',      domLocation_value: 'input.mfn-field-value[data-key="scaleY"]', defaultValue: 1  },
        { key: 'translateX',  domLocation_value: 'input.mfn-field-value[data-key="translateX"]', defaultValue: 0  },
        { key: 'translateY',  domLocation_value: 'input.mfn-field-value[data-key="translateY"]', defaultValue: 0  },

        // Not in matrix array, should be always last arr element!
        { key: 'rotate',      domLocation_value: 'input.mfn-field-value[data-key="rotate"]', defaultValue: 0  },
    ],
    ...
    loopThrouInputs() {
        ...
        for(let transform of this.inputs) {
            let queriedDOM = $(this.sidebarMenu).find('.transform_field ' + transform['domLocation_value']);
    ...
    renderTransforms(){
        ...
        let id = this.sidebarMenu.closest('.transform').attr('data-id');
        ...
        if( typeof edited_item['attr'][id]['val'][screen] === 'undefined' ) edited_item['attr'][id]['val'][screen] = {};

        edited_item['attr'][id]['selector'] = domLocation;
        edited_item['attr'][id]['style'] = style;

        edited_item['attr'][id]['val'][screen] = {
            ...this.readyObject,
...
$editpanel.on('change', '.panel-edit-item .mfn-form .field-to-object, .panel-edit-item .mfn-form .mfn-field-value', function() {
    if( $(this).val().startsWith('--') && !$(this).val().includes('var(') && $(this).val() != '--' ){
        $(this).val('var('+$(this).val()+')');
    }

    updateObject($(this));
}).on('focus', '.panel-edit-item .mfn-form .field-to-object, .panel-edit-item .mfn-form .mfn-field-value', function() {
    sidebar_form.focus = '[name="'+$(this).attr('name')+'"]';
    if( typeof $(this).attr('data-key') != 'undefined' ) sidebar_form.focus += '[data-key="'+$(this).attr('data-key')+'"]';
}).on('blur', '.panel-edit-item .mfn-form .field-to-object, .panel-edit-item .mfn-form .mfn-field-value', function() {
    sidebar_form.focus = false;
});


$(document).on('change', '.mfn-element-fields-wrapper .query_post_type .mfn-field-value', function() {
    mfn['current_post_type'] = $(this).val();
});

$(document).on('change', '.mfn-element-fields-wrapper .filters.post_type .mfn-field-value', function(e) {

    if( edited_item?.attr?.filters_fields && _.has(edited_item?.attr?.post_type) ) {

        $('.mfn-ui').addClass('mfn-modal-open').append('<div class="mfn-modal modal-confirm show"> <div class="mfn-modalbox mfn-form mfn-shadow-1"> <div class="modalbox-header"> <div class="options-group"> <div class="modalbox-title-group"> <span class="modalbox-icon mfn-icon-delete"></span> <div class="modalbox-desc"> <h4 class="modalbox-title">Post type change detected</h4> </div></div></div><div class="options-group"> <a class="mfn-option-btn mfn-option-blank btn-large btn-modal-close btn-modal-abort" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a> </div></div><div class="modalbox-content"> <img class="icon" alt="" src="'+mfn.themepath+'/muffin-options/svg/warning.svg"> <h3>Post type conflict</h3> <p>You’re using taxonomies in your filters that aren’t assigned to the newly selected post type. If you continue, these filters will be removed. You can also cancel this action and keep your current filters.</p><a class="mfn-btn btn-modal-abort" href="#"><span class="btn-wrapper">Cancel</span></a> <a class="mfn-btn mfn-btn-red btn-wide btn-modal-confirm" href="#"><span class="btn-wrapper">Continue</span></a> </div></div></div>');
        ...
        $('.btn-modal-abort').on('click', function(e) {
            ...
            $('.mfn-element-fields-wrapper .filters.post_type .mfn-field-value').val(mfn['current_post_type']);

            edited_item.attr.post_type = mfn['current_post_type'];
        });
        ...
        $('.btn-modal-confirm').on('click', function(e) {
            ...
            delete(edited_item.attr.filters_fields);
...
$(document).on('change', '.mfn-element-fields-wrapper .query_terms_taxonomy .mfn-field-value', function() {
    mfn['current_tax'] = $(this).val();
});

$(document).on('click', '.mfn-reset-val-icon', function(e) {
    e.preventDefault();
    let id = $(this).closest('.mfn-form-row').attr('data-id');
    if( _.has(edited_item['attr'], id) ) delete(edited_item['attr'][id]);
    
    setTimeout(function() {
        loopAllStyleFields(edited_item.uid);
        if( _.has(edited_item, 'be_classes') && edited_item.be_classes.length ) loopAllStyleFields(edited_item.be_classes[0]);
        if( !id.includes('css_') ) re_render();
        sidebar_form.render();
    },10);

});

function updateObject( $field ){
    var it = $field.closest('.mfn-element-fields-wrapper').attr('data-element');
    var $editrow = $field.closest('.mfn-form-row');
    var $box = $field.closest('.mfn_input_wrapper');

    if( $editrow.hasClass('themeoption') ) return;
    if( $editrow.hasClass('pageoption') ) return;
    if( $editrow.hasClass('css_advanced_transform') || $editrow.hasClass('css_advanced_transform_hover') ) return;
    if( $editrow.find('.checkboxes.pseudo').length ) return;

    if( !_.has(edited_item, 'attr') ) edited_item['attr'] = {};

    if (Array.isArray(edited_item['attr'])) edited_item['attr'] = Object.assign({}, edited_item['attr']);

    var field_name = typeof $field.attr('name') !== 'undefined' ? $field.attr('name') : $field.attr('data-name');
    var selector = '';
    var style = '';
    var key = '';
    var order = 0; // for tabs
    var val = $field.is(':checkbox') && !$field.prop('checked') ? '' : $field.val(); // checkbox unclickable
    var has_updated = false;


    //console.log(val);

    if( $editrow.hasClass('object-css-input') && !$editrow.closest('.mfn-hotspot-point').length ) {
        selector = $editrow.attr('data-csspath');
        style = $editrow.attr('data-name');

        if( !_.has(edited_item['attr'], field_name) || typeof edited_item['attr'][field_name] == 'string' ) edited_item['attr'][field_name] = {};
        if( !_.has(edited_item['attr'][field_name], 'val') || typeof edited_item['attr'][field_name]['val'] == 'string' ) edited_item['attr'][field_name]['val'] = {};
        if( $field.closest('.mfn-form-row.mfn_field_rwd').length && !_.has(edited_item['attr'][field_name]['val'], screen) ) edited_item['attr'][field_name]['val'][screen] = {};
        if( $field.closest('.mfn-form-row.mfn_field_rwd').length && _.has(edited_item['attr'][field_name]['val'], screen) && Array.isArray(edited_item['attr'][field_name]['val'][screen])) edited_item['attr'][field_name]['val'][screen] = Object.assign({}, edited_item['attr'][field_name]['val'][screen]);
    }

    if( $field.closest('.typography').length && typeof $field.attr('data-key') !== 'undefined' ){
        key = $field.attr('data-key');
        selector = $field.closest('.typography').attr('data-csspath');
        style = 'typography';
        if( key.includes('font-family') && val.length ) val = val.replace('#', '');
    }else if( $editrow.closest('.inline-style-input.gradient').length || $editrow.hasClass('backdrop-filter') || $editrow.hasClass('filter') ) {
        key = $field.attr('data-key');
        selector = $editrow.closest('.inline-style-input').attr('data-csspath');
        style = $editrow.closest('.inline-style-input').attr('data-name');
    }else if( $editrow.find('.separated-fields').length && typeof $field.attr('data-key') !== 'undefined' ){
        key = $field.attr('data-key');
    }else if( $editrow.hasClass('mfn-tabs-row') && $field.closest('li.tab').length ){

        if( field_name.includes('tabs[') ) field_name = 'tabs';

        key = $field.attr('data-label');
        order = $field.attr('data-order');
        if( !_.has(edited_item['attr'][field_name], order) ) edited_item['attr'][field_name][order] = {};
        if( !_.has(edited_item['attr'][field_name][order], key) ) edited_item['attr'][field_name][order][key] = {};

        if( val.length ){
            edited_item['attr'][field_name][order][key] = val;
        }else if( typeof edited_item['attr'][field_name][order][key] != 'undefined' ) {
            delete(edited_item['attr'][field_name][order][key]);
        }

        has_updated = true;
    }else if( $editrow.closest('.mfn-multifield-point').length ){

        let field_id = $editrow.closest('.mfn-multifield-point').closest('.mfn-form-row').attr('data-name');
        let point_hash = $editrow.closest('.mfn-multifield-point').attr('id');

        if( edited_item['attr'][field_id].filter(item => item.hash == point_hash ).length ) {

            let ht = edited_item['attr'][field_id].filter(item => item.hash == point_hash )[0];

            if( field_name.includes('css_') ) {

                if( typeof ht['val'] == 'undefined' ) ht['val'] = {};

                if( _.has(ht['val'], field_name) ) {
                    if( $editrow.hasClass('mfn_field_rwd') ){
                        ht['val'][field_name]['val'][screen] = val;
                    }else{
                        ht['val'][field_name]['val'] = val;
                    }
                }else{
                    var tmp_val = {};
                    tmp_val['style'] = $editrow.attr('data-name');
                    tmp_val['selector'] = $editrow.attr('data-csspath');

                    if( $field.closest('.mfn-form-row').hasClass('mfn_field_rwd') ){
                        if( !_.has(tmp_val, 'val') ) tmp_val['val'] = {};
                        tmp_val['val'][screen] = val;
                    }else{
                        tmp_val['val'] = val;
                    }

                    ht['val'][field_name] = tmp_val;
                }
            }else{
                if( $field.closest('.mfn-form-row').hasClass('mfn_field_rwd') ){
                    if( !_.has(ht, field_name) || typeof ht[field_name] == 'string' ) ht[field_name] = {};
                    ht[field_name][screen] = val;
                }else{
                    ht[field_name] = val;
                }
    ...
    if( !has_updated ) {
        if( !_.isEmpty(val) || $field.closest('.mfn-placeholder-inherited-class').length ) {

            if( $editrow.hasClass('object-css-input') ){

                edited_item['attr'][field_name]['selector'] = selector;
                edited_item['attr'][field_name]['style'] = style;

                if( key.length ){
                    if( $editrow.hasClass('mfn_field_rwd') ){
                        edited_item['attr'][field_name]['val'][screen][key] = val;
                    }else{
                        edited_item['attr'][field_name]['val'][key] = val;
                    }
                }else{
                    if( $editrow.hasClass('mfn_field_rwd') ){
                        edited_item['attr'][field_name]['val'][screen] = val;
                    }else{
                        edited_item['attr'][field_name]['val'] = val;
                    }
                }
                ...
                if( $editrow.hasClass('mfn_field_rwd') ) {
                    if( !_.has(edited_item['attr'], field_name ) || typeof edited_item['attr'][field_name] == 'string' || ( typeof edited_item['attr'][field_name] == 'object' && Array.isArray(edited_item['attr'][field_name]) ) ) edited_item['attr'][field_name] = {};
                    edited_item['attr'][field_name][screen] = val;
                }else{
                    edited_item['attr'][field_name] = val;
                }

            }

            if( $field.closest('.mfn-placeholder-inherited-rwd').length || $field.closest('.mfn-placeholder-inherited-class').length || $field.closest('.mfn-placeholder-inherited-std').length ) {
                $field.closest('.mfn-placeholder-inherited-rwd').removeClass('mfn-placeholder-inherited-rwd');
                $field.closest('.mfn-placeholder-inherited-std').removeClass('mfn-placeholder-inherited-std');
                $field.closest('.mfn-placeholder-inherited-class').removeClass('mfn-placeholder-inherited-class');
                if( ($field.closest('.segmented-options').length || $field.hasClass('mfn-form-select')) && !$editrow.find('.mfn-reset-val-icon').length) {
                    $editrow.find('label').first().addClass('form-label-wrapper').append('<a href="#" class="mfn-option-btn mfn-option-blank mfn-reset-val-icon"><span class="mfn-icon mfn-icon-reset"></span></a>');
                }
            }

        }else if( _.isEmpty(val) && (_.has(edited_item['attr'], field_name) || $editrow.hasClass('mfn-placeholder-inherited-class')) ) {

            if( $editrow.hasClass('object-css-input') ) {

                if( key.length ){
                    if( $editrow.hasClass('mfn_field_rwd') ){
                        delete(edited_item['attr'][field_name]['val'][screen][key]);
                    }else{
                        delete(edited_item['attr'][field_name]['val'][key]);
                    }
                }else{
                    if( $editrow.hasClass('mfn_field_rwd') ){
                        delete(edited_item['attr'][field_name]['val'][screen]);
                    }else{
                        delete(edited_item['attr'][field_name]);
                    }
                }

                if( _.has(edited_item['attr'], field_name) && _.has(edited_item['attr'][field_name], 'val') && _.isEmpty( edited_item['attr'][field_name]['val'] ) ) delete(edited_item['attr'][field_name]);

            }else{
                if( $editrow.hasClass('mfn_field_rwd') ){
                    delete(edited_item['attr'][field_name][screen]);
                }else{
                    delete(edited_item['attr'][field_name]);
                }
            }

            setTimeout(sidebar_form.render, 50);
    ...
    if( typeof $field.attr('data-pointobj') !== 'undefined' && $content.find('.'+it+' .mfn-hotspot-style-line .hotspot-image').length ) {
        var c_id = $content.find('.'+it+' .mfn-hotspot-style-line .hotspot-image').attr('id').replace('wrapper', '');
        mfn_multifield_field.refresh(c_id, $field);
    }

    setTimeout( function() {

        if( $content.find('.'+it).hasClass('mfn-section-tabber') && $content.find('.'+it).find('.mfn-tabber-nav-hover-sliding').length ) {
            var iframeWin = document.getElementById('mfn-vb-ifr').contentWindow;
            iframeWin.jQuery(iframeWin.document).trigger('mfn:ajax:refresh');
        }
        ...
        if( ($field.val().length && $field.val().includes('{')) ) {
            ...
            if( $content.find('.'+it).closest('.mfn-looped-items').length || $content.find('.'+it).find('.mfn-looped-items').length || $content.find('.'+it).closest('.mfn-section-tabber').length ) {
        ...
        }else if( $editrow.hasClass('re_render') || $editrow.hasClass('mfn-tabs-row') ) {

            if( $content.find('.'+it).closest('.mfn-looped-items').length || $content.find('.'+it).find('.mfn-looped-items').length || $content.find('.'+it).hasClass('mfn-nested-wrap') ) {
                re_render( $content.find('.'+it).closest('.mcb-section.vb-item').attr('data-uid') );
            }else{
                re_render( edited_item.uid );
            }
        ...
        if( !$editrow.hasClass('re_render') && !$editrow.hasClass('disable-history') && !$content.find('.'+it).closest('.mfn-looped-items').length && !$editrow.find('.multiple-inputs').length ) historyStorage.add();

        if( $editrow.hasClass('banner_box') ) mfnBannerBox();

    }, 50);

    if( $editrow.hasClass('mfn_field_render_sidebar_form') ){
        setTimeout( function() {
             sidebar_form.render();
        }, 60);
    }
...
$editpanel.on('change paste', '.panel-view-options .mfn-form .mfn-field-value, .panel-view-options .mfn-form .mfn-form-checkbox', function() {
    ...
    let editrow = $(this).closest('.mfn-form-row');
    let name = editrow.attr('data-name');
    let id = editrow.attr('data-id');
    let val = $(this).val();
    let key = typeof $(this).attr('data-key') !== 'undefined' ? $(this).attr('data-key') : false;

    if( editrow.find('.checkboxes:not(.pseudo)').length && !$(this).is(':checked') ) val = '';

    if( editrow.hasClass('object-css-input') ) {

        if( typeof edited_item[id] == 'undefined' || typeof edited_item[id] != 'object' ) edited_item[id] = {};
        if( !_.has(edited_item[id], 'val') || typeof edited_item[id]['val'] == 'string' ) edited_item[id]['val'] = {}; 

        edited_item[id]['selector'] = editrow.attr('data-csspath');
        edited_item[id]['style'] = name;

        if( editrow.hasClass('mfn_field_rwd') ) {
            if( !_.has(edited_item[id]['val'], screen) ) {

                if( !_.isEmpty(edited_item[id]['val']) ) {

                    if( !_.has( edited_item[id]['val'], 'desktop' ) && !_.has( edited_item[id]['val'], 'laptop' ) && !_.has( edited_item[id]['val'], 'tablet' ) && !_.has( edited_item[id]['val'], 'mobile' ) ){
                        let tmp_val = JSON.parse( JSON.stringify(edited_item[id]['val']) );
                        edited_item[id]['val'] = {};
                        edited_item[id]['val'][screen] = tmp_val;
                    }

                }else{
                    edited_item[id]['val'][screen] = {};
                }

            }
            
            if( key ){
                if( !_.has(edited_item[id]['val'], screen) ) edited_item[id]['val'][screen] = {};
                edited_item[id]['val'][screen][key] = val;
            }else{
                edited_item[id]['val'][screen] = val;
            }
            
        }else{
            if( key ){
                edited_item[id]['val'][key] = val;
            }else{
                edited_item[id]['val'] = val;
            }
        }
    ...
    if( builder_type == 'header' ) {
        ...
        if( editrow.hasClass('header_mobile') && val == 'enabled' ) {
        ...
        if( editrow.hasClass('header_sticky') && val == 'enabled' ) {
...
function updatePageOpt(name, value){
    ...
    if( _.has(value, 'val') && _.has(value['val'], 'selector') ){
        delete(value['val']['selector']);
    }
    if( _.has(value, 'val') && _.has(value['val'], 'style') ){
        delete(value['val']['style']);
    }
...
$editpanel.on('keyup paste change', '.themeoption.logo-text .mfn-field-value', function() {
    ...
    if( val.length ){
        ...
        $content.find('#Top_bar .logo #logo, #Header_creative .logo #logo').html('<img class="logo-main scale-with-grid " src="'+mfn.themepath+'/images/logo/logo.png" alt="Luk test"><img class="logo-sticky scale-with-grid" src="'+mfn.themepath+'/images/logo/logo.png" alt="Luk test"><img class="logo-mobile scale-with-grid " src="https://muffingroup.com/dev8624/lukas/woo/wp-content/uploads/2021/08/clothingstore-mobile.webp" alt="Luk test"><img class="logo-mobile-sticky scale-with-grid " src="'+mfn.themepath+'/images/logo/logo.png" alt="Luk test">');
    }
});
...
$editpanel.on('change paste', '.panel-view-themeoptions .mfn-form input:not(.mfn-prevent-change), .panel-view-themeoptions .mfn-form .mfn-field-value', function() {
    ...
    if( $editrow.hasClass('themeoption') && ($editrow.hasClass('img-page-bg') || $editrow.hasClass('img-subheader-bg') || $editrow.hasClass('top-bar-bg-img') || $editrow.hasClass('subheader-image') || $editrow.hasClass('footer-bg-img') ) ) {
        ...
        if( value.length ){
            ...
            link.href = mfn.themepath+'/images/favicon.ico';
        }
        ...
        if($editrow.hasClass('re_render_to themeoption')) {
            ...
            if( $editrow.hasClass('re_render_if') ) {
                ...
                if( typeof window.onbeforeunload !== 'function' ) {
                   ...
                   if( to_been_aborted != false ) return; 

                    $('.mfn-ui').addClass('mfn-modal-open').append('<div class="mfn-modal modal-confirm show"> <div class="mfn-modalbox mfn-form mfn-shadow-1"> <div class="modalbox-header"> <div class="options-group"> <div class="modalbox-title-group"> <span class="modalbox-icon mfn-icon-delete"></span> <div class="modalbox-desc"> <h4 class="modalbox-title">Confirm preview</h4> </div></div></div><div class="options-group"> <a class="mfn-option-btn mfn-option-blank btn-large btn-modal-close btn-modal-abort" title="Close" href="#"><span class="mfn-icon mfn-icon-close"></span></a> </div></div><div class="modalbox-content"> <img class="icon" alt="" src="'+mfn.themepath+'/muffin-options/svg/warning.svg"> <h3>Confirm preview</h3> <p>Unsaved changes. Continue?</p><a class="mfn-btn btn-modal-abort" href="#"><span class="btn-wrapper">Let me save first</span></a> <a class="mfn-btn mfn-btn-red btn-wide btn-modal-confirm" href="#"><span class="btn-wrapper">Refresh</span></a> </div></div></div>');

                    $('.btn-modal-abort').on('click', function(e) {
                        e.preventDefault();
                        $('.mfn-ui').removeClass('mfn-modal-open');
                        $('.modal-confirm.show').remove();
                        to_been_aborted = true;
                    });
...
function refreshIframe(){
    ...
    document.getElementById('mfn-preview-wrapper').innerHTML = '<iframe id="mfn-vb-ifr" src="'+mfn.permalink+'" allowfullscreen="1"></iframe>';
...
function loadIframe() {
    ...
    if(mfn.permalink == 'mfn_update_db_required') {

        document.getElementById('mfn-preview-wrapper-holder').innerHTML = '<div class="mfn-bebuilder-iframe-error"><h3>BeBuilder Data Updater</h3><p>Click the button to start the update. <br><strong>Important!</strong> Create a backup of database before starting.</p><a href="#" class="button button-primary mfn-btn-blue mfn-btn mfn-be-css-db-update-action-button">Update Database</a></div>';
        console.error('BeBuilder: Database update required.');
        jQuery('body').removeClass('mfn-preloader-active');

    }else if(mfn.permalink == 'shop_page_id') {
    ...
    }else if( mfn.permalink == 'portfolio_post_type_missing' ) {
    ...
    }else if( mfn.permalink == 'shop_order_missing' ) {
